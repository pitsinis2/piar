(function () {
  ...
  function modalBody(project, area) {
    ...
    if (state.modal === "team") {
      return renderServiceTeamForm(project);
    }
  ...
  function renderServiceTeamForm(project) {
    return `
  ...
  function bindEvents() {
    ...
    document.querySelector("[data-save-team]")?.addEventListener("click", () => {
      const name = document.querySelector("[data-team-name]")?.value || "New team";
    ...
    document.querySelector("[data-add-directory]")?.addEventListener("click", (event) => {
      const sectionKey = event.currentTarget.getAttribute("data-add-directory");
      const section = directoryData[sectionKey];
      if (!section) return;
      const name = document.querySelector("[data-new-directory-name]")?.value || `New ${section.title}`;
      const detail = document.querySelector("[data-new-directory-detail]")?.value || "Created in preview";
      section.items.push({
        name,
