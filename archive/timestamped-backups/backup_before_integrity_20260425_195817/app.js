...
function syncLanguageInUrl(lang) {
  try {
    const validLang = normalizeLanguageCode(lang) || "en";
    const url = new URL(window.location.href);
    if (url.searchParams.get("lang") === validLang) return;
    url.searchParams.set("lang", validLang);
    window.history.replaceState({}, "", url.toString());
  } catch (error) {
    // no-op when URL manipulation is unavailable
  }
}
...
const DEFAULT_NAV_VIEW_ORDER = ["projects", "planner", "teams", "equipment", "clients", "theme", "audit"];
...
function rememberFormSnapshot(key, form) {
  if (!form) return;
  formSnapshots.set(key, captureFormSnapshot(form));
}
...
function getDirtyTrackedForms(keys = Object.keys(TRACKED_FORM_LABELS)) {
  return keys
    .map((key) => {
      ...
      if (captureFormSnapshot(form) === baseline) return null;
...
const els = {
  ...
  plannerMiniCalendar: document.querySelector("#planner-mini-calendar"),
  ...
  plannerWeekRangeToggleBtn: document.querySelector("#planner-week-range-toggle-btn"),
  ...
  plannerProjectPickerDialog: document.querySelector("#planner-project-picker-dialog"),
  plannerProjectPickerForm: document.querySelector("#planner-project-picker-form"),
  plannerProjectPickerSelect: document.querySelector("#planner-project-picker-select"),
  plannerProjectPickerDate: document.querySelector("#planner-project-picker-date"),
  closePlannerProjectPickerBtn: document.querySelector("#close-planner-project-picker-btn"),
  cancelPlannerProjectPickerBtn: document.querySelector("#cancel-planner-project-picker-btn"),
...
function normalizeState(rawState) {
  ...
  nextState.plannerAssignments = Array.isArray(nextState.plannerAssignments)
    ...
    : [];
  nextState.plannerProjectBlocks = Array.isArray(nextState.plannerProjectBlocks)
    ? nextState.plannerProjectBlocks.map(normalizePlannerProjectBlock)
    : [];
...
function createSystemUser({ id = crypto.randomUUID(), name, surname, tel, email, role = "user", qualification = 0, workmode = "none", status = "active", createdAt = new Date().toISOString() }) {
  return {
    id,
    name,
    surname,
...
function bindEvents() {
  ...
  for (const button of els.viewButtons) {
    ...
    button.addEventListener("click", () => {
      if (!confirmDiscardAndMaybeDeleteDraft()) return;
      if (button.dataset.view === "audit" && !isAdmin()) {
        showAppMessage("Only admins can view the full audit log.", "warning", "Audit Log");
        return;
      }
      pushNavigationState();
      currentView = button.dataset.view;
      if (button.dataset.view === "projects" && isMobileProjectViewport()) {
        currentMobileProjectsPane = "list";
      }
      render();
    });
  ...
  els.plannerWeekRangeToggleBtn?.addEventListener("click", togglePlannerWeekRange);
  ...
  els.plannerProjectPickerForm?.addEventListener("submit", onPlannerProjectPickerSave);
  els.closePlannerProjectPickerBtn?.addEventListener("click", closePlannerProjectPickerDialog);
  els.cancelPlannerProjectPickerBtn?.addEventListener("click", closePlannerProjectPickerDialog);
  els.plannerProjectPickerDialog?.addEventListener("cancel", (event) => {
    event.preventDefault();
    closePlannerProjectPickerDialog();
  });
...
function applyAppLanguage() {
  ...
  if (els.appLanguageSelect) {
    if (![...els.appLanguageSelect.options].some((option) => option.value === currentAppLanguage)) {
      currentAppLanguage = "en";
    }
    els.appLanguageSelect.value = currentAppLanguage;
  }
...
function onMemberAdd(event) {
  ...
  const user = existing || createSystemUser({ name, surname, tel, email, role, qualification, workmode });
  if (!existing) {
...
function openPhotoUploadOptionsDialog() {
...
function closePhotoUploadOptionsDialog() {
  pendingPhotoUploadAreaIds = new Set();
...
function toggleMemberForm() {
  isMemberFormExpanded = !isMemberFormExpanded;
...
function renderMemberFormState() {
  ...
  if (els.memberFormTitle) {
    els.memberFormTitle.textContent = editingMemberId ? "Edit Member" : "New Member";
  }
  if (els.memberForm) {
...
function closeEquipmentCreateForm() {
...
function toggleEquipmentForm() {
  isEquipmentFormExpanded = !isEquipmentFormExpanded;
  if (isEquipmentFormExpanded) resetEquipmentForm();
  else editingEquipmentId = null;
...
function toggleEquipmentCategoryForm() {
  ...
  isEquipmentCategoryFormExpanded = !isEquipmentCategoryFormExpanded;
...
function closeEquipmentCategoryForm() {
...
function renderServiceTeamMemberOptions(project = getCurrentProject()) {
  ...
  for (const member of visibleMembers) {
    const label = createCheckRow(
      ...
      member.id,
      getMemberDisplayName(member),
      `${member.tel || "No telephone"} | ${member.email || "No email"}`,
      { metaHtml: `${renderQualificationBadge(member.qualification)}${renderMemberWorkmodeBadge(member.workmode)}` }
    );
    const checkbox = label.querySelector("input");
    if (checkbox) {
...
function getCurrentUserNotifications(project = getCurrentProject()) {
  ...
  return (project?.mentionNotifications || [])
...
function renderNotificationsPanel(project = getCurrentProject()) {
  ...
  for (const entry of notifications.slice(0, 40)) {
    ...
    row.innerHTML = `
      <strong>${escapeHtml(getMemberDisplayName(actor) || "A user")}</strong>
      <span class="muted">mentioned you in this project.</span>
...
function renderViews() {
  ...
  els.appShell?.classList.remove("view-projects", "view-planner", "view-teams", "view-equipment", "view-clients", "view-theme", "view-audit");
...
function renderMemberDetail(member, project = getCurrentProject()) {
  ...
  const scheduleMarkup = memberProjects.length
    ...
    : `<p class="muted">No project schedule assigned yet.</p>`;
  const memberIdLabel = String(member.id || "").split("-")[0]?.toUpperCase() || "—";
  els.memberDetailCard.innerHTML = `
    <div class="directory-detail-header">
      <div class="directory-detail-title-group">
        ...
        <p class="directory-detail-subtitle">${escapeHtml(member.email || "No email")} • ${escapeHtml(member.tel || "No telephone")}</p>
      </div>
      <div class="directory-detail-actions" id="member-detail-actions"></div>
    </div>
    <div class="directory-detail-meta member-detail-summary">
      <span class="meta-pill">ID: ${escapeHtml(memberIdLabel)}</span>
    ...
    row.innerHTML = `
      <div class="directory-select-top">
        <div class="directory-select-title-group">
          ...
          <div class="directory-select-subtitle">${escapeHtml(member.email || "No email")}</div>
        </div>
...
function createPlannerProjectBlock({
  id = crypto.randomUUID(),
  projectId = "",
  startDate = todayInputValue(),
  endDate = startDate,
  ...
  const safeStart = parseIsoDateValue(startDate) ? startDate : todayInputValue();
  const safeEnd = parseIsoDateValue(endDate) ? endDate : safeStart;
  return {
    id,
    projectId,
...
function normalizePlannerProjectBlock(entry) {
  return createPlannerProjectBlock({
    id: entry?.id || crypto.randomUUID(),
    projectId: entry?.projectId || "",
    startDate: entry?.startDate || todayInputValue(),
    endDate: entry?.endDate || entry?.startDate || todayInputValue(),
    createdByUserId: entry?.createdByUserId || "",
    createdAt: entry?.createdAt || new Date().toISOString(),
  });
}
...
function eachIsoDateInRange(startDate, endDate, visitor) {
  const start = parseIsoDateValue(startDate);
  const end = parseIsoDateValue(endDate);
  if (!start || !end) return;
  const from = start <= end ? start : end;
  const to = start <= end ? end : start;
  for (let cursor = new Date(from.getTime()); cursor <= to; cursor.setDate(cursor.getDate() + 1)) {
    visitor(isoDateFromDate(cursor));
  }
}

function getPlannerProjectBlocks() {
  return (state.plannerProjectBlocks || [])
    .map(normalizePlannerProjectBlock)
    .filter((block) => Boolean(getPlannerProjectById(block.projectId)));
}

function isProjectPlacedOnDay(projectId, dateValue, excludeBlockId = "") {
  return getPlannerProjectBlocks().some((block) => {
    if (block.projectId !== projectId || block.id === excludeBlockId) return false;
    return block.startDate <= dateValue && block.endDate >= dateValue;
  });
}

function getPlannerBlocksForDate(dateValue) {
  return getPlannerProjectBlocks()
    .filter((block) => block.startDate <= dateValue && block.endDate >= dateValue)
    .sort((a, b) => {
      const firstProject = getPlannerProjectById(a.projectId);
      const secondProject = getPlannerProjectById(b.projectId);
      return getProjectDisplayName(firstProject || {}, true).localeCompare(getProjectDisplayName(secondProject || {}, true));
    });
}

function canPlaceProjectOnDate(projectId, dateValue, excludeBlockId = "") {
  const project = getPlannerProjectById(projectId);
  if (!project) return false;
  if (!isProjectOpenOnDate(project, dateValue)) return false;
  return !isProjectPlacedOnDay(projectId, dateValue, excludeBlockId);
}

function placeProjectOnDate(projectId, dateValue) {
  const project = getPlannerProjectById(projectId);
  if (!project) return false;
  if (!canManagePlannerProject(projectId)) {
    showAppMessage("Only admins, developers, or assigned managers can plan this project.", "warning", "Planner");
    return false;
  }
  if (!canPlaceProjectOnDate(projectId, dateValue)) {
    showAppMessage("This project is already placed on that day or not active on that date.", "warning", "Planner");
    return false;
  }
  state.plannerProjectBlocks.unshift(createPlannerProjectBlock({
    projectId,
    startDate: dateValue,
    endDate: dateValue,
    createdByUserId: state.currentUserId || "",
  }));
  logAudit("Planner Project Placed", {
    objectType: "planner",
    objectName: getProjectDisplayName(project),
    projectId: project.id,
    projectName: project.name || "Untitled project",
...
function extendPlannerProjectBlock(blockId, targetDate) {
  const block = (state.plannerProjectBlocks || []).find((entry) => entry.id === blockId);
  if (!block) return false;
  const project = getPlannerProjectById(block.projectId);
  if (!project) return false;
  if (!canManagePlannerProject(block.projectId)) {
    showAppMessage("Only admins, developers, or assigned managers can change this plan.", "warning", "Planner");
    return false;
  }
  if (targetDate < block.startDate) {
    showAppMessage("Extend only to the next day(s).", "warning", "Planner");
    return false;
  }

  let conflict = false;
  eachIsoDateInRange(block.startDate, targetDate, (day) => {
    if (!isProjectOpenOnDate(project, day) || isProjectPlacedOnDay(project.id, day, block.id)) conflict = true;
  });
  if (conflict) {
    showAppMessage("Cannot extend: one of the days is already used or outside the project period.", "warning", "Planner");
    return false;
  }
  if (block.endDate === targetDate) return false;
  block.endDate = targetDate;
  logAudit("Planner Project Extended", {
    objectType: "planner",
    objectName: getProjectDisplayName(project),
    projectId: project.id,
    projectName: project.name || "Untitled project",
...
function openPlannerProjectPickerDialog(dateValue) {
  if (!els.plannerProjectPickerDialog || !els.plannerProjectPickerSelect) return;
  plannerProjectPickerDate = dateValue || "";
  const candidates = getPlannerVisibleProjects()
    .filter((project) => canManagePlannerProject(project.id))
    .filter((project) => canPlaceProjectOnDate(project.id, plannerProjectPickerDate));

  if (!candidates.length) {
    showAppMessage("No available projects for this day.", "warning", "Planner");
    return;
  }

  els.plannerProjectPickerSelect.innerHTML = candidates
    .map((project) => `<option value="${project.id}">${escapeHtml(getProjectDisplayName(project))}</option>`)
    .join("");
  if (els.plannerProjectPickerDate) els.plannerProjectPickerDate.value = plannerProjectPickerDate;
  els.plannerProjectPickerDialog.showModal();
}

function closePlannerProjectPickerDialog() {
  plannerProjectPickerDate = "";
  els.plannerProjectPickerForm?.reset();
  if (els.plannerProjectPickerDialog?.open) els.plannerProjectPickerDialog.close();
}

function onPlannerProjectPickerSave(event) {
  event.preventDefault();
  const projectId = els.plannerProjectPickerSelect?.value || "";
  const dateValue = els.plannerProjectPickerDate?.value || plannerProjectPickerDate || "";
  if (!projectId || !dateValue) return;
  const placed = placeProjectOnDate(projectId, dateValue);
  if (placed) closePlannerProjectPickerDialog();
}

function hasPlannerDragPayload(event) {
  const transfer = event?.dataTransfer;
  if (!transfer) return false;
  const types = Array.from(transfer.types || []);
  if (types.includes("application/x-planner-project")) return true;
  if (types.includes("application/x-planner-extend")) return true;
  if (types.includes("text/plain")) return true;
  const plain = transfer.getData("text/plain") || "";
  return plain.startsWith("planner-project:") || plain.startsWith("planner-extend:");
}
...
function shiftPlannerPeriod(direction) {
  const step = plannerWeekDayCount;
...
function togglePlannerWeekRange() {
  plannerWeekDayCount = plannerWeekDayCount === 7 ? 5 : 7;
  renderPlanner();
}
...
function closePlannerAssignmentDialog() {
...
function buildPlannerPoolPanel(dateValue, teamRecords, options = {}) {
  ...
  section.className = `planner-pool-panel${options.horizontal ? " horizontal" : ""}${options.compact ? " compact" : ""}`;
  ...
  for (const record of availableTeams) {
    const chip = buildPlannerTeamChip(record);
    if (options.compact) {
...
function buildPlannerWeekBoard(dates, teamRecords) {
  ...
  for (const dateValue of dates) {
    ...
    dayColumn.className = "planner-day-column planner-drop-day-column";
    ...
    dropHint.className = "planner-day-drop-hint";
    dropHint.textContent = "Drag project card handle to next day to extend";
    ...
    addRow.className = "planner-day-add-row";
    const addBtn = document.createElement("button");
    addBtn.type = "button";
    addBtn.className = "icon-btn planner-day-add-project-btn";
    addBtn.textContent = "+";
    addBtn.title = "Add project";
    addBtn.addEventListener("click", () => openPlannerProjectPickerDialog(dateValue));
    addRow.append(addBtn);
    dayColumn.append(addRow);
    dayColumn.append(dropHint);

    const stack = document.createElement("div");
    stack.className = "planner-project-stack";
    const dayBlocks = getPlannerBlocksForDate(dateValue);
    if (!dayBlocks.length) {
      stack.append(createProjectDetailsEmptyState("No projects planned", "Drag a project from the left column."));
    } else {
      for (const block of dayBlocks) {
        const project = getPlannerProjectById(block.projectId);
        if (!project) continue;
        const card = document.createElement("article");
        card.className = "planner-project-card planner-project-scheduled-card";
        const theme = buildProjectTheme(project.surfaceColor || "#fffaf2");
        card.style.setProperty("--planner-project-surface", theme.surface);
        card.style.setProperty("--planner-project-soft", theme.soft);
        card.style.setProperty("--planner-project-frame", theme.frame);
        card.style.setProperty("--planner-project-card", theme.card);
        const isBlockEnd = block.endDate === dateValue;
        card.innerHTML = `
          <div class="planner-project-card-header">
            <div>
              <strong>${escapeHtml(getProjectDisplayName(project))}</strong>
              <span>${escapeHtml(block.startDate)} - ${escapeHtml(block.endDate)}</span>
            </div>
            ${isBlockEnd ? '<span class="meta-pill">End</span>' : '<span class="meta-pill">Middle</span>'}
          </div>
        `;

        if (isBlockEnd && canManagePlannerProject(project.id)) {
          const handle = document.createElement("button");
          handle.type = "button";
          handle.className = "planner-extend-handle";
          handle.textContent = "Pull to extend";
          handle.draggable = true;
          handle.addEventListener("dragstart", (event) => {
            event.dataTransfer?.setData("text/plain", `planner-extend:${block.id}`);
            event.dataTransfer?.setData("application/x-planner-extend", block.id);
    ...
    dayColumn.append(stack);

    dayColumn.addEventListener("dragover", (event) => {
      if (!hasPlannerDragPayload(event)) return;
      event.preventDefault();
      if (event.dataTransfer) event.dataTransfer.dropEffect = "copy";
      dayColumn.classList.add("drag-over");
    });
    dayColumn.addEventListener("dragleave", () => dayColumn.classList.remove("drag-over"));
    dayColumn.addEventListener("drop", (event) => {
      event.preventDefault();
      dayColumn.classList.remove("drag-over");
      const payload = event.dataTransfer?.getData("text/plain")
        || (event.dataTransfer?.getData("application/x-planner-project")
          ? `planner-project:${event.dataTransfer.getData("application/x-planner-project")}`
          : "")
        || (event.dataTransfer?.getData("application/x-planner-extend")
          ? `planner-extend:${event.dataTransfer.getData("application/x-planner-extend")}`
          : "");
      if (payload.startsWith("planner-project:")) {
        const projectId = payload.slice("planner-project:".length);
        placeProjectOnDate(projectId, dateValue);
        return;
      }
      if (payload.startsWith("planner-extend:")) {
        const blockId = payload.slice("planner-extend:".length);
        extendPlannerProjectBlock(blockId, dateValue);
      }
    });
...
function buildPlannerCalendarBoard(dates) {
  const board = document.createElement("div");
  board.className = "planner-week-board planner-calendar-board";
  for (const dateValue of dates) {
    const dayDate = parseIsoDateValue(dateValue);
    const dayColumn = document.createElement("section");
    dayColumn.className = "planner-day-column planner-calendar-column";
    const dayIndex = dayDate ? dayDate.getDay() : 1;
    const isWeekend = dayIndex === 0 || dayIndex === 6;

    const weekday = dayDate
      ? dayDate.toLocaleDateString("en-GB", { weekday: "short" })
      : "";
    const dayPart = dayDate
    ...
    const header = document.createElement("header");
    header.className = `planner-day-column-header planner-calendar-header${isWeekend ? " weekend" : ""}`;
    header.innerHTML = `<strong>${escapeHtml(`${weekday}-${dayPart}-${month}`)}</strong>`;

    const body = document.createElement("div");
    body.className = "planner-calendar-body";

    dayColumn.append(header, body);
    board.append(dayColumn);
  }
  return board;
}

function renderPlannerMiniCalendar(anchorIsoDate, selectedDates) {
  if (!els.plannerMiniCalendar) return;
  const anchor = parseIsoDateValue(anchorIsoDate) || new Date();
  const monthAnchorDate = parseIsoDateValue(plannerMiniMonthAnchor) || anchor;
  const monthStart = new Date(monthAnchorDate.getFullYear(), monthAnchorDate.getMonth(), 1, 12, 0, 0, 0);
  ...
  wrap.className = "planner-mini-calendar-shell";
  wrap.innerHTML = `
    <div class="planner-mini-calendar-top">
      <div class="planner-mini-calendar-title">${escapeHtml(monthLabel)}</div>
      <div class="planner-mini-calendar-nav">
        <button id="planner-mini-prev-month-btn" class="icon-btn" type="button" aria-label="Previous month">&#8249;</button>
        <button id="planner-mini-next-month-btn" class="icon-btn" type="button" aria-label="Next month">&#8250;</button>
      </div>
    </div>
    <div class="planner-mini-calendar-weekdays">
      <span class="weekno">CW</span>
      <span>M</span><span>D</span><span>M</span><span>D</span><span>F</span><span>S</span><span>S</span>
    </div>
  `;
  const days = document.createElement("div");
  days.className = "planner-mini-calendar-days";

  for (let row = 0; row < 6; row += 1) {
    const weekStart = new Date(gridStart.getTime());
    weekStart.setDate(weekStart.getDate() + (row * 7));
    const weekIso = isoDateFromDate(weekStart);
    const weekNumber = getIsoWeekNumber(weekIso);
    const weekCell = document.createElement("span");
    weekCell.className = "planner-mini-weekno";
    weekCell.textContent = String(weekNumber);
    days.append(weekCell);
    ...
    for (let col = 0; col < 7; col += 1) {
      ...
      const button = document.createElement("button");
      button.type = "button";
      button.className = "planner-mini-day";
      if (day.getMonth() !== monthStart.getMonth()) button.classList.add("outside");
      if (selectedSet.has(iso)) button.classList.add("selected");
      if (day.getDay() === 0 || day.getDay() === 6) button.classList.add("weekend");
      button.textContent = String(day.getDate());
      button.addEventListener("click", () => {
        plannerAnchorDate = iso;
        plannerMiniMonthAnchor = isoDateFromDate(new Date(day.getFullYear(), day.getMonth(), 1, 12, 0, 0, 0));
        renderPlanner();
      });
      days.append(button);
    }
  ...
  wrap.append(days);
  els.plannerMiniCalendar.innerHTML = "";
  els.plannerMiniCalendar.append(wrap);
  document.querySelector("#planner-mini-prev-month-btn")?.addEventListener("click", () => {
    const base = parseIsoDateValue(plannerMiniMonthAnchor) || monthStart;
    plannerMiniMonthAnchor = isoDateFromDate(new Date(base.getFullYear(), base.getMonth() - 1, 1, 12, 0, 0, 0));
    renderPlanner();
  });
  document.querySelector("#planner-mini-next-month-btn")?.addEventListener("click", () => {
    const base = parseIsoDateValue(plannerMiniMonthAnchor) || monthStart;
    plannerMiniMonthAnchor = isoDateFromDate(new Date(base.getFullYear(), base.getMonth() + 1, 1, 12, 0, 0, 0));
    renderPlanner();
  });
...
function renderPlanner() {
  const plannerShell = els.plannerView?.querySelector(".planner-shell");
  if (!plannerShell) return;

  const anchor = parseIsoDateValue(plannerAnchorDate) || new Date();
  plannerAnchorDate = isoDateFromDate(anchor);
  const monthAnchorDate = parseIsoDateValue(plannerMiniMonthAnchor) || anchor;
  plannerMiniMonthAnchor = isoDateFromDate(new Date(monthAnchorDate.getFullYear(), monthAnchorDate.getMonth(), 1, 12, 0, 0, 0));

  const monthStart = parseIsoDateValue(plannerMiniMonthAnchor) || new Date();
  const monthLabel = monthStart.toLocaleDateString("en-GB", { month: "long", year: "numeric" });
  const gridStart = new Date(monthStart.getTime());
  gridStart.setDate(gridStart.getDate() - ((gridStart.getDay() + 6) % 7)); // Monday start
  const selectedIso = plannerAnchorDate;

  let dayGrid = "";
  for (let row = 0; row < 6; row += 1) {
    const rowStart = new Date(gridStart.getTime());
    rowStart.setDate(rowStart.getDate() + (row * 7));
    const weekNo = getIsoWeekNumber(isoDateFromDate(rowStart));
    dayGrid += `<span class="planner-mini-weekno">${escapeHtml(String(weekNo))}</span>`;
    for (let col = 0; col < 7; col += 1) {
      const day = new Date(rowStart.getTime());
      day.setDate(day.getDate() + col);
      const iso = isoDateFromDate(day);
      const classes = [
        "planner-mini-day",
      ...
      dayGrid += `<button type="button" class="${classes}" data-iso="${iso}">${day.getDate()}</button>`;
    }
  }

  plannerShell.innerHTML = `
    <div class="planner-minimal-header">
      <div class="planner-minimal-title">Planner</div>
      <div class="planner-mini-calendar-shell compact">
        <div class="planner-mini-calendar-top">
          <div class="planner-mini-calendar-title">${escapeHtml(monthLabel)}</div>
          <div class="planner-mini-calendar-nav">
            <button id="planner-mini-prev-month-btn" class="icon-btn" type="button" aria-label="Previous month">&#8249;</button>
        ...
        <div class="planner-mini-calendar-weekdays">
          <span class="weekno"></span><span>M</span><span>D</span><span>M</span><span>D</span><span>F</span><span>S</span><span>S</span>
        </div>
        <div class="planner-mini-calendar-days">${dayGrid}</div>
      </div>
    </div>
  `;

  plannerShell.querySelector("#planner-mini-prev-month-btn")?.addEventListener("click", () => {
    const base = parseIsoDateValue(plannerMiniMonthAnchor) || new Date();
    plannerMiniMonthAnchor = isoDateFromDate(new Date(base.getFullYear(), base.getMonth() - 1, 1, 12, 0, 0, 0));
    renderPlanner();
  });
  plannerShell.querySelector("#planner-mini-next-month-btn")?.addEventListener("click", () => {
    const base = parseIsoDateValue(plannerMiniMonthAnchor) || new Date();
    plannerMiniMonthAnchor = isoDateFromDate(new Date(base.getFullYear(), base.getMonth() + 1, 1, 12, 0, 0, 0));
    renderPlanner();
  });
  plannerShell.querySelectorAll(".planner-mini-day[data-iso]").forEach((btn) => {
    btn.addEventListener("click", () => {
      plannerAnchorDate = btn.dataset.iso || plannerAnchorDate;
      renderPlanner();
    });
  });
}
...
function renderWorkspaceTabs(project) {
  els.folderTabList.innerHTML = "";
  const areas = (project?.areas || []).filter((area) => showArchivedWorkspaceItems || !area.archivedAt);
  ...
  if (els.workspaceShowArchivedAreasBtn) {
    const showToggle = currentView === "projects" && currentWorkspaceTab === "folders-hub";
    const archivedCount = (project?.areas || []).filter((area) => area.archivedAt).length;
...
function createPlansViewToggleControl(project, key = "details:plan") {
  ...
  tilesButton.textContent = "Kacheln";
  tilesButton.title = "Kacheln";
...
function renderAreaBrowserDialog() {
  ...
  if (els.areaBrowserTitle) els.areaBrowserTitle.textContent = area.name || "Area";
...
function buildProjectDetailsAreasSection(project, canManage, canWork, areaFilter = null) {
  ...
  const archivedAreas = (project.areas || []).filter((area) => area.archivedAt && areaMatchesSelectedTeams(area, project));
...
function loadState() {
  try {
    ...
    if (rawV3) return { ...structuredClone(initialState), ...JSON.parse(rawV3) };
