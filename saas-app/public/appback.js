...
function refreshDraftProjectLockState() {
  ...
  syncProjectContactOverrideFields(project);
...
const els = {
  ...
  aiAssistantView: document.querySelector("#ai-assistant-view"),
  aiListeningCue: document.querySelector("#ai-listening-cue"),
  ...
  projectAiBtn: document.querySelector("#project-ai-btn"),
  workspaceAiVoiceBtn: document.querySelector("#workspace-ai-voice-btn"),
  aiAssistantDialog: document.querySelector("#ai-assistant-dialog"),
  aiAssistantForm: document.querySelector("#ai-assistant-form"),
  aiThinkingOverlay: document.querySelector("#ai-thinking-overlay"),
  aiAssistantDialogTitle: document.querySelector("#ai-assistant-dialog-title"),
  aiDraftSection: document.querySelector("#ai-draft-section"),
  aiSettingsSection: document.querySelector("#ai-settings-section"),
  aiAssistantMode: document.querySelector("#ai-assistant-mode"),
  aiModeChoices: Array.from(document.querySelectorAll("[data-ai-mode-choice]")),
  aiContextProject: document.querySelector("#ai-context-project"),
  aiContextArea: document.querySelector("#ai-context-area"),
  aiStartDialogBtn: document.querySelector("#ai-start-dialog-btn"),
  aiVoiceTalkBtn: document.querySelector("#ai-voice-talk-btn"),
  aiAnswerNowBtn: document.querySelector("#ai-answer-now-btn"),
  aiKeepListeningBtn: document.querySelector("#ai-keep-listening-btn"),
  aiVoiceStatus: document.querySelector("#ai-voice-status"),
  aiChatLog: document.querySelector("#ai-chat-log"),
  aiChatInput: document.querySelector("#ai-chat-input"),
  aiSendMessageBtn: document.querySelector("#ai-send-message-btn"),
  aiReadPlayBtn: document.querySelector("#ai-read-play-btn"),
  aiReadPauseBtn: document.querySelector("#ai-read-pause-btn"),
  aiReadStopBtn: document.querySelector("#ai-read-stop-btn"),
  aiSpokenText: document.querySelector("#ai-spoken-text"),
  aiCreateDraftBtn: document.querySelector("#ai-create-draft-btn"),
  aiReadDraftBtn: document.querySelector("#ai-read-draft-btn"),
  aiReadNoteBottomBtn: document.querySelector("#ai-read-note-bottom-btn"),
  aiDownloadWordBtn: document.querySelector("#ai-download-word-btn"),
  aiFeedbackPanel: document.querySelector("#ai-feedback-panel"),
  aiDraftText: document.querySelector("#ai-draft-text"),
  aiSaveTarget: document.querySelector("#ai-save-target"),
  aiLanguage: document.querySelector("#ai-language"),
  aiDefaultTarget: document.querySelector("#ai-default-target"),
  aiQuickLanguage: document.querySelector("#ai-quick-language"),
  aiAnswerDelaySeconds: document.querySelector("#ai-answer-delay-seconds"),
  aiSummaryLanguage: document.querySelector("#ai-summary-language"),
  aiSummaryPause: document.querySelector("#ai-summary-pause"),
  aiSummaryMagic: document.querySelector("#ai-summary-magic"),
  aiNewUnassignedChatBtn: document.querySelector("#ai-new-unassigned-chat-btn"),
  aiConversationDraftList: document.querySelector("#ai-conversation-draft-list"),
  aiAssistantNewDraftBtn: document.querySelector("#ai-assistant-new-draft-btn"),
  aiAssistantSettingsBtn: document.querySelector("#ai-assistant-settings-btn"),
  closeAiAssistantBtn: document.querySelector("#close-ai-assistant-btn"),
  minimizeAiAssistantBtn: document.querySelector("#minimize-ai-assistant-btn"),
  aiMinimizedBar: document.querySelector("#ai-minimized-bar"),
  cancelAiAssistantBtn: document.querySelector("#cancel-ai-assistant-btn"),
  aiStartNewSectionBtn: document.querySelector("#ai-start-new-section-btn"),
  aiLearnTrigger: document.querySelector("#ai-learn-trigger"),
  aiLearnMeaning: document.querySelector("#ai-learn-meaning"),
  aiLearnAddBtn: document.querySelector("#ai-learn-add-btn"),
  aiLearningList: document.querySelector("#ai-learning-list"),
  saveAiSettingsBtn: document.querySelector("#save-ai-settings-btn"),
  saveAiDraftBtn: document.querySelector("#save-ai-draft-btn"),
  ...
  projectAddressDifferent: document.querySelector("#project-address-different"),
  projectAddress: document.querySelector("#project-address"),
  projectPhoneDifferent: document.querySelector("#project-phone-different"),
  projectPhone: document.querySelector("#project-phone"),
  ...
  aiFloatingBtn: document.querySelector("#ai-floating-btn"),
  ...
  areaDialogTitle: document.querySelector("#area-dialog-title"),
  ...
  areaBulkRows: document.querySelector("#area-bulk-rows"),
  areaBulkAddRowBtn: document.querySelector("#area-bulk-add-row-btn"),
  areaBulkCreateBtn: document.querySelector("#area-bulk-create-btn"),
...
function normalizeState(rawState) {
  ...
  nextState.dailyWorkContacts = Array.isArray(nextState.dailyWorkContacts)
    ...
    : [];
  nextState.aiConversationDrafts = Array.isArray(nextState.aiConversationDrafts)
...
function normalizeProject(project, rootState) {
  const normalized = {
    ...
    addressDiffersFromClient: Boolean(project.addressDiffersFromClient),
    phoneDiffersFromClient: Boolean(project.phoneDiffersFromClient),
...
mountPopupShells();
const AI_ASSISTANT_SETTINGS_KEY = "project-manager-ai-assistant-settings-v1";
const AI_ASSISTANT_DRAFT_KEY = "project-manager-ai-assistant-open-draft-v1";
const AI_ASSISTANT_MEMORY_KEY = "project-manager-ai-assistant-memory-v1";
const AI_PROJECT_ASSISTANT_MEMORY_KEY = "project-manager-ai-project-memory-v1";
try {
  localStorage.removeItem(AI_ASSISTANT_MEMORY_KEY);
  localStorage.removeItem(AI_PROJECT_ASSISTANT_MEMORY_KEY);
} catch (error) {
  // Local storage can be unavailable in private or restricted browser modes.
}
let aiRecognition = null;
let aiListening = false;
let aiSessionTranscriptBase = "";
let aiManualStopRequested = false;
let aiDialogActive = false;
let aiDialogAskedQuestions = false;
let aiOpenQuestions = [];
let aiQuestionAnswers = {};
let aiQuestionAskCounts = {};
let aiQuestionSession = null;
let aiLastQuestionAsked = "";
let aiQuestionRoundComplete = false;
let aiQuestionListenStartedAt = 0;
let aiLastGeneratedDraft = "";
let aiSilenceCountdownTimer = null;
let aiSilenceCountdownRemaining = 0;
let aiLiveOrganizing = false;
let aiLastOrganizedTranscript = "";
let aiSpeechPausedRecognition = false;
let aiSkipDraftSaveOnClose = false;
let aiChatHistory = [];
let aiActiveProjectId = "";
let aiActiveProjectName = "";
let activeAiConversationDraftId = "";
let aiLastChatSendKey = "";
let aiLastChatSendAt = 0;
let aiSpeechState = "idle";
let aiSpeechCurrentText = "";
let aiSpeechManualStopRequested = false;
let aiSpeechSequence = 0;

function getAiAssistantSettings() {
  const defaults = {
    language: "English",
    defaultTarget: "project-notes",
    answerDelaySeconds: 2,
  };
  try {
    return { ...defaults, ...(JSON.parse(localStorage.getItem(AI_ASSISTANT_SETTINGS_KEY) || "{}") || {}) };
  } catch (error) {
    return defaults;
  }
}

function saveAiAssistantSettings(settings) {
...
function normalizeAiChatMessage(message = {}) {
  return {
    ...
    createdAt: message.createdAt || new Date().toISOString(),
    feedback: ["like", "dislike"].includes(message.feedback) ? message.feedback : "",
    meta: message.meta || null,
  };
}

function normalizeAiConversationDraft(draft = {}) {
  const chatHistory = Array.isArray(draft.chatHistory)
    ? draft.chatHistory.map(normalizeAiChatMessage).filter((message) => message.content.trim())
    : [];
  return {
...
function getAiConversationTitle(chatHistory = aiChatHistory) {
  const firstUserMessage = (chatHistory || []).find((message) => message.role === "user" && String(message.content || "").trim());
  const text = cleanAiTranscript(firstUserMessage?.content || "");
  if (!text) return "AI conversation draft";
  return text.length > 54 ? `${text.slice(0, 54).trim()}...` : text;
}

function isAiNoProjectContext() {
  return Boolean(els.aiContextProject) && !els.aiContextProject.value;
}

function getAiAssistantDraftState() {
  try {
    return JSON.parse(localStorage.getItem(AI_ASSISTANT_DRAFT_KEY) || "null") || null;
  } catch (error) {
    return null;
  }
}

function hasAiAssistantDraftContent() {
  const draft = getAiAssistantDraftState();
  return Boolean(
    String(draft?.spokenText || "").trim()
    || String(draft?.draftText || "").trim()
    || (Array.isArray(draft?.chatHistory) && draft.chatHistory.length > 1)
  );
}

function saveAiAssistantDraftState() {
  const payload = {
    mode: els.aiAssistantMode?.value || "summary",
    projectId: els.aiContextProject ? (els.aiContextProject.value || "") : (state.selectedProjectId || ""),
    conversationDraftId: activeAiConversationDraftId || "",
    areaId: els.aiContextArea?.value || "project",
    saveTarget: els.aiSaveTarget?.value || "project-notes",
    spokenText: els.aiSpokenText?.value || "",
    draftText: els.aiDraftText?.value || "",
    generatedDraft: aiLastGeneratedDraft || "",
    chatHistory: aiChatHistory.map((message) => ({
      role: message.role === "user" ? "user" : "assistant",
      content: String(message.content || ""),
    ...
    statusText: els.aiVoiceStatus?.textContent || "",
    updatedAt: new Date().toISOString(),
  };
  localStorage.setItem(AI_ASSISTANT_DRAFT_KEY, JSON.stringify(payload));
}

function clearAiAssistantDraftState() {
  localStorage.removeItem(AI_ASSISTANT_DRAFT_KEY);
}

function getAiAssistantMemory() {
  try {
    const memory = JSON.parse(localStorage.getItem(AI_ASSISTANT_MEMORY_KEY) || "[]");
...
function saveAiAssistantMemory(memory) {
  localStorage.setItem(AI_ASSISTANT_MEMORY_KEY, JSON.stringify(memory.slice(0, 50)));
}

function summarizeAiCorrectionPattern(aiDraft, finalDraft) {
  const aiText = cleanAiTranscript(aiDraft);
  const finalText = cleanAiTranscript(finalDraft);
  if (!aiText || !finalText) return "User saved the AI note without enough comparison data.";
  if (aiText === finalText) return "User accepted the AI draft without manual correction.";
  const aiLines = new Set(aiDraft.split("\n").map((line) => cleanAiTranscript(line)).filter(Boolean));
  const changedLines = finalDraft.split("\n").map((line) => cleanAiTranscript(line)).filter((line) => line && !aiLines.has(line));
  if (changedLines.length) {
    return `User corrected or added: ${changedLines.slice(0, 4).join(" | ")}`;
  }
  return "User edited wording, structure, or details before saving.";
}

function rememberAiDraftCorrection({ mode, transcript, aiDraft, finalDraft, context, settings }) {
  // Learning memory is disabled for the simple typing assistant.
}

function getAiMemoryForRequest({ mode, settings }) {
  return [];
}

function getProjectAssistantMemory() {
  try {
    const memory = JSON.parse(localStorage.getItem(AI_PROJECT_ASSISTANT_MEMORY_KEY) || "[]");
    return Array.isArray(memory) ? memory : [];
  } catch (error) {
    return [];
  }
...
function saveProjectAssistantMemory(memory) {
  localStorage.setItem(AI_PROJECT_ASSISTANT_MEMORY_KEY, JSON.stringify(memory.slice(0, 120)));
}

function addProjectAssistantMemoryRule({ trigger, meaning, scope = "global", project = getAiContextProject() }) {
  const cleanTrigger = cleanAiTranscript(trigger);
  const cleanMeaning = cleanAiTranscript(meaning);
  if (!cleanTrigger || !cleanMeaning) return null;
  const rule = {
    id: crypto.randomUUID(),
    type: "user_rule",
    scope,
    ...
    ruleText: `When the user says or writes "${cleanTrigger}", interpret it as: ${cleanMeaning}`,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
  };
  const memory = getProjectAssistantMemory();
  const key = `${rule.scope}|${rule.projectId || ""}|${normalizeMemoryText(rule.trigger)}`;
  const existingIndex = memory.findIndex((entry) => `${entry.scope}|${entry.projectId || ""}|${normalizeMemoryText(entry.trigger)}` === key);
  if (existingIndex >= 0) {
    memory[existingIndex] = { ...memory[existingIndex], ...rule, id: memory[existingIndex].id, createdAt: memory[existingIndex].createdAt || rule.createdAt };
  } else {
    memory.unshift(rule);
  }
  saveProjectAssistantMemory(memory);
  return rule;
}

function normalizeMemoryText(value = "") {
  return cleanAiTranscript(value).toLowerCase();
}

function extractProjectAssistantRule(message = "", project = getAiContextProject()) {
  const text = cleanAiTranscript(message);
  if (!text) return null;
  const patterns = [
    /(?:when|whenever)\s+(?:i|we|the user)?\s*(?:write|writes|name|names|use|uses)\s+["“]?([^"”]+?)["”]?\s+(?:it\s+)?(?:means|is)\s+(.+)/i,
    /["“]?([^"”]+?)["”]?\s+(?:means|mean|stands for)\s+(.+)/i,
    /(?:otan)\s+(?:grafo|grafei|grafi)\s+["“]?([^"”]+?)["”]?\s+(?:auto\s+)?(?:simainei|simeni)\s+(.+)/i,
    /["“]?([^"”]+?)["”]?\s+(?:simainei|simeni)\s+(.+)/i,
    /(?:οταν|όταν)\s+(?:γραφω|γράφω|γραφει|γράφει)\s+["“]?([^"”]+?)["”]?\s+(?:αυτο\s+)?(?:σημαινει|σημαίνει)\s+(.+)/i,
    /["“]?([^"”]+?)["”]?\s+(?:σημαινει|σημαίνει)\s+(.+)/i,
  ];
  for (const pattern of patterns) {
    const match = text.match(pattern);
    if (!match) continue;
    const trigger = cleanAiTranscript(match[1]);
    const meaning = cleanAiTranscript(match[2]);
    if (!trigger || !meaning || trigger.length > 90 || meaning.length > 320) continue;
    const normalizedMeaning = normalizeMemoryText(meaning);
    if (normalizedMeaning.includes("i do not find") || normalizedMeaning.includes("i cannot find") || normalizedMeaning.includes("\u03b4\u03b5\u03bd \u03b2\u03c1\u03b9\u03c3\u03ba")) continue;
    return {
      id: crypto.randomUUID(),
      type: "user_rule",
      scope: "project",
      ...
      trigger,
      meaning,
      ruleText: `When the user says or writes "${trigger}", interpret it as: ${meaning}`,
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
  }
  if (/\bname\s*-\s*floor\b/i.test(text) || /\barea.+floor\b/i.test(text) || /όνομα.+όροφο/i.test(text)) {
    return {
      id: crypto.randomUUID(),
      type: "naming_rule",
      scope: "global",
      ...
      projectName: "",
      trigger: "area naming",
      meaning: "Area names may follow NAME - FLOOR. Text before '-' is the area name. Number after '-' is the floor.",
      ruleText: "Area naming rule: NAME - FLOOR. Text before '-' is the area name. Number after '-' is the floor.",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
  }
  const normalized = normalizeMemoryText(text);
  const compact = normalizeAreaLookupText(text).replace(/\s+/g, "");
  const simpleMeaningMatch = text.match(/["']?(.{1,80}?)["']?\s+(?:means|mean|is|einai|\u03b5\u03b9\u03bd\u03b1\u03b9|\u03c3\u03b7\u03bc\u03b1\u03b9\u03bd\u03b5\u03b9|\u03c3\u03b7\u03bc\u03b1\u03af\u03bd\u03b5\u03b9)\s+(.{2,320})/i);
  if (simpleMeaningMatch) {
    const trigger = cleanAiTranscript(simpleMeaningMatch[1]);
    const meaning = cleanAiTranscript(simpleMeaningMatch[2]);
    const badMeaning = normalizeMemoryText(meaning);
    if (trigger && meaning && !badMeaning.includes("i do not find") && !badMeaning.includes("i cannot find") && !badMeaning.includes("\u03b4\u03b5\u03bd \u03b2\u03c1\u03b9\u03c3\u03ba")) {
      return {
        id: crypto.randomUUID(),
        type: "user_rule",
        scope: "project",
  ...
  if (/name\s*-\s*(number|floor)|area\s*-\s*floor|-\s*\d+/.test(normalized) && /(floor|orofos|\u03bf\u03c1\u03bf\u03c6)/.test(compact)) {
    return {
      id: crypto.randomUUID(),
      type: "naming_rule",
      scope: "global",
      ...
      projectName: "",
      trigger: "area naming",
      meaning: "In area names, the number after '-' is the floor.",
      ruleText: "Area naming rule: NAME - FLOOR. Text before '-' is the area name. Number after '-' is the floor.",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    };
...
function rememberProjectAssistantRuleFromMessage(message, project = getAiContextProject()) {
  const rule = extractProjectAssistantRule(message, project);
  if (!rule) return null;
  const memory = getProjectAssistantMemory();
  const key = `${rule.scope}|${rule.projectId || ""}|${normalizeMemoryText(rule.trigger)}|${normalizeMemoryText(rule.meaning)}`;
  const existingIndex = memory.findIndex((entry) => `${entry.scope}|${entry.projectId || ""}|${normalizeMemoryText(entry.trigger)}|${normalizeMemoryText(entry.meaning)}` === key);
  if (existingIndex >= 0) {
    memory[existingIndex] = { ...memory[existingIndex], ...rule, id: memory[existingIndex].id, updatedAt: new Date().toISOString() };
  } else {
    memory.unshift(rule);
  }
  saveProjectAssistantMemory(memory);
...
function getProjectAssistantMemoryForRequest(project, question = "") {
  const normalizedQuestion = normalizeMemoryText(question);
  return getProjectAssistantMemory()
    .filter((entry) => entry.scope === "global" || !entry.projectId || entry.projectId === project?.id)
    .filter((entry) => {
      const text = normalizeMemoryText(`${entry.trigger || ""} ${entry.meaning || ""} ${entry.ruleText || ""}`);
      return !text.includes("i do not find") && !text.includes("i cannot find") && !text.includes("\u03b4\u03b5\u03bd \u03b2\u03c1\u03b9\u03c3\u03ba");
    })
    .map((entry) => {
      const trigger = normalizeMemoryText(entry.trigger);
      const score = trigger && normalizedQuestion.includes(trigger) ? 10 : 1;
      return { entry, score };
    })
    .sort((left, right) => right.score - left.score || String(right.entry.updatedAt || "").localeCompare(String(left.entry.updatedAt || "")))
    .slice(0, 12)
    .map(({ entry }) => ({
      type: entry.type || "user_rule",
...
function renderAiLearningList() {
  if (!els.aiLearningList) return;
  const memory = getProjectAssistantMemory();
  if (!memory.length) {
    els.aiLearningList.innerHTML = `<p class="muted">No learned rules yet. Add words, naming rules, and corrections here.</p>`;
    return;
  }
  els.aiLearningList.innerHTML = memory.slice(0, 40).map((entry) => `
    <article class="ai-learning-item">
      <div>
        <strong>${escapeHtml(entry.trigger || "Rule")}</strong>
        <p>${escapeHtml(entry.meaning || entry.ruleText || "")}</p>
        <span class="muted">${escapeHtml(entry.scope || "global")}${entry.projectName ? ` - ${escapeHtml(entry.projectName)}` : ""}</span>
      </div>
      <button class="ghost-btn" type="button" data-ai-memory-delete="${escapeHtml(entry.id || "")}">Delete</button>
    </article>
  `).join("");
}

function onAiLearnAdd() {
  const rule = addProjectAssistantMemoryRule({
    trigger: els.aiLearnTrigger?.value || "",
    meaning: els.aiLearnMeaning?.value || "",
    scope: "global",
  });
  if (!rule) {
    showAppMessage("Add what the word/rule is and what it means.", "warning", "AI Learn");
    return;
  }
  if (els.aiLearnTrigger) els.aiLearnTrigger.value = "";
  if (els.aiLearnMeaning) els.aiLearnMeaning.value = "";
  renderAiLearningList();
  showAppMessage("AI learning saved.", "success", "AI Learn");
}

function onAiLearningListClick(event) {
  const button = event.target.closest?.("[data-ai-memory-delete]");
  if (!button) return;
  const id = button.dataset.aiMemoryDelete || "";
  saveProjectAssistantMemory(getProjectAssistantMemory().filter((entry) => entry.id !== id));
  renderAiLearningList();
}

function restoreAiAssistantDraftState(expectedProjectId = null) {
  const draft = getAiAssistantDraftState();
  if (!draft) return false;
  const restoredProjectId = Object.prototype.hasOwnProperty.call(draft, "projectId") ? draft.projectId : state.selectedProjectId;
  if (expectedProjectId !== null && String(restoredProjectId || "") !== String(expectedProjectId || "")) {
    return false;
  }
  populateAiContext(restoredProjectId);
  if (els.aiContextProject) els.aiContextProject.value = restoredProjectId || "";
  populateAiContext(els.aiContextProject?.value || "");
  activeAiConversationDraftId = draft.conversationDraftId || "";
  if (els.aiContextArea && draft.areaId) els.aiContextArea.value = draft.areaId;
  if (els.aiSaveTarget && draft.saveTarget) els.aiSaveTarget.value = draft.saveTarget;
  if (els.aiSpokenText) els.aiSpokenText.value = draft.spokenText || "";
  if (els.aiDraftText) els.aiDraftText.value = draft.draftText || "";
  aiLastGeneratedDraft = draft.generatedDraft || draft.draftText || "";
  if (Array.isArray(draft.chatHistory) && draft.chatHistory.length) {
    aiChatHistory = draft.chatHistory
      .map((message) => ({
        role: message?.role === "user" ? "user" : "assistant",
        content: String(message?.content || ""),
        createdAt: message?.createdAt || new Date().toISOString(),
  ...
  setAiAssistantMode(draft.mode || "summary", { keepStatus: true, keepSpokenText: true, keepDraftText: true });
  if (els.aiVoiceStatus) {
    els.aiVoiceStatus.textContent = draft.statusText || "Restored your assistant draft. You can continue editing, download it, or save it.";
  }
  syncAiContextAvailability();
  renderAiChat();
  return Boolean(String(draft.spokenText || "").trim() || String(draft.draftText || "").trim() || aiChatHistory.length > 1);
}

function syncAiAssistantSummary() {
  const settings = getAiAssistantSettings();
  if (els.aiSummaryLanguage) els.aiSummaryLanguage.textContent = settings.language;
  if (els.aiSummaryPause) els.aiSummaryPause.textContent = `${getAiAnswerDelaySeconds(settings)} sec wait`;
  if (els.aiSummaryMagic) els.aiSummaryMagic.textContent = "Reads draft aloud";
}

function syncAiAssistantControls(settings = getAiAssistantSettings()) {
  if (els.aiLanguage) els.aiLanguage.value = settings.language;
  if (els.aiDefaultTarget) els.aiDefaultTarget.value = settings.defaultTarget;
  if (els.aiQuickLanguage) els.aiQuickLanguage.value = settings.language;
  if (els.aiAnswerDelaySeconds) els.aiAnswerDelaySeconds.value = String(getAiAnswerDelaySeconds(settings));
}

function saveAiQuickSettings() {
  const previous = getAiAssistantSettings();
  const language = els.aiQuickLanguage?.value || previous.language;
  const answerDelaySeconds = getAiAnswerDelaySeconds({
    ...previous,
    answerDelaySeconds: els.aiAnswerDelaySeconds?.value || previous.answerDelaySeconds,
  });
  const next = {
    ...previous,
    language,
  ...
  syncAiAssistantSummary();
  syncAiAssistantControls(next);
  setAiAssistantMode(els.aiAssistantMode?.value || "summary");
}

function getAiAnswerDelaySeconds(settings = getAiAssistantSettings()) {
  const value = Math.round(Number(settings.answerDelaySeconds));
  if (!Number.isFinite(value)) return 2;
  return Math.min(10, Math.max(1, value));
}

function isAiGreek(settings = getAiAssistantSettings()) {
  return settings.language === "Greek";
}

function getAiProjectOptions() {
  return (state.projects || []).filter((project) => project && !project.isDraft && !project.archivedAt);
}

function getAiProjectLabel(project) {
  if (!project) return "No project";
  const number = formatProjectNumberValue(project.projectNumber);
  const client = getClientById(project.clientId);
  const clientName = client ? formatClientName(client) : "";
  return [number, project.name || "Untitled project", clientName].filter(Boolean).join(" - ");
}

function getAiContextProject() {
  if (els.aiContextProject) {
    const selectedId = els.aiContextProject.value || "";
    if (!selectedId) return null;
    return getAiProjectOptions().find((project) => project.id === selectedId) || null;
  }
  const selectedId = state.selectedProjectId || "";
  return getAiProjectOptions().find((project) => project.id === selectedId) || getCurrentProject();
}

function getAiContextArea(project = getAiContextProject()) {
  const selectedAreaId = els.aiContextArea?.value || "";
  if (!project || !selectedAreaId || selectedAreaId === "project") return null;
  return getAreaById(selectedAreaId, project);
}

function populateAiContext(projectId = state.selectedProjectId) {
  const projects = getAiProjectOptions();
  if (els.aiContextProject) {
    els.aiContextProject.innerHTML = "";
    els.aiContextProject.append(new Option("No project - AI draft", ""));
    for (const project of projects) {
      els.aiContextProject.append(new Option(getAiProjectLabel(project), project.id));
    }
    const requestedProjectId = String(projectId || "");
    els.aiContextProject.value = projects.some((project) => project.id === requestedProjectId)
      ? requestedProjectId
      : "";
  }

  const project = getAiContextProject();
  if (els.aiContextArea) {
    const previousAreaId = selectedProjectAreaId || els.aiContextArea.value || "project";
    els.aiContextArea.innerHTML = "";
    els.aiContextArea.append(new Option(project ? "Whole project" : "No project", "project"));
    for (const area of (project?.areas || []).filter((entry) => !entry.archivedAt)) {
      els.aiContextArea.append(new Option(area.name || "Untitled area", area.id));
    }
    els.aiContextArea.disabled = !project;
    els.aiContextArea.value = (project?.areas || []).some((area) => area.id === previousAreaId && !area.archivedAt)
      ? previousAreaId
      : "project";
  }
}

function syncAiContextAvailability() {
  if (els.aiStartDialogBtn) els.aiStartDialogBtn.disabled = false;
  if (els.aiVoiceTalkBtn) els.aiVoiceTalkBtn.disabled = false;
  if (els.aiCreateDraftBtn) els.aiCreateDraftBtn.disabled = false;
  if (els.saveAiDraftBtn) els.saveAiDraftBtn.disabled = false;
}

function getAiModeLabel(mode, settings = getAiAssistantSettings()) {
  if (isAiGreek(settings)) {
    if (mode === "offer") return "Προσχέδιο προσφοράς από ομιλία";
    if (mode === "material") return "Λίστα υλικών από ομιλία";
    if (mode === "conversation") return "Περίληψη συζήτησης";
    return "Φωνητική σημείωση / περίληψη έργου";
  }
  if (mode === "offer") return "Offer explanation";
  if (mode === "material") return "Materials assistant";
  if (mode === "conversation") return "Meeting / conversation summary";
  return "Project assistant";
}

function getAiDraftTemplate(mode, settings = getAiAssistantSettings()) {
  if (isAiGreek(settings)) {
    if (mode === "offer") {
      return "Προσχέδιο - χρειάζεται έλεγχο\n\nΕτοιμάστε προσχέδιο προσφοράς από τις φωνητικές σημειώσεις του εργοταξίου. Συμπεριλάβετε εργασίες, υλικά, ανοιχτές ερωτήσεις και ό,τι πρέπει να ελεγχθεί πριν σταλεί.";
    }
    if (mode === "material") {
      return "- 8 μ. χαλκοσωλήνα, χρειάζεται έλεγχο\n- 12 στηρίγματα τοίχου\n- Σιλικόνη για αρμούς μπάνιου, χρειάζεται έλεγχο";
    }
    return "Προσχέδιο περίληψης έργου\n\nΟ τεχνικός ανέφερε πρόοδο στο επιλεγμένο έργο ή χώρο. Ελέγξτε ονόματα, ποσότητες και ημερομηνίες πριν την αποθήκευση.";
  }
  if (mode === "offer") {
    return "Draft\n\nPrepare an offer draft from the spoken site notes. Include labor, materials, and anything that must be checked before sending.";
  }
  if (mode === "material") {
    return "- 8 m copper pipe, needs check\n- 12 wall brackets\n- Silicone for bathroom joints, needs check";
  }
  if (mode === "conversation") {
    return "Conversation summary draft\n\nDecisions\n- Add the decisions here.\n\nTasks\n- Add the tasks here.\n\nNext steps\n- Add the next steps here.";
  }
  return "Project summary draft\n\nTechnician reported progress in the selected project or area. Review names, quantities, and dates before saving.";
}

function getAiContextLabel() {
  const project = getAiContextProject();
  const area = getAiContextArea(project);
  if (!project) return "No project selected";
  return area ? `${project.name || "Untitled project"} / ${area.name || "Untitled area"}` : (project.name || "Untitled project");
}

function getAiFriendlyPrompt(mode, settings = getAiAssistantSettings()) {
  if (!getAiContextProject()) {
    return "Tell me what you want to write. I will organize it as an AI draft that is not assigned to a project.";
  }
  const context = getAiContextLabel();
  if (settings.language === "German") return `Hallo, ich mache Notizen für ${context}. Drücke das Mikrofon, sprich frei, dann drücke Stop.`;
  if (settings.language === "Greek") return `Γεια, κρατάω σημειώσεις για ${context}. Πάτησε το μικρόφωνο, μίλησε ελεύθερα και μετά πάτησε Stop.`;
  if (settings.language === "Italian") return `Ciao, sto prendendo appunti per ${context}. Premi il microfono, parla liberamente, poi premi Stop.`;
  if (settings.language === "English") return `Hello, I am keeping notes for ${context}. Press the microphone, speak freely, then press Stop.`;
  if (settings.language === "German") return `Hallo, ich mache Notizen für ${context}. Drücke Stop, wenn du fertig bist.`;
  if (settings.language === "Greek") return `Γεια, κρατάω σημειώσεις για ${context}. Πάτησε Stop όταν είσαι έτοιμος.`;
  if (settings.language === "Italian") return `Ciao, sto prendendo appunti per ${context}. Premi Stop quando hai finito.`;
  if (settings.language === "English") return `Hello, I am keeping notes for ${context}. Press Stop when you are ready.`;
  if (settings.language === "German") {
    if (mode === "offer") return `Hallo, ich höre dich. Ich schreibe einen Angebotsentwurf für ${context}. Drücke das Mikrofon, erzähle mir die Arbeit und die Materialien, dann drücke Stop.`;
    if (mode === "material") return `Hallo, ich höre dich. Ich erstelle eine Materialliste für ${context}. Drücke das Mikrofon, nenne mir die Artikel und Mengen, dann drücke Stop.`;
    if (mode === "conversation") return `Hallo, ich höre dich. Ich fasse die Besprechung für ${context} zusammen. Drücke das Mikrofon, lass die Unterhaltung laufen, dann drücke Stop.`;
    return `Hallo, ich höre dich. Ich schreibe eine Projektnotiz für ${context}. Drücke das Mikrofon, erzähle mir was passiert ist, dann drücke Stop.`;
  }
  if (settings.language === "Greek") {
    if (mode === "offer") return `Γεια, σε ακούω. Θα γράψω ένα προσχέδιο προσφοράς για ${context}. Πάτησε το μικρόφωνο, πες μου τις εργασίες και τα υλικά, μετά πάτησε Stop.`;
    if (mode === "material") return `Γεια, σε ακούω. Θα φτιάξω λίστα υλικών για ${context}. Πάτησε το μικρόφωνο, πες μου τα υλικά και τις ποσότητες, μετά πάτησε Stop.`;
    if (mode === "conversation") return `Γεια, σε ακούω. Θα κάνω περίληψη της συζήτησης για ${context}. Πάτησε το μικρόφωνο, άφησε τη συζήτηση να ακουστεί, μετά πάτησε Stop.`;
    return `Γεια, σε ακούω. Θα γράψω μια σημείωση έργου για ${context}. Πάτησε το μικρόφωνο, πες μου τι έγινε, μετά πάτησε Stop.`;
  }
  if (settings.language === "Italian") {
    if (mode === "offer") return `Ciao, ti ascolto. Preparo una bozza di offerta per ${context}. Premi il microfono, dimmi i lavori e i materiali, poi premi Stop.`;
    if (mode === "material") return `Ciao, ti ascolto. Preparo una lista materiali per ${context}. Premi il microfono, dimmi gli articoli e le quantità, poi premi Stop.`;
    if (mode === "conversation") return `Ciao, ti ascolto. Preparo un riassunto della conversazione per ${context}. Premi il microfono, lascia parlare, poi premi Stop.`;
    return `Ciao, ti ascolto. Scrivo una nota di progetto per ${context}. Premi il microfono, raccontami cosa è successo, poi premi Stop.`;
  }
  if (mode === "offer") {
    return `Hi, I hear you. I will prepare an offer draft for ${context}. Press the microphone, tell me the work and materials, then press Stop.`;
  }
  if (mode === "material") {
    return `Hi, I hear you. I will make a material list for ${context}. Press the microphone, tell me the items and quantities, then press Stop.`;
  }
  if (mode === "conversation") {
    return `Hi, I hear you. I will summarize the conversation for ${context}. Press the microphone, let the discussion happen, then press Stop.`;
  }
  return `Hi, I hear you. I will write a project note for ${context}. Press the microphone, tell me what happened, then press Stop.`;
}

function cleanAiTranscript(text) {
  return String(text || "").replace(/\s+/g, " ").trim();
}

function splitAiThoughts(text) {
  return cleanAiTranscript(text)
    .split(/(?:\s*[.;]\s*|\s+\band\b\s+|\s+\bthen\b\s+)/i)
    .map((part) => part.trim())
    .filter(Boolean);
}

function polishAiSentence(text) {
...
function isLikelyMaterialLine(text) {
  return /\b(pipe|cable|paint|silicone|cement|screw|bracket|valve|socket|wire|tile|wood|panel|material|order|buy|need|needs|χρει|υλικ)\b/i.test(text);
}

function isLikelyWorkLine(text) {
  return /\b(fix|install|repair|remove|replace|check|build|paint|clean|connect|finish|work|done|went|visited|επισκευ|τοποθετ|εργασ)\b/i.test(text);
}

function getAiClarifyingQuestions(text) {
  return [];
  const cleanText = cleanAiTranscript(text);
  const questions = [];
  const suspiciousNumbers = cleanText.match(/\b\d{3,}\b/g) || [];
  for (const number of suspiciousNumbers) {
    if (!/^20\d{2}$/.test(number)) {
      questions.push(`Please confirm the number "${number}". It may be a quantity or a misheard date.`);
    }
  }
  const possibleDayNumbers = cleanText.match(/\b([3-9]\d)\b/g) || [];
  if (possibleDayNumbers.length && /\b(month|date|day|august|june|july|september|october|november|december)\b/i.test(cleanText)) {
    questions.push(`Please confirm the date. I heard "${possibleDayNumbers[0]}", which may not be a valid day of the month.`);
  }
  if (/\b(or|maybe|not sure|i think|something|somewhere|unclear|περίπου|ίσως)\b/i.test(cleanText)) {
    questions.push("There is unclear wording in the note. Please confirm the exact detail before saving.");
    if (/\bor\b/i.test(cleanText) && !/\b(maybe|not sure|i think|something|somewhere|unclear)\b/i.test(cleanText)) {
      questions.pop();
    }
  }
  if (/\b(month|date|tomorrow|today|next week|μήνα|ημερομην)\b/i.test(cleanText) && !/\b\d{1,2}[\/.-]\d{1,2}|\b\d{1,2}\b/.test(cleanText)) {
    questions.push("Please confirm the date or deadline.");
  }
  return Array.from(new Set(questions));
}

function isActionableAiQuestion(question) {
  const text = String(question || "").toLowerCase();
  return /\d/.test(text)
    || /\b(number|quantity|amount|date|deadline|month|day|august|june|july|september|october|november|december|datum|frist|termin|scadenza|data|numero|quantit|ημερομην|προθεσμ|μήνα|αριθμ)\b/i.test(text)
    || /\b(unclear|not clear|not sure|maybe|or\b|misheard|recognized badly|capita male|unklar|falsch verstanden|ασαφ|λάθος)\b/i.test(text);
}

function filterActionableAiQuestions(questions = []) {
  return [];
  return dedupeAiQuestions(questions).filter(isActionableAiQuestion).slice(0, 3);
}

function getAiSecretaryLabels(language = getAiAssistantSettings().language) {
  if (language === "German") {
    return {
      title: "Projektnotiz",
      ...
      cause: "Ursache",
      questions: "Fragen",
      review: "Hinweis der Assistenz",
      reviewText: "Du kannst weitere Details ergänzen, den Entwurf prüfen oder speichern.",
    };
  }
  if (language === "Greek") {
    return {
      ...
      happened: "Τι συνέβη",
      todo: "Τι πρέπει να γίνει",
      cause: "Αιτία",
      questions: "Ερωτήσεις",
      review: "Σημείωση βοηθού",
      reviewText: "Μπορείτε να προσθέσετε περισσότερες λεπτομέρειες, να ελέγξετε ή να αποθηκεύσετε.",
    };
  }
  if (language === "Italian") {
    return {
      title: "Nota di progetto",
      happened: "Cosa è successo",
      todo: "Cosa bisogna fare",
      cause: "Causa",
      questions: "Domande",
      review: "Nota dell'assistente",
      reviewText: "Puoi aggiungere altri dettagli, controllare la bozza o salvarla.",
    };
  }
  return {
    ...
    cause: "Cause",
    questions: "",
    review: "Assistant note",
    reviewText: "You can add more details, review the draft, or save it.",
  };
}

function localizeAiQuestion(question, language = getAiAssistantSettings().language) {
  if (language === "German") {
    if (question.includes("Please confirm the number")) return question.replace("Please confirm the number", "Bitte bestätige die Zahl").replace("It may be a quantity or a misheard date.", "Es könnte eine Menge oder ein falsch verstandenes Datum sein.");
    if (question.includes("unclear wording")) return "Es gibt eine unklare Formulierung. Bitte bestätige das genaue Detail vor dem Speichern.";
    if (question.includes("date or deadline")) return "Bitte bestätige das Datum oder die Frist.";
  }
  if (language === "Greek") {
    if (question.includes("Please confirm the number")) return question.replace("Please confirm the number", "Επιβεβαιώστε τον αριθμό").replace("It may be a quantity or a misheard date.", "Μπορεί να είναι ποσότητα ή λάθος αναγνώριση ημερομηνίας.");
    if (question.includes("unclear wording")) return "Υπάρχει ασαφής διατύπωση. Επιβεβαιώστε την ακριβή λεπτομέρεια πριν την αποθήκευση.";
    if (question.includes("date or deadline")) return "Επιβεβαιώστε την ημερομηνία ή την προθεσμία.";
  }
  if (language === "Italian") {
    if (question.includes("Please confirm the number")) return question.replace("Please confirm the number", "Conferma il numero").replace("It may be a quantity or a misheard date.", "Potrebbe essere una quantità o una data capita male.");
    if (question.includes("unclear wording")) return "C'è una formulazione poco chiara. Conferma il dettaglio esatto prima di salvare.";
    if (question.includes("date or deadline")) return "Conferma la data o la scadenza.";
  }
  return question;
}

function formatAiQuestions(questions, language = getAiAssistantSettings().language) {
  return "";
  if (!questions.length) return "";
  const labels = getAiSecretaryLabels(language);
  return ["", labels.questions, ...questions.map((question) => `- ${localizeAiQuestion(question, language)}`)].join("\n");
}

function getAiQuestionFlowLabels(language = getAiAssistantSettings().language) {
  if (language === "German") {
    return {
      open: "offene Fragen",
      ...
      listening: "Ich höre deine Antwort.",
      countdown: "Sekunden bis zur nächsten Frage",
      repeated: "Diese Frage kam schon mehrfach. Soll ich es so schreiben oder brauchst du mehr Details?",
      asIs: "So schreiben",
      more: "Mehr Details",
      complete: "Alle Fragen sind beantwortet. Ich aktualisiere den Entwurf.",
      savedWarning: "Es gibt offene Fragen. Möchtest du trotzdem speichern?",
    };
  }
  if (language === "Greek") {
    return {
      open: "ανοιχτές ερωτήσεις",
      answer: "Απάντηση ερωτήσεων",
      wait: "Περίμενε",
      next: "Επόμενη",
      end: "Τέλος",
      listening: "Ακούω την απάντησή σου.",
      countdown: "δευτερόλεπτα για την επόμενη ερώτηση",
      repeated: "Αυτή η ερώτηση εμφανίστηκε ξανά. Να το γράψω έτσι ή θέλεις περισσότερες λεπτομέρειες;",
      asIs: "Γράψ' το έτσι",
      more: "Περισσότερες λεπτομέρειες",
      complete: "Όλες οι ερωτήσεις απαντήθηκαν. Ενημερώνω το προσχέδιο.",
      savedWarning: "Υπάρχουν ανοιχτές ερωτήσεις. Θέλεις να αποθηκευτεί έτσι;",
    };
  }
  if (language === "Italian") {
    return {
      ...
      listening: "Sto ascoltando la tua risposta.",
      countdown: "secondi alla prossima domanda",
      repeated: "Questa domanda è già uscita più volte. Vuoi scriverla così o aggiungere altri dettagli?",
      asIs: "Scrivi così",
      more: "Più dettagli",
      complete: "Tutte le domande hanno una risposta. Aggiorno la bozza.",
      savedWarning: "Ci sono domande aperte. Vuoi salvare comunque?",
    };
  }
  return {
    open: "items",
    ...
    listening: "I am listening to your answer.",
    countdown: "seconds until next question",
    repeated: "This question has come up more than twice. Do you want me to write it like this, or do you want to add more details?",
    asIs: "Write it like this",
    more: "More details",
    complete: "All questions are answered. I am updating the draft.",
    savedWarning: "Save this draft as it is?",
  };
}

function getUnansweredAiQuestions() {
  return aiOpenQuestions.filter((question) => !String(aiQuestionAnswers[question] || "").trim());
}

function normalizeAiQuestionForDedupe(question) {
  return String(question || "")
    .toLowerCase()
    .replace(/[^\p{L}\p{N}\s]/gu, " ")
    .replace(/\b(please|confirm|check|the|a|an|is|are|it|this|that|bitte|conferma|επιβεβαιώστε)\b/giu, " ")
    .replace(/\s+/g, " ")
    .trim();
}

function getAiQuestionTopic(question) {
  const text = normalizeAiQuestionForDedupe(question);
  const raw = String(question || "").toLowerCase();
  if (/\b(date|deadline|month|day|june|july|august|september|october|november|december|datum|frist|termin|scadenza|data|ημερομην|προθεσμ|μήνα)\b/i.test(raw)) return "date";
  if (/\b(number|quantity|amount|cm|mm|meter|metre|m2|m²|zahl|menge|numero|quantit|αριθμ|ποσό|ποσότητα)\b/i.test(raw) || /\d/.test(raw)) return "number";
  if (/\b(pay|cost|price|invoice|insurance|client|customer|zahlen|kosten|rechnung|versicherung|pagare|costo|fattura|assicurazione|πληρώ|κόστος|τιμή|ασφάλεια|πελάτ)\b/i.test(raw)) return "money";
  if (/\b(who|person|responsible|installer|worker|plumber|wer|verantwortlich|installateur|chi|responsabile|idraulico|ποιος|υπεύθυν|υδραυλικ|τεχνικ)\b/i.test(raw)) return "person";
  if (/\b(cause|reason|because|why|ursache|grund|perch|causa|αιτία|γιατί)\b/i.test(raw)) return "cause";
  if (/\b(work|scope|replace|repair|todo|task|arbeiten|leistung|ripar|sostitu|lavor|εργασία|επισκευ|αντικατάστ)\b/i.test(raw)) return "scope";
  if (/\b(material|materials|item|order|supplier|materiali|articoli|υλικ)\b/i.test(raw)) return "material";
  if (/\b(location|area|room|bathroom|where|ort|bereich|raum|bagno|stanza|χώρος|μπάνιο|πού)\b/i.test(raw)) return "location";
...
function dedupeAiQuestions(questions = []) {
  ...
  const result = [];
  for (const question of questions.map(String).filter(Boolean)) {
    const key = normalizeAiQuestionForDedupe(question);
    const topic = getAiQuestionTopic(question);
    if (!key || seen.has(key) || seenTopics.has(topic)) continue;
    seen.add(key);
...
function getNextAiQuestion() {
  const unanswered = getUnansweredAiQuestions();
  if (!unanswered.length) return "";
  return unanswered.find((question) => question !== aiLastQuestionAsked) || unanswered[0];
}

function stopAiQuestionCountdown() {
  if (!aiQuestionSession) return;
  window.clearInterval(aiQuestionSession.countdownTimer);
  window.clearTimeout(aiQuestionSession.silenceTimer);
  aiQuestionSession.countdownTimer = null;
  aiQuestionSession.silenceTimer = null;
}

function resetAiQuestionSilenceCountdown() {
  if (!aiQuestionSession?.active) return;
  stopAiQuestionCountdown();
  aiQuestionSession.remaining = 8;
  renderAiFeedback(aiOpenQuestions, getAiAssistantSettings().language);
  aiQuestionSession.countdownTimer = window.setInterval(() => {
    if (!aiQuestionSession?.active) return;
    aiQuestionSession.remaining = Math.max(0, aiQuestionSession.remaining - 1);
    renderAiFeedback(aiOpenQuestions, getAiAssistantSettings().language);
  }, 1000);
}

function appendAiQuestionAnswersToTranscript() {
  const answeredLines = Object.entries(aiQuestionAnswers)
    .filter(([, answer]) => String(answer || "").trim())
    .map(([question, answer]) => `Question: ${question} Answer: ${answer}`);
  if (!answeredLines.length || !els.aiSpokenText) return;
  const current = cleanAiTranscript(els.aiSpokenText.value || "");
  els.aiSpokenText.value = [current, "Clarifications:", ...answeredLines].filter(Boolean).join("\n");
}

async function completeAiQuestionSession() {
  stopAiQuestionCountdown();
  if (aiRecognition) {
    try { aiRecognition.stop(); } catch (error) {}
  }
  aiListening = false;
  hideAiListeningCue();
  const labels = getAiQuestionFlowLabels();
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = labels.complete;
  appendAiQuestionAnswersToTranscript();
  aiOpenQuestions = [];
  aiQuestionSession = null;
  aiQuestionRoundComplete = true;
  renderAiFeedback([]);
  const mode = els.aiAssistantMode?.value || "summary";
  const settings = getAiAssistantSettings();
  const result = await createAiDraft({
    mode,
    transcript: els.aiSpokenText?.value || "",
    context: getAiDraftContext(),
    settings,
  });
  if (els.aiDraftText) els.aiDraftText.value = result.draft;
  renderAiFeedback([]);
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I updated the draft with your answers. Review it, read it aloud, or save it.";
}

function finishCurrentAiQuestionAnswer(reason = "next") {
  if (!aiQuestionSession?.active) return;
  const question = aiQuestionSession.currentQuestion;
  const answer = cleanAiTranscript(aiQuestionSession.currentAnswer || "");
  if (answer) {
    aiQuestionAnswers[question] = answer;
  } else if (reason === "end") {
    aiQuestionAnswers[question] = "No clear answer received. Keep the user's original wording.";
  } else {
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I did not hear an answer yet. You can answer, edit the note by hand, or press End on the last question.";
    return;
  }
  stopAiQuestionCountdown();
  if (aiRecognition) {
  ...
  aiListening = false;
  hideAiListeningCue();
  const nextQuestion = getNextAiQuestion();
  if (!nextQuestion) {
    completeAiQuestionSession();
    return;
  }
  aiQuestionSession.currentIndex = aiOpenQuestions.indexOf(nextQuestion);
  askCurrentAiQuestion({ prefixNext: true });
}

function askCurrentAiQuestion(options = {}) {
  if (!aiQuestionSession?.active) return;
  const question = aiOpenQuestions[aiQuestionSession.currentIndex] || getNextAiQuestion();
  if (!question) {
    completeAiQuestionSession();
    return;
  }
  aiQuestionSession.currentQuestion = question;
  aiLastQuestionAsked = question;
  aiQuestionSession.currentAnswer = "";
  aiQuestionSession.lastTranscript = "";
  aiQuestionAskCounts[question] = (aiQuestionAskCounts[question] || 0) + 1;
  renderAiFeedback(aiOpenQuestions, getAiAssistantSettings().language);
  const labels = getAiQuestionFlowLabels();
  const questionText = aiQuestionAskCounts[question] > 2 ? labels.repeated : question;
  const prompt = options.prefixNext ? `${labels.next}: ${questionText}` : questionText;
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = prompt;
  speakAiText(prompt, () => startAiQuestionAnswerListening());
}

function startAiQuestionAnswerListening() {
  const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  const labels = getAiQuestionFlowLabels();
  if (!Recognition) {
    showAppMessage("Speech recognition is not available in this browser. Type the answer into the spoken text preview, then press Next.", "warning", "AI Assistant");
    return;
  }
  aiRecognition = new Recognition();
  aiRecognition.lang = getAiRecognitionLanguage();
  aiRecognition.continuous = true;
  aiRecognition.interimResults = true;
  aiManualStopRequested = false;
  aiListening = true;
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = labels.listening;
  aiRecognition.onresult = (event) => {
    if (!aiListening || aiManualStopRequested || aiLiveOrganizing || !aiQuestionSession?.active) return;
    let transcript = "";
    for (let index = 0; index < event.results.length; index += 1) {
      transcript += event.results[index][0]?.transcript || "";
    }
    const answer = cleanAiTranscript(transcript);
    const normalizedAnswer = normalizeAiQuestionForDedupe(answer);
    const normalizedQuestion = normalizeAiQuestionForDedupe(aiQuestionSession?.currentQuestion || "");
    if (Date.now() - aiQuestionListenStartedAt < 900) return;
    if (normalizedAnswer && normalizedQuestion && (normalizedAnswer === normalizedQuestion || normalizedQuestion.includes(normalizedAnswer) || normalizedAnswer.includes(normalizedQuestion))) return;
    if (answer && answer !== aiQuestionSession?.lastTranscript) {
      aiQuestionSession.currentAnswer = answer;
      aiQuestionSession.lastTranscript = answer;
      resetAiQuestionSilenceCountdown();
    }
  };
  aiRecognition.onerror = () => {
    if (aiQuestionSession?.currentAnswer) resetAiQuestionSilenceCountdown();
  };
  aiRecognition.onend = () => {
    if (aiQuestionSession?.active && aiListening) {
      try {
        aiRecognition.start();
      } catch (error) {
        resetAiQuestionSilenceCountdown();
  ...
  aiRecognition.start();
  aiQuestionListenStartedAt = Date.now();
  aiQuestionSession.remaining = 8;
  renderAiFeedback(aiOpenQuestions, getAiAssistantSettings().language);
}

function startAiQuestionSession() {
  const unanswered = getUnansweredAiQuestions();
  if (!unanswered.length) return;
  if (aiListening && shouldDiscardDraft) {
    clearAiSilenceCountdown();
    aiManualStopRequested = true;
    aiListening = false;
    hideAiListeningCue();
    try {
      aiRecognition?.stop?.();
    } catch (error) {
      // Browser recognition can throw if it already stopped itself.
    }
  } else if (aiListening) {
    stopAiListening("Stopped listening.");
  }
  window.speechSynthesis?.cancel?.();
  aiLastQuestionAsked = "";
  aiQuestionRoundComplete = false;
  aiQuestionSession = {
    active: true,
    currentIndex: aiOpenQuestions.indexOf(unanswered[0]),
  ...
  askCurrentAiQuestion();
}

function waitForCurrentAiQuestion() {
  if (!aiQuestionSession?.active) return;
  resetAiQuestionSilenceCountdown();
}

function keepCurrentAiQuestionAsIs() {
  if (!aiQuestionSession?.active) return;
  aiQuestionAnswers[aiQuestionSession.currentQuestion] = "Save as currently written; no more details provided.";
  finishCurrentAiQuestionAnswer("next");
}

function onAiFeedbackClick(event) {
  const button = event.target?.closest?.("[data-ai-question-action]");
  const action = button?.dataset?.aiQuestionAction;
  if (!action) return;
  event.preventDefault();
  event.stopPropagation();
  if (action === "start") {
    startAiQuestionSession();
  } else if (action === "wait" || action === "more") {
    waitForCurrentAiQuestion();
  } else if (action === "next") {
    finishCurrentAiQuestionAnswer("next");
  } else if (action === "end") {
    finishCurrentAiQuestionAnswer("end");
  } else if (action === "as-is") {
    keepCurrentAiQuestionAsIs();
  }
}

function renderAiFeedback(questions = [], language = getAiAssistantSettings().language) {
  if (!els.aiFeedbackPanel) return;
  aiOpenQuestions = [];
  aiQuestionAnswers = {};
  aiQuestionAskCounts = {};
  els.aiFeedbackPanel.classList.add("hidden");
  els.aiFeedbackPanel.innerHTML = "";
  return;
  if (aiQuestionRoundComplete && !aiQuestionSession?.active) {
    aiOpenQuestions = [];
    els.aiFeedbackPanel.classList.add("hidden");
    els.aiFeedbackPanel.innerHTML = "";
    return;
  }
  aiOpenQuestions = filterActionableAiQuestions(questions);
  aiQuestionAnswers = Object.fromEntries(Object.entries(aiQuestionAnswers).filter(([question]) => aiOpenQuestions.includes(question)));
  aiQuestionAskCounts = Object.fromEntries(Object.entries(aiQuestionAskCounts).filter(([question]) => aiOpenQuestions.includes(question)));
  const unanswered = getUnansweredAiQuestions();
  els.aiFeedbackPanel.classList.toggle("hidden", !aiOpenQuestions.length);
  if (!aiOpenQuestions.length) {
    els.aiFeedbackPanel.innerHTML = "";
    return;
  }
  const labels = getAiSecretaryLabels(language);
  const flowLabels = getAiQuestionFlowLabels(language);
  const countLabel = `${unanswered.length} ${flowLabels.open}`;
  const current = aiQuestionSession?.active ? aiQuestionSession.currentQuestion : "";
  const isLastQuestion = aiQuestionSession?.active && unanswered.length <= 1;
  const nextAction = isLastQuestion ? "end" : "next";
  const nextLabel = isLastQuestion ? flowLabels.end : flowLabels.next;
  const countdown = "";
  const repeated = current && (aiQuestionAskCounts[current] || 0) > 2
    ? `<div class="ai-question-repeat"><span>${escapeHtml(flowLabels.repeated)}</span><button class="ghost-btn" data-ai-question-action="as-is" type="button">${escapeHtml(flowLabels.asIs)}</button><button class="ghost-btn" data-ai-question-action="more" type="button">${escapeHtml(flowLabels.more)}</button></div>`
    : "";
  els.aiFeedbackPanel.innerHTML = `
    <div class="ai-question-header">
      <strong>${escapeHtml(countLabel)}</strong>
      <button class="primary-btn ai-question-start-btn" data-ai-question-action="start" type="button">${escapeHtml(flowLabels.answer)}</button>
    </div>
    ${countdown}
    ${repeated}
    <ul>${aiOpenQuestions.map((question) => {
      const answered = String(aiQuestionAnswers[question] || "").trim();
      const active = question === current ? " class=\"active\"" : "";
      return `<li${active}>${escapeHtml(localizeAiQuestion(question, language))}${answered ? ` <span class="ai-question-answered">Answered</span>` : ""}</li>`;
    }).join("")}</ul>
    ${aiQuestionSession?.active ? `<div class="ai-question-actions"><button class="ghost-btn" data-ai-question-action="wait" type="button">${escapeHtml(flowLabels.wait)}</button><button class="primary-btn" data-ai-question-action="${nextAction}" type="button">${escapeHtml(nextLabel)}</button></div>` : ""}
  `;
}

function renderProjectAssistantMeta(result = {}) {
  if (!els.aiFeedbackPanel) return;
  els.aiFeedbackPanel.classList.add("hidden");
  els.aiFeedbackPanel.innerHTML = "";
}

function cleanAiReviewLabels(text = "") {
  return String(text || "")
    .replace(/\s*,?\s*\bneeds\s+(review|check)\b\.?/gi, "")
    .replace(/\s*,?\s*\bNeeds\s+(review|check)\b\.?/g, "")
    .replace(/\bDraft\s*-\s*needs\s+review\b/gi, "Draft")
    .replace(/\bNotes\s+for\s+review\b/gi, "Notes")
    .replace(/\bquantity\s+needs\s+check\b/gi, "quantity unclear")
    .replace(/^\s*-\s*$/gm, "")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}

function formatAiChatContent(content = "", highlightQuotes = false) {
  const cleanContent = cleanAiReviewLabels(content);
  if (!highlightQuotes) return escapeHtml(cleanContent).replace(/\n/g, "<br>");
  const parts = [];
  const pattern = /(["'])([^"'\n]{2,80})\1/g;
  let cursor = 0;
  let match = null;
  while ((match = pattern.exec(cleanContent)) !== null) {
    parts.push(escapeHtml(cleanContent.slice(cursor, match.index)));
    parts.push(`<span class="ai-unclear-quote">${escapeHtml(match[0])}</span>`);
    cursor = match.index + match[0].length;
  }
  parts.push(escapeHtml(cleanContent.slice(cursor)));
  return parts.join("").replace(/\n/g, "<br>");
}

function buildGreekSummaryDraft(transcript, contextLabel) {
  const text = cleanAiTranscript(transcript).toLowerCase();
  const linesWhatHappened = [];
  const linesToDo = [];
  const linesCause = [];
  const questions = [];

  if (/λάθος|βλακεία|πρόβλημα/.test(text)) {
    linesWhatHappened.push("Έγινε ένα λάθος κατά τη διάρκεια των εργασιών.");
  }
  if (/τρύπησε|τρυπάνι|σωλήνα|σωλήνας/.test(text)) {
    linesWhatHappened.push("Τρυπήθηκε κατά λάθος μια σωλήνα με το τρυπάνι.");
    linesCause.push("Διάτρηση σωλήνας κατά τη διάρκεια των εργασιών.");
  }
  const nameMatch = transcript.match(/\b(Παναγιώτης|Παναγιωτης|παναγιώτης|παναγιωτης)\b/);
  if (nameMatch) {
    linesCause[0] = "Διάτρηση σωλήνας από τον Παναγιώτη κατά τη διάρκεια των εργασιών.";
    linesWhatHappened.push("Ο Παναγιώτης τρύπησε κατά λάθος μια σωλήνα με το τρυπάνι.");
  }
  if (/πλημμύρ/.test(text)) {
    linesWhatHappened.push("Προκλήθηκε πλημμύρα στο μπάνιο.");
  }
  if (/ζημι|προβλήματα/.test(text)) {
    linesWhatHappened.push("Η πλημμύρα δημιούργησε επιπλέον ζημιές.");
  }
  if (/ξηλώσουμε|ξηλω/.test(text) && /υδραυλ/.test(text)) {
    linesToDo.push("Ξήλωμα όλων των υδραυλικών εγκαταστάσεων.");
  }
  if (/καινούριες αποχετεύσεις|νέες αποχετεύσεις|αποχετεύσεις/.test(text)) {
    linesToDo.push("Τοποθέτηση νέων αποχετεύσεων.");
  }
  if (/είδη υγιεινής|ειδη υγιεινης|υγιεινής/.test(text)) {
    linesToDo.push("Αντικατάσταση των ειδών υγιεινής.");
  }
  if (/πλημμύρ|ζημι|προβλήματα/.test(text)) {
    linesToDo.push("Έλεγχος και αποκατάσταση τυχόν πρόσθετων ζημιών από τη διαρροή.");
  }
  if (/ίδια ή τα ίδια|ή τα ίδια|τα ίδια ή/.test(text)) {
    questions.push("Επιβεβαιώστε αν εννοείτε \"τα είδη υγιεινής\".");
  }
  ...
  return [
    ...
    "",
    "Τι συνέβη",
    ...(linesWhatHappened.length ? Array.from(new Set(linesWhatHappened)).map((line) => `- ${line}`) : [`- ${polishAiSentence(transcript)}`]),
    "",
    "Τι πρέπει να γίνει",
    ...(linesToDo.length ? Array.from(new Set(linesToDo)).map((line) => `- ${line}`) : ["- Χρειάζεται έλεγχος και επιβεβαίωση των επόμενων εργασιών."]),
    "",
    "Αιτία",
    ...(linesCause.length ? Array.from(new Set(linesCause)).map((line) => `- ${line}`) : ["- Χρειάζεται επιβεβαίωση της αιτίας."]),
  ].filter(Boolean).join("\n");
}

function buildItalianSummaryDraft(transcript, contextLabel) {
  const text = cleanAiTranscript(transcript).toLowerCase();
  const linesWhatHappened = [];
  const linesToDo = [];
  const linesCause = [];
  ...
  if (/bagno/.test(text) && /tubi|tubo|idraulico|installatore/.test(text)) {
    linesWhatHappened.push("Sono stati eseguiti lavori sui tubi del bagno.");
  }
  if (/cambiare|cambiato|sostituit/.test(text) && /tubi/.test(text)) {
    linesWhatHappened[0] = "Sono stati sostituiti tutti i tubi del bagno da un idraulico.";
  }
  if (/installatore|idraulico/.test(text) && /bucat|foro|forato/.test(text)) {
    linesWhatHappened.push("Durante i lavori, l'idraulico ha forato accidentalmente un tubo.");
    linesCause.push("L'idraulico ha accidentalmente forato un tubo durante i lavori.");
  }
  const holeMatch = text.match(/\b(\d+)\s*(mm|millimetri)\b/);
  if (holeMatch) {
    linesWhatHappened.push(`Il foro è di circa ${holeMatch[1]} ${holeMatch[2]}.`);
  }
  if (/allag|acqua|casino/.test(text) && /bagno/.test(text)) {
    linesWhatHappened.push("Si è verificato un grave allagamento del bagno.");
  }
  const waterMatch = text.match(/\b(\d+)\s*cm\b/);
  if (waterMatch) {
    linesWhatHappened.push(`L'acqua ha raggiunto circa ${waterMatch[1]} cm di altezza.`);
  }
  if (/pompieri|vigili/.test(text)) {
    linesWhatHappened.push("È stato necessario chiamare i Vigili del Fuoco per pompare via l'acqua.");
  }
  if (/vasca/.test(text)) {
    linesToDo.push("Sostituire la vasca da bagno.");
  }
  if (/scarico|toilette|wc|water|cacare|tassa/.test(text)) {
    linesToDo.push("Rifare lo scarico della toilette.");
  }
  if (/sanitari|igienic/.test(text)) {
    linesToDo.push("Sostituire tutti i sanitari del bagno.");
  }
  if (/allag|acqua|danni|casino/.test(text)) {
    linesToDo.push("Riparare tutti i danni causati dall'allagamento.");
  }
  if (/foto di un installatore|tassa per il cacare/.test(text)) {
    questions.push("Confermare le parole riconosciute male: probabilmente non sono \"foto di un installatore\" e \"tassa per il cacare\".");
  }
  if (linesCause.length && /allag|acqua/.test(text)) {
    linesCause[0] = "L'idraulico ha accidentalmente forato un tubo durante i lavori, causando il grave allagamento del bagno.";
  }

  return [
    "Appunti - Problema nel bagno",
    "",
    `Progetto: ${contextLabel}`,
    "",
    "Cosa è successo",
    ...(linesWhatHappened.length ? Array.from(new Set(linesWhatHappened)).map((line) => `- ${line}`) : [`- ${polishAiSentence(transcript)}`]),
    "",
    "Cosa bisogna fare",
    ...(linesToDo.length ? Array.from(new Set(linesToDo)).map((line) => `- ${line}`) : ["- Verificare e confermare i lavori necessari."]),
    "",
    "Causa",
    ...(linesCause.length ? Array.from(new Set(linesCause)).map((line) => `- ${line}`) : ["- La causa deve essere confermata."]),
  ].filter(Boolean).join("\n");
}

function isBathroomFloodTranscript(transcript) {
  const text = cleanAiTranscript(transcript).toLowerCase();
  return /(bathroom|bagno|bad|badezimmer|μπάνιο)/i.test(text)
    && /(pipe|tubo|rohr|σωλήν|water|acqua|wasser|νερό|flood|allag|überschwemm|πλημμύρ|hole|foro|loch|τρύπ)/i.test(text);
}

function buildEnglishBathroomSummaryDraft(transcript, contextLabel) {
  const text = cleanAiTranscript(transcript).toLowerCase();
  const happened = [];
  const todo = [];
  const cause = [];

  if (/replace|changed|change|new/.test(text) && /pipe|pipes|plumbing/.test(text)) {
    happened.push("The bathroom pipes were replaced or worked on.");
  }
  if (/plumber|installer|worker/.test(text) && /drill|hole|puncture|broke|hit/.test(text) && /pipe/.test(text)) {
    happened.push("During the work, the installer accidentally drilled into a pipe.");
    cause.push("The installer accidentally drilled into a pipe during the work.");
  }
  const holeMatch = text.match(/\b(\d+)\s*(mm|millimeters|millimetres)\b/);
  if (holeMatch) happened.push(`The hole is approximately ${holeMatch[1]} ${holeMatch[2]}.`);
  if (/flood|water|leak/.test(text) && /bathroom/.test(text)) happened.push("A serious bathroom flood occurred.");
  const waterMatch = text.match(/\b(\d+)\s*cm\b/);
  if (waterMatch) happened.push(`The water reached approximately ${waterMatch[1]} cm.`);
  if (/fire brigade|firefighters|fire department/.test(text)) happened.push("The fire brigade had to be called to pump out the water.");
  if (/bathtub|bath tub|bath/.test(text)) todo.push("Replace the bathtub.");
  if (/toilet|wc|drain|sewer|waste/.test(text)) todo.push("Redo the toilet drain.");
  if (/sanitary|fixtures|toilet|sink|basin/.test(text)) todo.push("Replace the bathroom sanitary fixtures.");
  if (/flood|water|damage/.test(text)) todo.push("Repair all damage caused by the flooding.");
  if (cause.length && /flood|water/.test(text)) cause[0] = "The installer accidentally drilled into a pipe during the work, causing the serious bathroom flood.";
  ...
  return [
    ...
    "",
    "What happened",
    ...(happened.length ? Array.from(new Set(happened)).map((line) => `- ${line}`) : [`- ${polishAiSentence(transcript)}`]),
    "",
    "What needs to be done",
    ...(todo.length ? Array.from(new Set(todo)).map((line) => `- ${line}`) : ["- Check and confirm the required repair work."]),
    "",
    "Cause",
    ...(cause.length ? Array.from(new Set(cause)).map((line) => `- ${line}`) : ["- The cause must be confirmed."]),
  ].join("\n");
}

function buildGermanBathroomSummaryDraft(transcript, contextLabel) {
  const text = cleanAiTranscript(transcript).toLowerCase();
  const happened = [];
  const todo = [];
  const cause = [];

  if (/wechsel|gewechselt|erneuert|neu/.test(text) && /rohr|rohre|leitungen|wasserleitungen/.test(text)) {
    happened.push("Die Rohre im Badezimmer wurden erneuert oder bearbeitet.");
  }
  if (/installateur|monteur|handwerker/.test(text) && /bohr|loch|angebohrt|durchbohrt/.test(text) && /rohr/.test(text)) {
    happened.push("Während der Arbeiten hat der Installateur versehentlich ein Rohr angebohrt.");
    cause.push("Der Installateur hat während der Arbeiten versehentlich ein Rohr angebohrt.");
  }
  const holeMatch = text.match(/\b(\d+)\s*(mm|millimeter)\b/);
  if (holeMatch) happened.push(`Das Loch ist ungefähr ${holeMatch[1]} ${holeMatch[2]} groß.`);
  if (/überschwemm|wasser|leck|geflutet/.test(text) && /bad|badezimmer/.test(text)) happened.push("Es kam zu einer starken Überschwemmung im Badezimmer.");
  const waterMatch = text.match(/\b(\d+)\s*cm\b/);
  if (waterMatch) happened.push(`Das Wasser stand ungefähr ${waterMatch[1]} cm hoch.`);
  if (/feuerwehr/.test(text)) happened.push("Die Feuerwehr musste gerufen werden, um das Wasser abzupumpen.");
  if (/badewanne|wanne/.test(text)) todo.push("Die Badewanne ersetzen.");
  if (/toilette|wc|abfluss|abwasser/.test(text)) todo.push("Den Toilettenabfluss erneuern.");
  if (/sanitär|waschbecken|wc|toilette/.test(text)) todo.push("Alle Sanitärobjekte im Badezimmer ersetzen.");
  if (/überschwemm|wasser|schaden|schäden/.test(text)) todo.push("Alle durch die Überschwemmung entstandenen Schäden reparieren.");
  if (cause.length && /überschwemm|wasser/.test(text)) cause[0] = "Der Installateur hat während der Arbeiten versehentlich ein Rohr angebohrt und dadurch die starke Überschwemmung im Badezimmer verursacht.";

  return [
    "Notizen - Problem im Badezimmer",
    "",
    `Projekt: ${contextLabel}`,
    "",
    "Was passiert ist",
    ...(happened.length ? Array.from(new Set(happened)).map((line) => `- ${line}`) : [`- ${polishAiSentence(transcript)}`]),
    "",
    "Was zu tun ist",
    ...(todo.length ? Array.from(new Set(todo)).map((line) => `- ${line}`) : ["- Die notwendigen Reparaturarbeiten prüfen und bestätigen."]),
    "",
    "Ursache",
    ...(cause.length ? Array.from(new Set(cause)).map((line) => `- ${line}`) : ["- Die Ursache muss bestätigt werden."]),
  ].join("\n");
}

function buildStructuredSummaryDraft(transcript, contextLabel, language = getAiAssistantSettings().language) {
  if (language === "Greek") return buildGreekSummaryDraft(transcript, contextLabel);
  if (language === "Italian") return buildItalianSummaryDraft(transcript, contextLabel);
  if (language === "German" && isBathroomFloodTranscript(transcript)) return buildGermanBathroomSummaryDraft(transcript, contextLabel);
  if (language === "English" && isBathroomFloodTranscript(transcript)) return buildEnglishBathroomSummaryDraft(transcript, contextLabel);
  const labels = getAiSecretaryLabels(language);
  const text = cleanAiTranscript(transcript);
  const thoughts = splitAiThoughts(text);
  const questions = getAiClarifyingQuestions(text);
  const workLines = thoughts.filter(isLikelyWorkLine);
  const materialLines = thoughts.filter(isLikelyMaterialLine);
  const otherLines = thoughts.filter((line) => !workLines.includes(line) && !materialLines.includes(line));
  return [
    labels.title,
    "",
    `Project: ${contextLabel}`,
    "",
    labels.happened,
    ...(workLines.length ? workLines : thoughts).map((line) => `- ${polishAiSentence(line)}`),
    "",
    labels.todo,
    ...(materialLines.length ? materialLines.map((line) => `- ${polishAiSentence(line)}`) : ["- Needs check"]),
    "",
    labels.cause,
    ...(otherLines.length ? otherLines.map((line) => `- ${polishAiSentence(line)}`) : ["- Needs confirmation."]),
    formatAiQuestions(questions, language),
    "",
    labels.review,
    `- ${labels.reviewText}`,
  ].join("\n");
...
function getAiConversationLabels(language = getAiAssistantSettings().language) {
  if (language === "German") return { title: "Gesprächszusammenfassung", decisions: "Entscheidungen", tasks: "Aufgaben", next: "Nächste Schritte", note: "Hinweis der Assistenz" };
  if (language === "Greek") return { title: "Περίληψη συζήτησης", decisions: "Αποφάσεις", tasks: "Εργασίες", next: "Επόμενα βήματα", note: "Σημείωση βοηθού" };
  if (language === "Italian") return { title: "Riassunto della conversazione", decisions: "Decisioni", tasks: "Compiti", next: "Prossimi passi", note: "Nota dell'assistente" };
  return { title: "Conversation summary", decisions: "Decisions", tasks: "Tasks", next: "Next steps", note: "Assistant note" };
}

function buildConversationSummaryDraft(transcript, contextLabel, language = getAiAssistantSettings().language) {
  const labels = getAiConversationLabels(language);
  const text = cleanAiTranscript(transcript);
  const thoughts = splitAiThoughts(text);
  const taskLines = thoughts.filter((line) => /\b(need|must|should|todo|task|call|send|check|finish|prepare|order|πρέπει|χρει|aufgabe|muss|soll|bisogna|deve)\b/i.test(line));
  const decisionLines = thoughts.filter((line) => /\b(decided|agree|approved|confirmed|will|θα|αποφασ|συμφων|beschlossen|vereinbart|deciso|confermato)\b/i.test(line));
  return [
    labels.title,
    "",
    `Project: ${contextLabel}`,
    "",
    labels.decisions,
    ...(decisionLines.length ? decisionLines.map((line) => `- ${polishAiSentence(line)}`) : ["- Needs review"]),
    "",
    labels.tasks,
    ...(taskLines.length ? taskLines.map((line) => `- ${polishAiSentence(line)}`) : ["- Needs review"]),
    labels.next,
    ...(taskLines.length ? taskLines.map((line) => `- Follow up: ${polishAiSentence(line)}`) : ["- Review the summary and add next steps if needed."]),
    "",
    labels.note,
    "- Review before saving. Conversation summaries can miss speaker intent if the transcript is unclear.",
  ].join("\n");
}

function getAiDraftContext() {
  const project = getAiContextProject();
  const area = getAiContextArea(project);
  return {
    project,
    area,
    label: getAiContextLabel(),
    language: getAiAssistantSettings().language,
  };
}

function shouldAiReviewSavedNotes(text = "", settings = getAiAssistantSettings()) {
...
function collectAiSavedProjectNotes(project = getAiContextProject()) {
  if (!project) return [];
  const containers = [
    { label: "Project details", items: getProjectDetailsFolder(project)?.items || [] },
    ...(project.folders || []).filter((folder) => !folder.archivedAt).map((folder) => ({
      label: folder.name || "Team folder",
      items: folder.items || [],
    })),
    ...(project.areas || []).filter((area) => !area.archivedAt).map((area) => ({
      label: area.name || "Area",
      items: area.items || [],
    })),
  ];
  return containers.flatMap((container) => (container.items || [])
    .filter((item) => ["note", "task"].includes(item?.type) && !item.archivedAt && String(item.content || item.notes || item.title || "").trim())
    .map((item) => ({
      title: item.title || (item.type === "task" ? "Task" : "Note"),
      content: [
        item.type === "task" ? `Task status: ${item.status || "open"}` : "",
        item.assigneeName ? `Responsible: ${item.assigneeName}` : "",
        item.dueDate ? `Due: ${formatDateDisplay(item.dueDate)}` : "",
        String(item.content || item.notes || "").trim(),
      ].filter(Boolean).join("\n"),
...
function buildAiSavedNotesReviewTranscript(commandText, context = getAiDraftContext()) {
  const notes = collectAiSavedProjectNotes(context.project);
  if (!notes.length) {
    return `The user asked for a review of saved notes, but there are no saved notes yet for ${context.label}. Create a short helpful answer saying there are no saved notes to summarize.`;
  }
  return [
    `The user asked: ${cleanAiTranscript(commandText) || "Tell me what happened."}`,
    "",
    `Review these saved project notes for ${context.label}.`,
    "Make a short, useful management summary: what happened, current problems, decisions, tasks, risks, and next steps. Do not invent facts.",
    "",
    "Saved notes:",
    ...notes.map((note, index) => [
      `${index + 1}. ${note.title} (${note.source}${note.createdAt ? `, ${formatDateDisplay(note.createdAt)}` : ""})`,
      note.content,
    ].join("\n")),
  ].join("\n\n");
}

function getAiItemType(item, fallbackType = "note") {
  if (!item) return fallbackType;
  if (item.aiDraft && /offer/i.test(item.title || "")) return "offer";
  if (/material/i.test(item.title || "")) return "material";
  if (/conversation|meeting/i.test(item.title || "")) return "conversation";
  if (/problem|issue|damage|risk/i.test(item.title || "")) return "problem";
  if (/decision|decided|approved/i.test(item.title || "")) return "decision";
  if (item.dueDate || /deadline|due/i.test(item.title || "")) return "deadline";
  if (["note", "task", "file", "photo"].includes(item.type)) {
    if (item.type === "file") return "document";
    if (item.type === "photo") return "document";
    return item.type;
  }
  return fallbackType;
}

function trimAiRecordText(text, maxLength = 1200) {
  const value = String(text || "").trim();
  return value.length > maxLength ? `${value.slice(0, maxLength - 20)}... [truncated]` : value;
}

function inferAreaFloorHint(areaName = "") {
  const name = String(areaName || "").toLowerCase();
  const suffixMatch = name.match(/(?:^|[\s_-])(\d+)\s*$/);
  if (suffixMatch) return suffixMatch[1];
  if (/\b(first|1st|1)\s*floor\b/.test(name) || /\b(1ος|1ο|πρωτος|πρώτος|πρωτο|πρώτο)\b/.test(name)) return "1";
  if (/\b(second|2nd|2)\s*floor\b/.test(name) || /\b(2ος|2ο|δευτερος|δεύτερος|δευτερο|δεύτερο)\b/.test(name)) return "2";
  if (/\b(third|3rd|3)\s*floor\b/.test(name) || /\b(3ος|3ο|τριτος|τρίτος|τριτο|τρίτο)\b/.test(name)) return "3";
  return "";
}

function parseProjectAreaName(areaName = "") {
  const rawName = String(areaName || "").trim();
  const explicit = rawName.match(/^(.*?)\s*-\s*(\d+)\s*$/);
  const namePart = (explicit ? explicit[1] : rawName).trim() || rawName || "Untitled area";
  const floor = explicit ? explicit[2] : inferAreaFloorHint(rawName);
  const compact = namePart.toLowerCase().replace(/\s+/g, "");
  const roomType = /^(mpanio|banio|bath|bathroom|μπανιο|μπάνιο)/i.test(compact) ? "bathroom" : "";
  return {
    rawName,
    namePart,
    floor,
...
function getFloorAliases(floor = "") {
  const value = String(floor || "").trim();
  if (value === "1") return "first floor, 1st floor, floor 1, protos orofos, πρώτος όροφος";
  if (value === "2") return "second floor, 2nd floor, floor 2, deuteros orofos, δεύτερος όροφος";
  if (value === "3") return "third floor, 3rd floor, floor 3, tritos orofos, τρίτος όροφος";
  if (value === "4") return "fourth floor, 4th floor, floor 4, tetartos orofos, τέταρτος όροφος";
  return "";
}

function normalizeAreaLookupText(text = "") {
  return String(text || "")
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
function parseProjectAreaIdentity(area = {}) {
  const rawName = String(area.name || "").trim();
  const explicit = rawName.match(/^(.*?)\s*-\s*(\d+)\s*$/);
  const name = (explicit ? explicit[1] : rawName).trim() || rawName || "Untitled area";
  const lookup = normalizeAreaLookupText(`${rawName} ${area.floor || ""}`);
  let floor = explicit ? explicit[2] : "";
  if (!floor && /\b(first|1st|floor 1|level 1|protos|proto|prwtos|prwto|πρωτος|πρωτο|1ος|1ο)\b/.test(lookup)) floor = "1";
  if (!floor && /\b(second|2nd|floor 2|level 2|deuteros|deutero|defteros|deftero|δευτερος|δευτερο|2ος|2ο)\b/.test(lookup)) floor = "2";
  if (!floor && /\b(third|3rd|floor 3|level 3|tritos|trito|τριτος|τριτο|3ος|3ο)\b/.test(lookup)) floor = "3";
  if (!floor && /\b(fourth|4th|floor 4|level 4|tetartos|tetarto|τεταρτος|τεταρτο|4ος|4ο)\b/.test(lookup)) floor = "4";
  const compactName = normalizeAreaLookupText(name).replace(/\s+/g, "");
  const roomType = /^(mpanio|banio|bath|bathroom|μπανιο)/i.test(compactName) ? "bathroom" : "";
  return { rawName, name, floor, roomType };
}

function getQuestionFloorHint(question = "") {
  const text = normalizeAreaLookupText(question);
  if (!text) return "";
  if (/1st|1ος|1ο/.test(text)) return "1";
  if (/2nd|2ος|2ο/.test(text)) return "2";
  if (/3rd|3ος|3ο/.test(text)) return "3";
  if (/4th|4ος|4ο/.test(text)) return "4";
  if (/\b(first|1st|floor 1|level 1|protos|proto|prwtos|prwto|πρωτος|πρωτο|1ος|1ο)\b/.test(text)) return "1";
  if (/\b(second|2nd|floor 2|level 2|deuteros|deutero|defteros|deftero|δευτερος|δευτερο|2ος|2ο)\b/.test(text)) return "2";
  if (/\b(third|3rd|floor 3|level 3|tritos|trito|τριτος|τριτο|3ος|3ο)\b/.test(text)) return "3";
  if (/\b(fourth|4th|floor 4|level 4|tetartos|tetarto|τεταρτος|τεταρτο|4ος|4ο)\b/.test(text)) return "4";
  return "";
}

function isFloorQuestion(question = "") {
  const text = normalizeAreaLookupText(question);
  return Boolean(getQuestionFloorHint(question) && /\b(what|which|where|list|show|exei|echei|ti|τι|ποια|ποιο|υπαρχει|υπαρχουν|floor|orofos|ροφος|όροφος)\b/.test(text));
}

function inferProjectAreaRoomType(areaName = "") {
  const compact = normalizeAreaLookupText(areaName).replace(/\s+/g, "");
  if (/^(mpan|banio|bath|bathroom|\u03bc\u03c0\u03b1\u03bd\u03b9\u03bf)/i.test(compact)) return "bathroom";
  if (/^(wc|toilet)/i.test(compact)) return "wc";
  if (/^(kouzina|kuzina|kitchen|\u03ba\u03bf\u03c5\u03b6\u03b9\u03bd\u03b1)/i.test(compact)) return "kitchen";
  if (/^(saloni|living)/i.test(compact)) return "living room";
  if (/^(bedroom|room|domatio)/i.test(compact)) return "room";
  return "";
}

function getParsedProjectArea(area = {}) {
  const parsed = parseProjectAreaIdentity(area);
  return {
    ...parsed,
    roomType: parsed.roomType || inferProjectAreaRoomType(`${parsed.name || ""} ${parsed.rawName || ""}`),
  };
}

function getProjectFloorIndex(project = getAiContextProject()) {
  const floors = new Map();
  for (const area of project?.areas || []) {
    if (!area || area.archivedAt) continue;
    const parsed = getParsedProjectArea(area);
    if (!parsed.floor) continue;
    const entry = {
      id: area.id || "",
      rawName: parsed.rawName || area.name || "Untitled area",
      name: parsed.name || area.name || "Untitled area",
      floor: parsed.floor,
      roomType: parsed.roomType || "",
      status: area.completedAt ? "complete" : "open / not complete",
      completedAt: area.completedAt || "",
      openTasks: (area.items || []).filter((item) => item.type === "task" && item.status !== "Done" && !item.archivedAt).length,
      noteCount: (area.items || []).filter((item) => item.type === "note" && !item.archivedAt).length,
    };
    if (!floors.has(entry.floor)) floors.set(entry.floor, []);
    floors.get(entry.floor).push(entry);
  }
  return floors;
}

function getProjectFloorRecords(project = getAiContextProject()) {
  return [...getProjectFloorIndex(project).entries()].map(([floor, areas]) => {
    const bathroomCount = areas.filter((area) => area.roomType === "bathroom").length;
    const areaLines = areas.map((area) => [
      `- ${area.name}`,
      area.roomType ? `type: ${area.roomType}` : "type: unclear",
      `status: ${area.status}`,
      area.openTasks ? `open tasks: ${area.openTasks}` : "",
      area.noteCount ? `notes: ${area.noteCount}` : "",
    ].filter(Boolean).join("; "));
    return {
      type: "other",
      id: `floor-index:${project?.id || "project"}:${floor}`,
      title: `Floor ${floor} area index`,
      date: "",
      source: "Deterministic area parser",
      text: trimAiRecordText([
        `Floor: ${floor}`,
        `Area count: ${areas.length}`,
        `Bathroom count: ${bathroomCount}`,
        "Areas:",
        ...areaLines,
        "Rule: area names like NAME - FLOOR mean the area is on that floor.",
      ].join("\n")),
    };
  });
}

function getProjectAreaIndexRecords(project = getAiContextProject()) {
  const areas = (project?.areas || [])
    .filter((area) => area && !area.archivedAt)
    .map((area) => {
      const parsed = getParsedProjectArea(area);
      return {
        id: area.id || "",
  ...
  const lines = areas.map((area) => [
    `- ${area.name}`,
    `raw: ${area.rawName}`,
    area.floor ? `floor: ${area.floor}` : "floor: missing",
    area.roomType ? `type: ${area.roomType}` : "type: unclear",
    `status: ${area.status}`,
    area.openTasks ? `open tasks: ${area.openTasks}` : "",
    area.noteCount ? `notes: ${area.noteCount}` : "",
  ...
  return [{
    type: "other",
    id: `area-index:${project?.id || "project"}`,
    title: "Complete area index",
    date: "",
    source: "Deterministic area parser",
    text: trimAiRecordText([
      "Authoritative complete list of project areas:",
      ...lines,
      "Rules:",
      "- NAME - FLOOR means text before '-' is the area name and the number after '-' is the floor.",
      "- mpanio means bathroom.",
      "- If a user asks where a room type exists, list matching area names and floors.",
    ].join("\n"), 4000),
  }];
}

function buildProjectBrain(project = getAiContextProject(), question = "") {
  ...
  const areas = (project.areas || [])
    ...
    .map((area) => {
      ...
      return {
        ...
        floor: parsed.floor || "",
        status: area.completedAt ? "complete" : "open",
        completedAt: area.completedAt || "",
        openTasks: (area.items || []).filter((item) => item.type === "task" && item.status !== "Done" && !item.archivedAt).length,
        notes: (area.items || []).filter((item) => item.type === "note" && !item.archivedAt).map((item) => ({
          id: item.id || "",
          title: item.title || "Note",
          text: trimAiRecordText(item.content || item.text || item.notes || "", 700),
          createdAt: item.createdAt || "",
        })),
      };
    });
  ...
  for (const area of areas) {
    const key = area.floor || "unknown";
    if (!floors[key]) floors[key] = [];
    floors[key].push({
      id: area.id,
      name: area.name,
      rawName: area.rawName,
      roomType: area.roomType,
      status: area.status,
    });
  }
  const records = collectProjectAssistantData(project).map((record) => ({
    type: record.type || "other",
    id: record.id || "",
    title: record.title || "",
    source: record.source || "",
    date: record.date || "",
    text: trimAiRecordText(record.text || "", 1000),
  }));
  return {
    project: {
      id: project.id || "",
      number: formatProjectNumberValue(project.projectNumber) || "",
      name: project.name || "",
      client: client ? formatClientName(client) : "",
      status: project.status || "active",
      startDate: project.startDate || "",
    },
    interpretationRules: [
      "Area naming rule: NAME - FLOOR. Text before '-' is the area name. Number after '-' is the floor.",
      "Greek/Greeklish: mpanio/mpano/banio/bathroom means bathroom.",
      "Greek/Greeklish: kouzina/kuzina/kitchen means kitchen.",
      "If floor is empty, say that the floor is missing instead of inventing it.",
      "If status is open, say it is open / not complete.",
      "Use projectBrain.areas and projectBrain.floors as the source of truth for area/floor questions.",
    ],
    learnedRules: getProjectAssistantMemoryForRequest(project, question),
    areas,
    floors,
    records,
...
function collectProjectAssistantData(project = getAiContextProject()) {
  if (!project) return [];
  const client = getClientById(project.clientId);
  const records = [{
    type: "project",
    ...
    source: "Project",
    text: [
      `Project number: ${formatProjectNumberValue(project.projectNumber) || "missing"}`,
      `Project name: ${project.name || "missing"}`,
      `Client: ${client ? formatClientName(client) : "missing"}`,
      `Start date: ${project.startDate ? formatDateDisplay(project.startDate) : "missing"}`,
      `Status: ${project.status || "active"}`,
    ].join("\n"),
  }];
  records.push(...getProjectAreaIndexRecords(project), ...getProjectFloorRecords(project));
  for (const area of project.areas || []) {
    const parsedArea = getParsedProjectArea(area);
    const floorHint = parsedArea.floor;
    const openTasks = (area.items || []).filter((item) => item.type === "task" && item.status !== "Done" && !item.archivedAt).length;
    const noteCount = (area.items || []).filter((item) => item.type === "note" && !item.archivedAt).length;
    const areaStatus = area.archivedAt ? "archived" : area.completedAt ? "complete" : "open / not complete";
    records.push({
      type: "other",
      id: area.id || crypto.randomUUID(),
      title: `Area: ${area.name || "Untitled area"}`,
      date: area.completedAt || "",
      source: "Project areas",
      text: trimAiRecordText([
        `Area name: ${area.name || "Untitled area"}`,
        `Parsed area name: ${parsedArea.name}`,
        parsedArea.roomType ? `Interpreted area type: ${parsedArea.roomType}` : "Interpreted area type: unclear",
        `Area status: ${areaStatus}`,
        floorHint ? `Floor hint from area name: ${floorHint}` : "Floor hint from area name: missing",
        floorHint ? `Floor aliases: ${getFloorAliases(floorHint)}` : "",
        floorHint ? `Area naming rule used: NAME - FLOOR` : "",
        `Open tasks in area: ${openTasks}`,
        `Saved notes in area: ${noteCount}`,
        area.completedAt ? `Completed at: ${formatDateDisplay(area.completedAt)}` : "Completed at: missing",
      ].join("\n")),
    });
  }
  const containers = [
    { source: "Project details", items: getProjectDetailsFolder(project)?.items || [] },
    ...(project.areas || []).filter((area) => !area.archivedAt).map((area) => ({ source: `Area: ${area.name || "Untitled area"}`, items: area.items || [] })),
    ...(project.folders || []).filter((folder) => !folder.archivedAt).map((folder) => ({ source: `Team: ${folder.name || "Untitled team"}`, items: folder.items || [] })),
  ];
    ...
    for (const item of container.items || []) {
      ...
      const text = [
        ...
        Array.isArray(item.checklist) && item.checklist.length
          ? `Checklist:\n${item.checklist.map((entry) => `- ${entry.done ? "[x]" : "[ ]"} ${entry.text || ""}`).join("\n")}`
          : "",
        item.status ? `Status: ${item.status}` : "",
        item.assigneeName ? `Responsible: ${item.assigneeName}` : "",
        item.dueDate ? `Due: ${formatDateDisplay(item.dueDate)}` : "",
        item.fileName ? `Document: ${item.fileName}` : "",
        item.imageName ? `Image: ${item.imageName}` : "",
        item.caption || "",
      ].filter(Boolean).join("\n").trim();
      if (!text) continue;
      records.push({
        type: getAiItemType(item),
        id: item.id || crypto.randomUUID(),
        title: item.title || item.name || item.fileName || "Project record",
        date: item.createdAt || item.updatedAt || item.dueDate || "",
        source: container.source,
        text: trimAiRecordText(text),
      });
    }
  }
  ...
  for (const message of project.chatMessages || []) {
    if (!message || (!message.text && !(message.attachments || []).length)) continue;
    const channel = channelById.get(message.channelId || "project:general");
    const sender = getUserById(message.createdByUserId);
    const text = [
      message.text || "",
      sender ? `Sender: ${getMemberDisplayName(sender)}` : "",
      channel ? `Channel: ${channel.title}` : "",
      message.importantAt ? "Marked important" : "",
      Array.isArray(message.attachments) && message.attachments.length
        ? `Attachments: ${message.attachments.map((attachment) => attachment.name || "Attachment").join(", ")}`
        : "",
    ].filter(Boolean).join("\n").trim();
    records.push({
      type: "conversation",
      id: message.id || crypto.randomUUID(),
      title: channel ? `Chat: ${channel.title}` : "Project chat",
      date: message.createdAt || "",
      source: channel ? `Chat: ${channel.title}` : "Project chat",
      text: trimAiRecordText(text),
    });
  }
  return records;
...
function scoreProjectAssistantRecord(record, question) {
  const words = cleanAiTranscript(question).toLowerCase().split(/\W+/).filter((word) => word.length > 2);
  const haystack = `${record.title || ""} ${record.source || ""} ${record.text || ""}`.toLowerCase();
  let score = 0;
  for (const word of words) {
    if (haystack.includes(word)) score += 2;
  }
  const normalizedQuestion = cleanAiTranscript(question).toLowerCase();
  if (/\b(first|1st|protos|πρώτος|πρωτος)\b/.test(normalizedQuestion) && haystack.includes("floor hint from area name: 1")) score += 20;
  if (/\b(second|2nd|deuteros|δεύτερος|δευτερος)\b/.test(normalizedQuestion) && haystack.includes("floor hint from area name: 2")) score += 20;
  if (/\b(third|3rd|tritos|τρίτος|τριτος)\b/.test(normalizedQuestion) && haystack.includes("floor hint from area name: 3")) score += 20;
  if (/\b(fourth|4th|tetartos|τέταρτος|τεταρτος)\b/.test(normalizedQuestion) && haystack.includes("floor hint from area name: 4")) score += 20;
  if (["task", "material", "offer", "conversation"].some((type) => haystack.includes(type) && question.toLowerCase().includes(type))) score += 5;
  return score;
}

function getRelevantProjectAssistantData(question, project = getAiContextProject()) {
  const records = collectProjectAssistantData(project);
  const memoryRecords = getProjectAssistantMemoryForRequest(project, question).map((entry) => ({
    type: "other",
    id: `memory:${entry.updatedAt}:${entry.trigger}`,
    title: entry.type === "naming_rule" ? "Assistant memory: naming rule" : "Assistant memory: user rule",
    date: entry.updatedAt || "",
    source: "Assistant memory",
    text: [entry.ruleText, entry.trigger ? `Trigger: ${entry.trigger}` : "", entry.meaning ? `Meaning: ${entry.meaning}` : ""].filter(Boolean).join("\n"),
  }));
  records.unshift(...memoryRecords);
  const scored = records
    .map((record) => ({ record, score: scoreProjectAssistantRecord(record, question) }))
    .sort((left, right) => right.score - left.score);
  const relevant = scored.filter((entry) => entry.score > 0).map((entry) => entry.record);
  const alwaysIncludeTypes = new Set(["note", "conversation", "offer", "material", "problem", "decision", "deadline", "other"]);
  const alwaysIncluded = records.filter((record) => alwaysIncludeTypes.has(record.type));
  const merged = [...alwaysIncluded, ...(relevant.length ? relevant : scored.map((entry) => entry.record))];
  return merged.filter((record, index, list) => list.findIndex((entry) => entry.id === record.id && entry.source === record.source) === index).slice(0, 200);
}

function buildLocalProjectAssistantAnswer({ question, projectContext, projectData }) {
  const records = Array.isArray(projectData) ? projectData : [];
  if (!records.length) {
    return {
      answer: "I do not have saved project information for this yet.\n\n- Add notes, tasks, materials, or decisions first.\n- Then I can answer from the project data.",
      usedProjectData: [],
      missingInformation: ["No saved project data found."],
      suggestedFollowUpQuestions: ["What happened in this project?", "What is still open?"],
      source: "local-project-fallback",
    };
  }
  const lines = records.slice(0, 5).map((record) => `- ${record.title}: ${String(record.text || "").split(/\n/).filter(Boolean).slice(0, 2).join(" ")}`);
  return {
    answer: [
      "I found these project records. ChatGPT is not available, so this is a basic local answer.",
      "",
      ...lines,
      "",
      "Please check the details before using this answer.",
    ].join("\n"),
    usedProjectData: records.slice(0, 5).map((record) => ({
      type: record.type,
      id: record.id,
      title: record.title,
      date: record.date || "",
    })),
    missingInformation: ["ChatGPT was not available for deeper reasoning."],
    suggestedFollowUpQuestions: ["What needs to be done next?", "What is missing?"],
    source: "local-project-fallback",
  };
}

function isGreekAssistantQuestion(text = "", language = "") {
  return /[\u0370-\u03ff]/.test(String(text || "")) || /greek|ελλην/i.test(String(language || ""));
}

function formatFloorLabel(floor = "", greek = false) {
  const value = String(floor || "").trim();
  if (!greek) {
    if (value === "1") return "first floor";
    if (value === "2") return "second floor";
    if (value === "3") return "third floor";
    if (value === "4") return "fourth floor";
    return `floor ${value}`;
  }
  if (value === "1") return "1ο όροφο";
  if (value === "2") return "2ο όροφο";
  if (value === "3") return "3ο όροφο";
  if (value === "4") return "4ο όροφο";
  return `όροφο ${value}`;
}

function buildDeterministicFloorAssistantAnswer({ question, project, userLanguage }) {
  if (!project || !isFloorQuestion(question)) return null;
  const floor = getQuestionFloorHint(question);
  if (!floor) return null;
  const areas = getProjectFloorIndex(project).get(floor) || [];
  const greek = isGreekAssistantQuestion(question, userLanguage);
  const floorLabel = formatFloorLabel(floor, greek);
  if (!areas.length) {
    return {
      assistantMessage: greek
        ? `Δεν βρίσκω περιοχές στον ${floorLabel} από τα ονόματα των περιοχών του έργου.`
        : `I do not find any areas on the ${floorLabel} from the project area names.`,
      answer: "",
      activeProjectId: project.id || "",
      activeProjectName: project.name || "",
      usedProjectData: [],
      missingInformation: greek ? [`Δεν βρέθηκαν περιοχές για τον ${floorLabel}.`] : [`No areas found for the ${floorLabel}.`],
      needsProjectConfirmation: false,
      suggestedActions: ["save_conversation", "make_resume", "leave"],
      source: "deterministic-area-parser",
      projectData: getProjectFloorRecords(project),
    };
  }
  const bathroomAreas = areas.filter((area) => area.roomType === "bathroom");
  const allBathrooms = bathroomAreas.length === areas.length;
  const names = areas.map((area) => area.name).join(", ");
  const openCount = areas.filter((area) => area.status !== "complete").length;
  let assistantMessage = "";
  if (greek) {
    const typeText = allBathrooms
      ? `${areas.length === 1 ? "1 μπάνιο" : `${areas.length} μπάνια`}`
      : `${areas.length === 1 ? "1 περιοχή" : `${areas.length} περιοχές`}`;
    const statusText = openCount === areas.length
      ? areas.length === 1 ? "Είναι ακόμα ανοιχτό / όχι ολοκληρωμένο." : "Είναι ακόμα ανοιχτά / όχι ολοκληρωμένα."
      : `${openCount} από αυτά είναι ακόμα ανοιχτά / όχι ολοκληρωμένα.`;
    assistantMessage = `Στον ${floorLabel} υπάρχουν ${typeText}: ${names}. ${statusText}`;
  } else {
    const typeText = allBathrooms
      ? `${areas.length === 1 ? "1 bathroom" : `${areas.length} bathrooms`}`
      : `${areas.length === 1 ? "1 area" : `${areas.length} areas`}`;
    const statusText = openCount === areas.length
      ? areas.length === 1 ? "It is still open / not complete." : "They are still open / not complete."
      : `${openCount} of them are still open / not complete.`;
    assistantMessage = `On the ${floorLabel}, there are ${typeText}: ${names}. ${statusText}`;
  }
  return {
    assistantMessage,
    ...
    activeProjectId: project.id || "",
    activeProjectName: project.name || "",
    usedProjectData: areas.map((area) => ({
      type: "other",
      id: area.id,
      title: `Area: ${area.rawName}`,
      date: area.completedAt || "",
    })),
    missingInformation: [],
    needsProjectConfirmation: false,
...
function getQuestionFloorHintStrict(question = "") {
  const text = normalizeAreaLookupText(question);
  if (!text) return "";
  const compact = text.replace(/\s+/g, "");
  if (/(^|\D)1(st)?(\D|$)/.test(text) || /\b(first|floor 1|level 1|protos|proto|prwtos|prwto)\b/.test(text) || /\u03c0\u03c1\u03c9\u03c4/.test(compact)) return "1";
  if (/(^|\D)2(nd)?(\D|$)/.test(text) || /\b(second|floor 2|level 2|deuteros|deutero|defteros|deftero)\b/.test(text) || /\u03b4\u03b5\u03c5\u03c4\u03b5\u03c1/.test(compact)) return "2";
  if (/(^|\D)3(rd)?(\D|$)/.test(text) || /\b(third|floor 3|level 3|tritos|trito)\b/.test(text) || /\u03c4\u03c1\u03b9\u03c4/.test(compact)) return "3";
  if (/(^|\D)4(th)?(\D|$)/.test(text) || /\b(fourth|floor 4|level 4|tetartos|tetarto)\b/.test(text) || /\u03c4\u03b5\u03c4\u03b1\u03c1\u03c4/.test(compact)) return "4";
  return "";
}

function isFloorQuestionStrict(question = "") {
  const text = normalizeAreaLookupText(question);
  const compact = text.replace(/\s+/g, "");
  const mentionsFloor = /\b(floor|level|orofos)\b/.test(text) || /\u03bf\u03c1\u03bf\u03c6/.test(compact);
  const asksContent = /\b(what|which|where|list|show|exei|echei|ti)\b/.test(text)
    || /\u03c4\u03b9/.test(text)
    || /\u03b5\u03c7\u03c9|\u03b5\u03c7\u03b5\u03b9|\u03c5\u03c0\u03b1\u03c1\u03c7/.test(compact);
  return Boolean(getQuestionFloorHintStrict(question) && (mentionsFloor || asksContent));
}

function formatFloorLabelStrict(floor = "", greek = false) {
  const value = String(floor || "").trim();
  if (!greek) {
    if (value === "1") return "first floor";
    if (value === "2") return "second floor";
  ...
  return `${value}\u03bf \u03cc\u03c1\u03bf\u03c6\u03bf`;
}

function buildDeterministicFloorAssistantAnswerStrict({ question, project, userLanguage }) {
  if (!project || !isFloorQuestionStrict(question)) return null;
  const floor = getQuestionFloorHintStrict(question);
  const areas = getProjectFloorIndex(project).get(floor) || [];
  const greek = isGreekAssistantQuestion(question, userLanguage);
  const floorLabel = formatFloorLabelStrict(floor, greek);
  if (!areas.length) {
    const message = greek
      ? `\u0394\u03b5\u03bd \u03b2\u03c1\u03af\u03c3\u03ba\u03c9 \u03c0\u03b5\u03c1\u03b9\u03bf\u03c7\u03ad\u03c2 \u03c3\u03c4\u03bf\u03bd ${floorLabel}.`
      : `I do not find any areas on the ${floorLabel}.`;
    return {
  ...
  const openCount = areas.filter((area) => area.status !== "complete").length;
  const message = greek
    ? `\u03a3\u03c4\u03bf\u03bd ${floorLabel} \u03c5\u03c0\u03ac\u03c1\u03c7\u03bf\u03c5\u03bd ${allBathrooms ? (areas.length === 1 ? "1 \u03bc\u03c0\u03ac\u03bd\u03b9\u03bf" : `${areas.length} \u03bc\u03c0\u03ac\u03bd\u03b9\u03b1`) : (areas.length === 1 ? "1 \u03c0\u03b5\u03c1\u03b9\u03bf\u03c7\u03ae" : `${areas.length} \u03c0\u03b5\u03c1\u03b9\u03bf\u03c7\u03ad\u03c2`)}: ${names}. ${openCount === areas.length ? "\u0395\u03af\u03bd\u03b1\u03b9 \u03b1\u03ba\u03cc\u03bc\u03b1 \u03b1\u03bd\u03bf\u03b9\u03c7\u03c4\u03ac / \u03cc\u03c7\u03b9 \u03bf\u03bb\u03bf\u03ba\u03bb\u03b7\u03c1\u03c9\u03bc\u03ad\u03bd\u03b1." : `${openCount} \u03b1\u03c0\u03cc \u03b1\u03c5\u03c4\u03ac \u03b5\u03af\u03bd\u03b1\u03b9 \u03b1\u03ba\u03cc\u03bc\u03b1 \u03b1\u03bd\u03bf\u03b9\u03c7\u03c4\u03ac / \u03cc\u03c7\u03b9 \u03bf\u03bb\u03bf\u03ba\u03bb\u03b7\u03c1\u03c9\u03bc\u03ad\u03bd\u03b1.`}`
    : `On the ${floorLabel}, there are ${allBathrooms ? (areas.length === 1 ? "1 bathroom" : `${areas.length} bathrooms`) : (areas.length === 1 ? "1 area" : `${areas.length} areas`)}: ${names}. ${openCount === areas.length ? "They are still open / not complete." : `${openCount} of them are still open / not complete.`}`;
  return {
    assistantMessage: message,
    answer: message,
    activeProjectId: project.id || "",
...
function getQuestionRoomTypeHint(question = "") {
  const text = normalizeAreaLookupText(question).replace(/\s+/g, "");
  if (/mpanio|banio|bathroom|bath|μπανιο/.test(text)) return "bathroom";
  return "";
}

function isRoomLocationQuestion(question = "") {
  const normalized = normalizeAreaLookupText(question);
  const compact = normalized.replace(/\s+/g, "");
  const roomType = getQuestionRoomTypeHint(question);
  if (!roomType) return false;
  return /\b(where|which|what|list|show|pou|pu|where)\b/.test(normalized)
    || /\u03c0\u03bf\u03c5|\u03c0\u03bf\u03b9\u03b1|\u03c0\u03bf\u03b9\u03bf|\u03b5\u03c7\u03c9|\u03b5\u03c7\u03b5\u03b9|\u03c5\u03c0\u03b1\u03c1\u03c7/.test(compact);
}

function buildDeterministicRoomLocationAnswer({ question, project, userLanguage }) {
  if (!project || !isRoomLocationQuestion(question)) return null;
  const roomType = getQuestionRoomTypeHint(question);
  const greek = isGreekAssistantQuestion(question, userLanguage);
  const matches = (project.areas || [])
    .filter((area) => area && !area.archivedAt)
    .map((area) => ({ area, parsed: getParsedProjectArea(area) }))
    .filter(({ parsed }) => parsed.roomType === roomType);
  if (!matches.length) return null;
  const lines = matches.map(({ area, parsed }) => ({
    id: area.id || "",
    rawName: parsed.rawName || area.name || "Untitled area",
    name: parsed.name || area.name || "Untitled area",
    floor: parsed.floor || "",
  ...
  const names = lines.map((entry) => entry.floor
    ? `${entry.name} (${greek ? formatFloorLabelStrict(entry.floor, true) : formatFloorLabelStrict(entry.floor, false)})`
    : `${entry.name} (${greek ? "\u03cc\u03c1\u03bf\u03c6\u03bf\u03c2 \u03b4\u03b5\u03bd \u03b2\u03c1\u03ad\u03b8\u03b7\u03ba\u03b5" : "floor missing"})`
  ).join(", ");
  const openCount = lines.filter((entry) => entry.status !== "complete").length;
  const message = greek
    ? `\u0392\u03c1\u03af\u03c3\u03ba\u03c9 ${lines.length === 1 ? "1 \u03bc\u03c0\u03ac\u03bd\u03b9\u03bf" : `${lines.length} \u03bc\u03c0\u03ac\u03bd\u03b9\u03b1`}: ${names}. ${openCount === lines.length ? "\u0395\u03af\u03bd\u03b1\u03b9 \u03b1\u03ba\u03cc\u03bc\u03b1 \u03b1\u03bd\u03bf\u03b9\u03c7\u03c4\u03ac / \u03cc\u03c7\u03b9 \u03bf\u03bb\u03bf\u03ba\u03bb\u03b7\u03c1\u03c9\u03bc\u03ad\u03bd\u03b1." : `${openCount} \u03b1\u03c0\u03cc \u03b1\u03c5\u03c4\u03ac \u03b5\u03af\u03bd\u03b1\u03b9 \u03b1\u03ba\u03cc\u03bc\u03b1 \u03b1\u03bd\u03bf\u03b9\u03c7\u03c4\u03ac / \u03cc\u03c7\u03b9 \u03bf\u03bb\u03bf\u03ba\u03bb\u03b7\u03c1\u03c9\u03bc\u03ad\u03bd\u03b1.`}`
    : `I find ${lines.length === 1 ? "1 bathroom" : `${lines.length} bathrooms`}: ${names}. ${openCount === lines.length ? "They are still open / not complete." : `${openCount} of them are still open / not complete.`}`;
  return {
    assistantMessage: message,
    answer: message,
    activeProjectId: project.id || "",
    activeProjectName: project.name || "",
    usedProjectData: lines.map((entry) => ({
      type: "other",
      id: entry.id,
      title: `Area: ${entry.rawName}`,
      date: "",
    })),
    missingInformation: [],
    needsProjectConfirmation: false,
    suggestedActions: ["save_conversation", "make_resume", "leave"],
    source: "deterministic-area-parser",
    projectData: getProjectAreaIndexRecords(project),
  };
}

function buildLocalAiSecretaryDraft(mode, transcript, context, settings = getAiAssistantSettings()) {
  const cleanText = cleanAiTranscript(transcript);
  const contextLabel = context?.label || getAiContextLabel();
  const thoughts = splitAiThoughts(cleanText);
  const questions = getAiClarifyingQuestions(cleanText);
  if (!cleanText) return getAiDraftTemplate(mode, settings);
  if (mode === "summary") {
    return buildStructuredSummaryDraft(cleanText, contextLabel, settings.language);
  }
  if (mode === "conversation") {
    return buildConversationSummaryDraft(cleanText, contextLabel, settings.language);
  }
  if (mode === "offer") {
    const workLines = thoughts.filter(isLikelyWorkLine);
    const materialLines = thoughts.filter(isLikelyMaterialLine);
    ...
    return [
      ...
      "",
      "Notes for review",
      ...(otherLines.length ? otherLines.map((line) => `- ${polishAiSentence(line)}`) : ["- Confirm labor, quantities, and final wording before sending."]),
      formatAiQuestions(questions, settings.language),
      "",
      "Assistant note: You can add more details, or review and save this draft.",
    ].join("\n");
  }
  if (mode === "material") {
    const lines = thoughts.length ? thoughts : [cleanText];
    return [
      "Material list to order",
      "",
      `Project: ${contextLabel}`,
      "",
      ...lines.map((line) => `- ${polishAiSentence(line)}${/\d/.test(line) ? "" : " needs check"}`),
      formatAiQuestions(questions, settings.language),
      "",
      "Assistant note: Add more items if needed, or review and save.",
    ].join("\n");
  }
  return [
    ...
    `Project: ${contextLabel}`,
    "",
    thoughts.map(polishAiSentence).join(" "),
    formatAiQuestions(questions, settings.language),
    "",
    "Assistant note: You can continue speaking to add another paragraph, or review and save.",
  ].join("\n");
}
...
function formatAiEndpointError(body, fallbackMessage) {
  ...
  if (body.error?.message) return `${body.error.type || "openai_error"}: ${body.error.message}`;
  if (body.message) return `${body.type || "openai_error"}: ${body.message}`;
  return fallbackMessage;
}

async function fetchAiSecretaryJson(path, options = {}) {
  const response = await fetch(`${AI_SECRETARY_BASE_URL}${path}`, options);
  let body = null;
  try {
    body = await response.json();
  } catch (error) {
    // Keep the generic status message if the server did not return JSON.
  }
  if (!response.ok) {
    throw new Error(formatAiEndpointError(body, `AI secretary request failed: ${response.status}`));
  }
  return body || {};
}

async function checkAiSecretaryHealth() {
  const health = await fetchAiSecretaryJson("/health");
  if (!health.openaiReachable) {
    throw new Error(formatAiEndpointError(health, "ChatGPT is not reachable from the AI secretary server."));
  }
  return health;
}

function getRecentAiChatMemory(history = aiChatHistory) {
...
function getLastAssistantAnswerFromChat(history = aiChatHistory) {
  return [...(Array.isArray(history) ? history : [])]
    .reverse()
    .find((message) => message?.role === "assistant" && String(message?.content || "").trim())?.content || "";
}

function getAiConversationDraftContext(currentMessage = "", history = aiChatHistory) {
  const recentHistory = getRecentAiChatMemory(history);
  const currentUserMessage = cleanAiTranscript(currentMessage);
  const currentEditableDraft = String(els.aiDraftText?.value || "").trim();
  const previousAssistantAnswer = String(getLastAssistantAnswerFromChat(history) || "").trim();
  return {
    currentUserMessage,
    currentEditableDraft,
    previousAssistantAnswer,
    chatHistory: currentUserMessage
      ? [...recentHistory, { role: "user", content: currentUserMessage, createdAt: new Date().toISOString() }]
      : recentHistory,
  };
}

function isAiRevisionRequest(message = "") {
  const text = normalizeAreaLookupText(message);
  return /\b(add|include|continue|change|update|rewrite|combine|together|all together|whole|complete|merge|also|more|same|previous|resume|summary|summarize|zusammen|weiter|ergänz|ergaenz|aggiungi|insieme|continua|resume|συνέχ|μαζί|ολα|όλα|προσθε|συνοψη|περιληψη)\b/i.test(text);
}

function isAiResumeRequest(message = "") {
  return /\b(resume|summary|summarize|recap|zusammenfassung|riassunto|συνοψη|σύνοψη|περιληψη|περίληψη)\b/i.test(normalizeAreaLookupText(message));
}

function buildAiFallbackTranscript(transcript, conversationContext = {}) {
  const cleanText = cleanAiTranscript(transcript);
  const previousDraft = cleanAiTranscript(conversationContext.currentEditableDraft || conversationContext.previousAssistantAnswer || "");
  const historyText = (conversationContext.chatHistory || [])
    .map((message) => `${message.role === "user" ? "User" : "Assistant"}: ${message.content}`)
    .join("\n\n");
  if (isAiResumeRequest(cleanText) && historyText) {
    return [
      "Recent conversation:",
      historyText,
      "",
      "Create a complete structured summary. Preserve important details, quantities, dates, names, materials, tasks, risks, and next steps.",
    ].join("\n");
  }
  if (isAiRevisionRequest(cleanText) && previousDraft) {
    return [
      "Existing draft:",
      previousDraft,
      "",
      ...
      cleanText,
      "",
      "Rewrite everything as one complete updated note. Keep the important details from the existing draft unless the user corrected them.",
    ].join("\n");
  }
  return cleanText;
}

function buildLocalConversationAwareDraft({ mode, transcript, context, settings, conversationContext }) {
  const cleanText = cleanAiTranscript(transcript);
  const previousDraft = cleanAiTranscript(conversationContext?.currentEditableDraft || conversationContext?.previousAssistantAnswer || "");
  const historyText = (conversationContext?.chatHistory || [])
    .map((message) => `${message.role === "user" ? "User" : "Assistant"}: ${message.content}`)
    .join("\n\n");
  if (isAiResumeRequest(cleanText) && historyText) {
    return buildConversationSummaryDraft(historyText, context?.label || getAiContextLabel(), settings.language);
  }
  if (isAiRevisionRequest(cleanText) && previousDraft) {
    return [
      previousDraft,
      ...
      `- ${polishAiSentence(cleanText)}`,
    ].join("\n");
  }
  return buildLocalAiSecretaryDraft(mode, buildAiFallbackTranscript(cleanText, conversationContext), context, settings);
}

async function requestAiSecretaryDraft({ mode, transcript, context, settings, conversationContext = null }) {
  await checkAiSecretaryHealth();
  const memory = getAiMemoryForRequest({ mode, settings });
  const response = await fetch(`${AI_SECRETARY_BASE_URL}/api/ai-secretary`, {
    method: "POST",
    ...
    body: JSON.stringify({
      ...
      memory,
      currentUserMessage: conversationContext?.currentUserMessage || transcript,
      currentDraft: conversationContext?.currentEditableDraft || "",
      previousAssistantAnswer: conversationContext?.previousAssistantAnswer || "",
      chatHistory: Array.isArray(conversationContext?.chatHistory) ? conversationContext.chatHistory : [],
  ...
  if (!response.ok) {
    let message = `AI secretary request failed: ${response.status}`;
    try {
      const errorBody = await response.json();
      message = formatAiEndpointError(errorBody, message);
    } catch (error) {
      // Keep the generic status message if the server did not return JSON.
    }
...
async function requestProjectAssistant(payload) {
  await checkAiSecretaryHealth();
  const response = await fetch(`${AI_SECRETARY_BASE_URL}/api/project-assistant`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
  if (!response.ok) {
    let message = `Project assistant request failed: ${response.status}`;
    try {
      const errorBody = await response.json();
      message = formatAiEndpointError(errorBody, message);
    } catch (error) {
...
async function askProjectAssistant({
  ...
  selectedProjectId,
  selectedProjectName,
  projectContext = getAiDraftContext(),
  userLanguage = getAiAssistantSettings().language,
  userSettings = getAiAssistantSettings(),
} = {}) {
  const cleanQuestion = cleanAiTranscript(userMessage || question);
  const requestedProjectId = selectedProjectId || projectContext.project?.id || "";
  const project = getAiProjectOptions().find((entry) => entry.id === requestedProjectId) || projectContext.project || getAiContextProject();
  const projectData = getRelevantProjectAssistantData(cleanQuestion, project);
  const assistantMemory = getProjectAssistantMemoryForRequest(project, cleanQuestion);
  const projectBrain = buildProjectBrain(project, cleanQuestion);
  const deterministicFloorAnswer = buildDeterministicRoomLocationAnswer({
    question: cleanQuestion,
    project,
    userLanguage,
  }) || buildDeterministicFloorAssistantAnswerStrict({
    question: cleanQuestion,
    project,
    userLanguage,
  }) || buildDeterministicFloorAssistantAnswer({
    question: cleanQuestion,
    project,
    userLanguage,
  });
  if (deterministicFloorAnswer) return deterministicFloorAnswer;
  const availableProjects = getAiProjectOptions().map((entry) => ({
    id: entry.id,
    name: entry.name || "Untitled project",
    projectNumber: formatProjectNumberValue(entry.projectNumber),
  }));
  const payload = {
    userMessage: cleanQuestion,
    chatHistory,
    selectedProjectId: selectedProjectId || project?.id || "",
    selectedProjectName: selectedProjectName || project?.name || "",
    availableProjects,
    projectContext: {
      id: project?.id || "",
      name: project?.name || "",
      label: projectContext.label || getAiContextLabel(),
      area: projectContext.area ? { id: projectContext.area.id, name: projectContext.area.name || "" } : null,
  ...
  try {
    const result = await requestProjectAssistant(payload);
    return {
      assistantMessage: String(result.assistantMessage || result.answer || ""),
      answer: String(result.assistantMessage || result.answer || ""),
      activeProjectId: String(result.activeProjectId || payload.selectedProjectId || ""),
      activeProjectName: String(result.activeProjectName || payload.selectedProjectName || ""),
      usedProjectData: Array.isArray(result.usedProjectData) ? result.usedProjectData : [],
      missingInformation: Array.isArray(result.missingInformation) ? result.missingInformation : [],
      needsProjectConfirmation: Boolean(result.needsProjectConfirmation),
      suggestedActions: Array.isArray(result.suggestedActions) ? result.suggestedActions : [],
      suggestedFollowUpQuestions: Array.isArray(result.suggestedFollowUpQuestions) ? result.suggestedFollowUpQuestions : [],
      source: result.source || "openai",
      projectData,
    };
  } catch (error) {
    console.warn("Project assistant endpoint unavailable; using local fallback.", error);
    return {
      ...buildLocalProjectAssistantAnswer({ question: cleanQuestion, projectContext, projectData }),
      assistantMessage: buildLocalProjectAssistantAnswer({ question: cleanQuestion, projectContext, projectData }).answer,
      activeProjectId: project?.id || "",
      activeProjectName: project?.name || "",
      errorMessage: error?.message || "ChatGPT is not available.",
      projectData,
    };
  }
}

function getAiOpeningMessage() {
  const project = getAiContextProject();
  if (!project) return "Tell me what you want to write. I will keep this as an AI draft until you save it.";
  const projectName = project.name || "No project selected";
  return `Tell me what you want to write for ${projectName}. I will organize it into a clear note, list, or draft.`;
}

function renderAiMessageActions(message, index) {
  if (message.role !== "assistant") return "";
  const feedback = ["like", "dislike"].includes(message.feedback) ? message.feedback : "";
  const likePressed = feedback === "like";
  const dislikePressed = feedback === "dislike";
  return `
    <div class="ai-message-actions" aria-label="Assistant answer actions">
      <button class="ai-message-action${likePressed ? " active" : ""}" data-ai-message-action="like" data-ai-message-index="${index}" type="button" title="Like this answer" aria-label="Like this answer" aria-pressed="${likePressed}">Like</button>
      <button class="ai-message-action${dislikePressed ? " active" : ""}" data-ai-message-action="dislike" data-ai-message-index="${index}" type="button" title="Dislike this answer" aria-label="Dislike this answer" aria-pressed="${dislikePressed}">Dislike</button>
      <button class="ai-message-action" data-ai-message-action="copy" data-ai-message-index="${index}" type="button" title="Copy this answer" aria-label="Copy this answer">Copy</button>
      <button class="ai-message-action" data-ai-message-action="export" data-ai-message-index="${index}" type="button" title="Export this answer to Word" aria-label="Export this answer to Word">Export</button>
    </div>
  `;
}
...
function renderAiChat(options = {}) {
  ...
  const previousScrollTop = els.aiChatLog.scrollTop;
  els.aiChatLog.innerHTML = aiChatHistory.map((message, index) => `
    <article class="ai-chat-message ${message.role === "user" ? "user" : "assistant"}">
      <div class="ai-chat-stack">
        <div class="ai-chat-bubble">
          <p>${formatAiChatContent(message.content || "", message.role === "assistant")}</p>
        </div>
        ${renderAiMessageActions(message, index)}
      </div>
    </article>
  `).join("");
  els.aiChatLog.scrollTop = options.preserveScroll ? previousScrollTop : els.aiChatLog.scrollHeight;
}
...
async function onAiChatActionClick(event) {
  const button = event.target?.closest?.("[data-ai-message-action]");
  if (!button || !els.aiChatLog?.contains(button)) return;
  const index = Number(button.dataset.aiMessageIndex);
  const message = aiChatHistory[index];
  if (!message || message.role !== "assistant") return;
  const action = button.dataset.aiMessageAction || "";
  if (action === "like" || action === "dislike") {
    message.feedback = message.feedback === action ? "" : action;
    renderAiChat({ preserveScroll: true });
    saveAiAssistantDraftState();
    showAppMessage(action === "like" ? "Feedback saved: useful answer." : "Feedback saved: answer needs improvement.", "success", "AI Assistant");
    return;
  }
  if (action === "copy") {
    const copied = await copyTextToClipboard(message.content || "");
    showAppMessage(copied ? "AI answer copied." : "Could not copy this answer.", copied ? "success" : "warning", "AI Assistant");
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = copied ? "AI answer copied." : "Could not copy this answer.";
    return;
  }
  if (action === "export") {
    downloadAiChatMessageAsWord(message, index);
  }
}

function resetAiChatForProject() {
  const project = getAiContextProject();
  activeAiConversationDraftId = "";
  aiActiveProjectId = project?.id || "";
  aiActiveProjectName = project?.name || "";
  aiChatHistory = [{ role: "assistant", content: getAiOpeningMessage(), createdAt: new Date().toISOString() }];
  if (els.aiChatInput) els.aiChatInput.value = "";
  if (els.aiSpokenText) els.aiSpokenText.value = "";
  if (els.aiDraftText) els.aiDraftText.value = "";
  renderAiFeedback([]);
  renderAiChat();
}

function clearAiChatComposer() {
  aiSessionTranscriptBase = "";
  if (els.aiSpokenText) els.aiSpokenText.value = "";
  if (els.aiChatInput) els.aiChatInput.value = "";
}

async function sendAiChatMessage(messageText = els.aiChatInput?.value || "", options = {}) {
  const cleanMessage = cleanAiTranscript(messageText);
  if (!cleanMessage || aiLiveOrganizing) return;
  const sendKey = cleanMessage.toLowerCase();
  if (sendKey === aiLastChatSendKey && Date.now() - aiLastChatSendAt < 8000) {
    clearAiChatComposer();
    saveAiAssistantDraftState();
    return;
  }
  aiLastChatSendKey = sendKey;
  aiLastChatSendAt = Date.now();
  const context = getAiDraftContext();
  const chatHistoryBeforeMessage = aiChatHistory.slice();
  const conversationContext = getAiConversationDraftContext(cleanMessage, chatHistoryBeforeMessage);
  aiChatHistory.push({ role: "user", content: cleanMessage, createdAt: new Date().toISOString() });
  clearAiChatComposer();
  renderAiChat();
  aiLiveOrganizing = true;
  setAiThinking(true, "I am organizing your text now.");
  try {
    const settings = getAiAssistantSettings();
    const mode = options.mode || getAiTypingModeFromMessage(cleanMessage);
    const result = await createAiDraft({
      mode,
    ...
    const assistantMessage = result.draft || "I could not prepare the text.";
    aiActiveProjectId = aiActiveProjectId || context.project?.id || "";
    aiActiveProjectName = aiActiveProjectName || context.project?.name || "";
    const finalAssistantMessage = assistantMessage;
    aiChatHistory.push({ role: "assistant", content: finalAssistantMessage, createdAt: new Date().toISOString(), meta: { source: result.source || "typing-assistant", mode } });
    if (els.aiDraftText) els.aiDraftText.value = finalAssistantMessage;
    renderProjectAssistantMeta({});
    renderAiChat();
    if (options.clearVoiceAfterSend) clearAiChatComposer();
    if (options.speakResponse) speakAiText(finalAssistantMessage);
    saveAiAssistantDraftState();
  } finally {
    aiLiveOrganizing = false;
    setAiThinking(false, "");
  }
}

function formatAiChatTranscript() {
  return aiChatHistory.map((message) => `${message.role === "user" ? "User" : "Assistant"}: ${message.content}`).join("\n\n");
}

function getAiConversationHasUserContent(chatHistory = aiChatHistory) {
  return (chatHistory || []).some((message) => message.role === "user" && cleanAiTranscript(message.content || ""));
}

function getAiConversationPreview(draft = {}) {
  const messages = Array.isArray(draft.chatHistory) ? draft.chatHistory : [];
  const assistantMessage = [...messages].reverse().find((message) => message.role === "assistant" && cleanAiTranscript(message.content || ""));
  const userMessage = messages.find((message) => message.role === "user" && cleanAiTranscript(message.content || ""));
  const text = cleanAiTranscript(assistantMessage?.content || draft.content || userMessage?.content || "");
  return text.length > 170 ? `${text.slice(0, 170).trim()}...` : text;
}

function renderAiConversationDrafts() {
  if (!els.aiConversationDraftList) return;
  const drafts = [...(state.aiConversationDrafts || [])].sort((left, right) => new Date(right.updatedAt || right.createdAt || 0) - new Date(left.updatedAt || left.createdAt || 0));
  if (!drafts.length) {
    els.aiConversationDraftList.innerHTML = `
      <article class="ai-conversation-draft-empty">
        <strong>No AI conversation drafts yet</strong>
        <p class="muted">Start a no-project AI chat, then click Save conversation to keep it here.</p>
      </article>
    `;
    return;
  }
  els.aiConversationDraftList.innerHTML = drafts.map((draft) => {
    const title = draft.title || "AI conversation draft";
    const preview = getAiConversationPreview(draft) || "Saved AI conversation.";
    const savedAt = draft.updatedAt || draft.createdAt || "";
    return `
      <article class="ai-conversation-draft-card">
...
function openAiConversationDraft(draftId) {
  const draft = (state.aiConversationDrafts || []).find((entry) => entry.id === draftId);
  if (!draft) return;
  clearAiAssistantDraftState();
  openAiAssistantDialog(draft.mode || "summary", { projectId: "" });
  activeAiConversationDraftId = draft.id;
  aiChatHistory = (draft.chatHistory || []).map(normalizeAiChatMessage).filter((message) => message.content.trim());
  if (!aiChatHistory.length) {
    aiChatHistory = [{ role: "assistant", content: getAiOpeningMessage(), createdAt: new Date().toISOString() }];
  }
  const lastAssistantMessage = [...aiChatHistory].reverse().find((message) => message.role === "assistant" && cleanAiTranscript(message.content || ""));
  if (els.aiDraftText) els.aiDraftText.value = lastAssistantMessage?.content || draft.content || "";
  aiLastGeneratedDraft = els.aiDraftText?.value || "";
  if (els.aiSpokenText) els.aiSpokenText.value = "";
  if (els.aiChatInput) els.aiChatInput.value = "";
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Loaded saved AI draft. Continue the chat or save it again.";
  renderAiChat();
  saveAiAssistantDraftState();
}

async function deleteAiConversationDraft(draftId) {
  const draft = (state.aiConversationDrafts || []).find((entry) => entry.id === draftId);
  if (!draft) return;
  const confirmed = await showAppConfirm(
    `Delete "${draft.title || "AI conversation draft"}"?`,
    "Delete AI Draft",
    { eyebrow: "AI Assistant", confirmLabel: "Delete", cancelLabel: "Keep", tone: "warning" },
  );
  if (!confirmed) return;
  state.aiConversationDrafts = (state.aiConversationDrafts || []).filter((entry) => entry.id !== draftId);
  if (activeAiConversationDraftId === draftId) activeAiConversationDraftId = "";
  persist();
  renderAiConversationDrafts();
  showAppMessage("AI draft deleted.", "success", "AI Assistant");
}

function onAiConversationDraftListClick(event) {
  const openButton = event.target?.closest?.("[data-ai-draft-open]");
  if (openButton && els.aiConversationDraftList?.contains(openButton)) {
    openAiConversationDraft(openButton.dataset.aiDraftOpen || "");
    return;
  }
  const deleteButton = event.target?.closest?.("[data-ai-draft-delete]");
  if (deleteButton && els.aiConversationDraftList?.contains(deleteButton)) {
    deleteAiConversationDraft(deleteButton.dataset.aiDraftDelete || "");
  }
}

function resetAiAfterConversationSave() {
  clearAiAssistantDraftState();
  activeAiConversationDraftId = "";
  startNewAiSection();
}
...
function saveAiConversation() {
  const project = getAiContextProject();
  if (!getAiConversationHasUserContent()) {
    showAppMessage("There is no conversation to save yet.", "info", "AI Assistant");
    return;
  }
  const content = formatAiChatTranscript();
  if (!project) {
    const now = new Date().toISOString();
    const chatHistory = aiChatHistory.map(normalizeAiChatMessage).filter((message) => message.content.trim());
    const existingIndex = (state.aiConversationDrafts || []).findIndex((entry) => entry.id === activeAiConversationDraftId);
    const existingDraft = existingIndex >= 0 ? state.aiConversationDrafts[existingIndex] : null;
    const draft = normalizeAiConversationDraft({
      ...existingDraft,
      id: existingDraft?.id || crypto.randomUUID(),
      title: getAiConversationTitle(chatHistory),
      content,
      chatHistory,
      mode: els.aiAssistantMode?.value || existingDraft?.mode || "summary",
      language: getAiAssistantSettings().language,
      createdAt: existingDraft?.createdAt || now,
      updatedAt: now,
      createdByUserId: existingDraft?.createdByUserId || state.currentUserId || "",
    });
    if (existingIndex >= 0) {
      state.aiConversationDrafts[existingIndex] = draft;
    } else {
      state.aiConversationDrafts.unshift(draft);
    }
    activeAiConversationDraftId = draft.id;
    logAudit("AI Conversation Draft Saved", {
      objectType: "ai-conversation-draft",
      objectName: draft.title,
    });
    persist();
    renderAiConversationDrafts();
    resetAiAfterConversationSave();
    showAppMessage("AI conversation saved in Drafts of conversation with AI.", "success", "AI Assistant");
    return;
  }
  if (!requirePermission(canWorkInProject(project), "You do not have access to save AI conversations here.")) return;
  const target = getProjectDetailsFolder(project);
  ...
  logAudit("AI Conversation Saved", {
    objectType: "note",
    objectName: "AI assistant conversation",
    projectId: project.id,
  });
  persist();
  render();
  resetAiAfterConversationSave();
  showAppMessage("AI conversation saved to project notes.", "success", "AI Assistant");
}

async function makeAiConversationResume() {
  if (aiChatHistory.length <= 1) {
    showAppMessage("Start a conversation first.", "info", "AI Assistant");
    return;
  }
  await sendAiChatMessage(
    "Make one complete structured summary of the whole conversation so far. Keep all important details from the earlier answers and my later additions. Preserve quantities, dates, names, materials, work scope, risks, tasks, and next steps. Write it as one coherent note and do not invent facts.",
    { mode: "conversation" },
  );
}

function getAiTypingModeFromMessage(message = "") {
  const text = normalizeAreaLookupText(message);
  const compact = text.replace(/\s+/g, "");
  if (
    /\b(material|materials|materiale|materiali|liste|lista|list|order|bedarf|bedarfsliste)\b/.test(text)
    || /\u03c5\u03bb\u03b9\u03ba/.test(compact)
    || /\u03bb\u03b9\u03c3\u03c4/.test(compact)
  ) {
    return "material";
  }
  if (/\b(offer|quote|angebot|offerta|prosfora)\b/.test(text) || /\u03c0\u03c1\u03bf\u03c3\u03c6\u03bf\u03c1/.test(compact)) {
    return "offer";
  }
  if (/\b(conversation|meeting|protocol|minutes|besprechung|riunione)\b/.test(text)) {
    return "conversation";
  }
  return "summary";
}

async function createAiDraft({ mode, transcript, context = getAiDraftContext(), settings = getAiAssistantSettings(), conversationContext = null }) {
  const cleanText = cleanAiTranscript(transcript);
  const draftConversationContext = conversationContext || getAiConversationDraftContext(cleanText);
  try {
    const aiResult = await requestAiSecretaryDraft({
      mode,
      transcript: cleanText,
      context,
      settings,
      conversationContext: draftConversationContext,
    });
    if (aiResult?.draft) {
      return {
        draft: cleanAiReviewLabels(aiResult.draft || ""),
        questions: [],
    ...
    console.warn("AI secretary endpoint unavailable; using local fallback.", error);
    var aiErrorMessage = error?.message || "ChatGPT is not available.";
  }
  return {
    draft: cleanAiReviewLabels(buildLocalConversationAwareDraft({
      mode,
      transcript: cleanText,
      context,
      settings,
    ...
    questions: [],
    source: "local-secretary-fallback",
    errorMessage: aiErrorMessage,
    needsReview: true,
  };
}

function setAiThinking(isThinking, message = "I am organizing the note now.") {
  els.aiThinkingOverlay?.classList.toggle("hidden", !isThinking);
  const label = els.aiThinkingOverlay?.querySelector("strong");
  if (label && message) label.textContent = message;
  if (message) els.aiVoiceStatus.textContent = message;
}

function getAiDraftReadySpeech(mode, settings = getAiAssistantSettings(), result = {}) {
  if (settings.language === "German") {
    const fallbackNote = result.source === "local-secretary-fallback" ? " Ich habe den lokalen Entwurfsassistenten benutzt, weil ChatGPT nicht erreichbar ist." : "";
    if (mode === "conversation") return `Ich habe die Unterhaltung geordnet.${fallbackNote} Die wichtigsten Punkte stehen jetzt in der Notiz. Du kannst weiter sprechen, die Notiz bearbeiten oder speichern.`;
    return `Ich habe die Notiz vorbereitet.${fallbackNote} Bitte prüfe sie kurz. Du kannst weiter sprechen, sie bearbeiten oder speichern.`;
  }
  if (settings.language === "Greek") {
    const fallbackNote = result.source === "local-secretary-fallback" ? " Χρησιμοποίησα τον τοπικό βοηθό, γιατί το ChatGPT δεν είναι διαθέσιμο." : "";
    if (mode === "conversation") return `Οργάνωσα τη συζήτηση.${fallbackNote} Τα βασικά σημεία είναι τώρα στη σημείωση. Μπορείς να συνεχίσεις, να τη διορθώσεις ή να την αποθηκεύσεις.`;
    return `Ετοίμασα τη σημείωση.${fallbackNote} Έλεγξέ τη λίγο. Μπορείς να συνεχίσεις, να τη διορθώσεις ή να την αποθηκεύσεις.`;
  }
  if (settings.language === "Italian") {
    const fallbackNote = result.source === "local-secretary-fallback" ? " Ho usato l'assistente locale perché ChatGPT non è disponibile." : "";
    if (mode === "conversation") return `Ho organizzato la conversazione.${fallbackNote} I punti principali sono nella nota. Puoi continuare, modificare o salvare.`;
    return `Ho preparato la nota.${fallbackNote} Controllala un momento. Puoi continuare, modificare o salvare.`;
  }
  const fallbackNote = result.source === "local-secretary-fallback"
    ? " I used the local draft assistant because ChatGPT is not available."
    : "";
  if (mode === "conversation") return `I organized the conversation.${fallbackNote} The main points are now in the note. You can keep talking, edit it, or save it.`;
  return `I prepared the note.${fallbackNote} Please check it once. You can keep talking, edit it, or save it.`;
}

function getAiSpokenDraftExcerpt(draft = "") {
  const lines = String(draft || "")
    .split(/\n+/)
    .map((line) => line.replace(/^[-*]\s*/, "").trim())
    .filter((line) => line && !/^project:/i.test(line) && !/^assistant note/i.test(line));
  const useful = lines.filter((line) => !/^(what happened|what needs to be done|cause|decisions|tasks|open questions|next steps|project summary draft|conversation summary draft)$/i.test(line));
  return useful.slice(0, 4).join(". ").replace(/\.\s*\./g, ".").slice(0, 700);
}

function getAiProjectReviewSpeech(draft, settings = getAiAssistantSettings(), result = {}) {
  const excerpt = getAiSpokenDraftExcerpt(draft);
  const fallbackNote = result.source === "local-secretary-fallback"
    ? " I used the local assistant because ChatGPT is not available."
    : "";
  if (settings.language === "German") {
    return `Ich habe die gespeicherten Projektnotizen geprüft.${fallbackNote} Kurz gesagt: ${excerpt || "ich habe noch nicht genug gespeicherte Notizen gefunden, um den Projektstatus sicher zusammenzufassen."}`;
  }
  if (settings.language === "Greek") {
    return `Έλεγξα τις αποθηκευμένες σημειώσεις του έργου.${fallbackNote} Με λίγα λόγια: ${excerpt || "δεν βρήκα ακόμη αρκετές αποθηκευμένες σημειώσεις για ασφαλή περίληψη."}`;
  }
  if (settings.language === "Italian") {
    return `Ho controllato le note salvate del progetto.${fallbackNote} In breve: ${excerpt || "non ho ancora trovato abbastanza note salvate per riassumere bene lo stato del progetto."}`;
  }
  return `I checked the saved project notes.${fallbackNote} Short version: ${excerpt || "I do not have enough saved notes yet to summarize the project status properly."}`;
}

async function updateAiDraftFromSpeech(options = {}) {
  if (aiLiveOrganizing) return;
  const settings = getAiAssistantSettings();
  if (!getAiContextProject()) {
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = getAiFriendlyPrompt(els.aiAssistantMode?.value || "summary", settings);
    return;
  }
  aiQuestionRoundComplete = false;
  aiLiveOrganizing = true;
  setAiThinking(true, "I am organizing your text now.");
  try {
    const question = els.aiSpokenText?.value || "";
    const cleanedQuestion = cleanAiTranscript(question);
    if (!cleanedQuestion) {
      if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Write or speak something first.";
      aiLiveOrganizing = false;
      return;
    }
    if (options.keepListening && cleanedQuestion && cleanedQuestion === aiLastOrganizedTranscript) {
      aiLiveOrganizing = false;
      return;
    }
    const context = getAiDraftContext();
    ...
    if (els.aiDraftText) els.aiDraftText.value = result.answer;
    aiLastGeneratedDraft = result.answer;
    aiLastOrganizedTranscript = cleanedQuestion;
    renderProjectAssistantMeta(result);
    if (els.aiVoiceStatus) {
      const sourceNote = result.source === "local-project-fallback"
        ? ` I used the local fallback because ChatGPT is not available${result.errorMessage ? `: ${result.errorMessage}` : "."}`
        : "";
      els.aiVoiceStatus.textContent = `${result.answer}${sourceNote}`;
    }
    saveAiAssistantDraftState();
    speakAiText(result.answer, () => {
      aiLiveOrganizing = false;
      if (options.keepListening) {
        aiSessionTranscriptBase = "";
        if (els.aiSpokenText) els.aiSpokenText.value = "";
        aiLastOrganizedTranscript = "";
      }
      if (options.keepListening && aiListening && els.aiVoiceStatus) {
        els.aiVoiceStatus.textContent = "I am listening again.";
      }
    });
  } finally {
    setAiThinking(false, "");
...
function getAiSpokenPrompt(mode, settings = getAiAssistantSettings()) {
  return getAiFriendlyPrompt(mode, settings);
}

function getAiSimpleSpokenWelcome(settings = getAiAssistantSettings()) {
  if (settings.language === "German") return "Hallo, ich höre dich.";
  if (settings.language === "Greek") return "Γεια, σε ακούω.";
  if (settings.language === "Italian") return "Ciao, ti ascolto.";
  return "Hello, I hear you.";
}

function setAiAssistantMode(mode, options = {}) {
  const nextMode = ["offer", "summary", "material", "conversation"].includes(mode) ? mode : "summary";
  const settings = getAiAssistantSettings();
  if (els.aiAssistantMode) els.aiAssistantMode.value = nextMode;
  for (const button of els.aiModeChoices || []) {
    const active = button.dataset.aiModeChoice === nextMode;
    button.classList.toggle("active", active);
    button.setAttribute("aria-pressed", String(active));
  }
  if (els.aiAssistantDialogTitle) els.aiAssistantDialogTitle.textContent = getAiModeLabel(nextMode, settings);
  if (els.aiVoiceStatus && !options.keepStatus) els.aiVoiceStatus.textContent = getAiFriendlyPrompt(nextMode, settings);
  if (els.aiSpokenText && !options.keepSpokenText) els.aiSpokenText.value = "";
  if (!options.keepSpokenText) renderAiFeedback([]);
  if (els.aiDraftText && !options.keepDraftText) els.aiDraftText.value = getAiDraftTemplate(nextMode, settings);
  syncAiContextAvailability();
}

function speakAiAssistant() {
  const mode = els.aiAssistantMode?.value || "summary";
  const text = getAiSpokenPrompt(mode);
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = text;
  if (!("speechSynthesis" in window) || typeof SpeechSynthesisUtterance === "undefined") {
    showAppMessage("Speech output is not available in this browser.", "warning", "AI Assistant");
    return;
  }
  window.speechSynthesis.cancel();
  const utterance = new SpeechSynthesisUtterance(text);
  utterance.lang = ({ German: "de-DE", Greek: "el-GR", Italian: "it-IT" }[getAiAssistantSettings().language]) || "en-US";
  window.speechSynthesis.speak(utterance);
}

function setAiSpeechControlState(state = "idle") {
  aiSpeechState = state;
  const isSpeaking = state === "speaking";
  const isPaused = state === "paused";
  els.aiReadPlayBtn?.classList.toggle("active", isSpeaking || isPaused);
  els.aiReadPauseBtn?.classList.toggle("active", isPaused);
  els.aiReadStopBtn?.classList.toggle("active", isSpeaking || isPaused);
  if (els.aiReadPlayBtn) els.aiReadPlayBtn.textContent = isPaused ? "Resume" : "Play";
  if (els.aiReadPauseBtn) els.aiReadPauseBtn.disabled = !isSpeaking;
  if (els.aiReadStopBtn) els.aiReadStopBtn.disabled = !(isSpeaking || isPaused);
}

function getLatestAiAssistantText() {
  const lastAssistantMessage = [...aiChatHistory].reverse()
    .find((message) => message.role === "assistant" && cleanAiTranscript(message.content || ""));
  return cleanAiTranscript(lastAssistantMessage?.content || els.aiDraftText?.value || "");
}

function playLatestAiAnswer() {
  const text = getLatestAiAssistantText();
  if (!text) {
    showAppMessage("There is no AI answer to read yet.", "info", "AI Assistant");
    return;
  }
  if (window.speechSynthesis?.paused && aiSpeechCurrentText === text) {
    window.speechSynthesis.resume();
    setAiSpeechControlState("speaking");
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Reading resumed.";
    return;
  }
  speakAiText(text);
}
...
function pauseAiSpeech() {
  ...
  setAiSpeechControlState("paused");
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Reading paused.";
}

function stopAiSpeech() {
  if (!window.speechSynthesis) return;
  aiSpeechManualStopRequested = true;
  aiSpeechSequence += 1;
  window.speechSynthesis.cancel();
  aiSpeechCurrentText = "";
  setAiSpeechControlState("idle");
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Reading stopped.";
}

function speakAiText(text, onEnd) {
  if (!("speechSynthesis" in window) || typeof SpeechSynthesisUtterance === "undefined") {
    onEnd?.();
    return;
  }
  const shouldResumeRecognition = Boolean(aiListening && aiRecognition);
  if (shouldResumeRecognition) {
    aiSpeechPausedRecognition = true;
    try {
      aiRecognition.stop();
    } catch (error) {
      // Browser recognition can throw if it already stopped itself.
  ...
  aiSpeechManualStopRequested = false;
  const speechSequence = aiSpeechSequence + 1;
  aiSpeechSequence = speechSequence;
  aiSpeechCurrentText = String(text || "");
  setAiSpeechControlState("speaking");
  const utterance = new SpeechSynthesisUtterance(text);
  utterance.lang = ({ German: "de-DE", Greek: "el-GR", Italian: "it-IT" }[getAiAssistantSettings().language]) || "en-US";
  const finishSpeech = () => {
    if (speechSequence !== aiSpeechSequence) return;
    const wasStopped = aiSpeechManualStopRequested;
    aiSpeechManualStopRequested = false;
    aiSpeechCurrentText = "";
    setAiSpeechControlState("idle");
    if (!wasStopped) onEnd?.();
    aiSpeechPausedRecognition = false;
    if (shouldResumeRecognition) {
      aiManualStopRequested = true;
      aiListening = false;
      hideAiListeningCue();
      if (els.aiVoiceTalkBtn) {
        els.aiVoiceTalkBtn.innerHTML = "&#127908;";
        els.aiVoiceTalkBtn.classList.remove("listening");
        els.aiVoiceTalkBtn.setAttribute("aria-label", "Start AI microphone");
      }
      if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = wasStopped ? "Reading stopped." : "Finished reading. Press the microphone when you want to speak again.";
    } else if (els.aiVoiceStatus && !wasStopped) {
      els.aiVoiceStatus.textContent = "Finished reading.";
    }
  };
...
function getAiRecognitionLanguage() {
  return ({ German: "de-DE", Greek: "el-GR", Italian: "it-IT" }[getAiAssistantSettings().language]) || "en-US";
}

function showAiListeningCue() {
  els.aiListeningCue?.classList.remove("hidden");
}

function hideAiListeningCue() {
  els.aiListeningCue?.classList.add("hidden");
}

function playAiListeningSound() {
  try {
    ...
    secondTone.frequency.setValueAtTime(1180, startAt + 0.11);
    ...
    firstTone.stop(startAt + 0.13);
    secondTone.start(startAt + 0.11);
    secondTone.stop(startAt + 0.29);
    window.setTimeout(() => audioContext.close?.(), 420);
  } catch (error) {
    // Audio cues are optional; visual cue still appears.
  }
}

function playAiListeningCue() {
  showAiListeningCue();
  playAiListeningSound();
}

function setAiListeningActionState(active) {
  els.aiAnswerNowBtn?.classList.toggle("active", Boolean(active));
  els.aiKeepListeningBtn?.classList.toggle("active", Boolean(active));
}

function clearAiSilenceCountdown() {
  window.clearInterval(aiSilenceCountdownTimer);
  aiSilenceCountdownTimer = null;
  aiSilenceCountdownRemaining = 0;
  setAiListeningActionState(false);
}

function startAiSilenceCountdown() {
  if (aiLiveOrganizing) return;
  clearAiSilenceCountdown();
  aiSilenceCountdownRemaining = getAiAnswerDelaySeconds();
  setAiListeningActionState(true);
  if (els.aiVoiceStatus) {
    els.aiVoiceStatus.textContent = `I will send your message in ${aiSilenceCountdownRemaining} seconds. Keep talking if you need more time.`;
  }
  aiSilenceCountdownTimer = window.setInterval(() => {
    aiSilenceCountdownRemaining -= 1;
    if (aiSilenceCountdownRemaining <= 0) {
      clearAiSilenceCountdown();
      finishAiListeningAndOrganize({ keepListening: true });
      return;
    }
    if (els.aiVoiceStatus) {
      els.aiVoiceStatus.textContent = `I will send your message in ${aiSilenceCountdownRemaining} seconds. Keep talking if you need more time.`;
...
function keepAiListening() {
  clearAiSilenceCountdown();
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I am listening. Take your time.";
}

function finishAiListeningAndOrganize(options = {}) {
  clearAiSilenceCountdown();
  aiManualStopRequested = true;
  aiListening = false;
  hideAiListeningCue();
  ...
  if ((els.aiSpokenText?.value || "").trim()) {
    sendAiChatMessage(els.aiSpokenText?.value || "", { speakResponse: true, clearVoiceAfterSend: true });
  }
}

function stopAiListening(reason = "Stopped listening.") {
  if (aiQuestionSession?.active) {
    finishCurrentAiQuestionAnswer("next");
    return;
  }
  clearAiSilenceCountdown();
  ...
  hideAiListeningCue();
  if (els.aiVoiceTalkBtn) {
    els.aiVoiceTalkBtn.textContent = "🎙";
    els.aiVoiceTalkBtn.textContent = "Mic";
    els.aiVoiceTalkBtn.innerHTML = "&#127908;";
    els.aiVoiceTalkBtn.classList.remove("listening");
    els.aiVoiceTalkBtn.setAttribute("aria-label", "Start AI microphone");
  }
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = reason;
  if ((els.aiSpokenText?.value || "").trim()) updateAiDraftFromSpeech();
  try {
    aiRecognition?.stop?.();
  } catch (error) {
    // Browser recognition can throw if it already stopped itself.
...
function startAiRecognition() {
  const Recognition = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!Recognition) {
    showAppMessage("Speech recognition is not available in this browser. You can still type the spoken text manually.", "warning", "AI Assistant");
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Speech recognition is not available in this browser.";
    hideAiListeningCue();
    return;
  }
  const settings = getAiAssistantSettings();
  ...
  aiRecognition.continuous = true;
  aiRecognition.interimResults = true;
  aiSessionTranscriptBase = cleanAiTranscript(els.aiSpokenText?.value || "");
  aiManualStopRequested = false;
  aiListening = true;
  clearAiSilenceCountdown();
  if (els.aiVoiceTalkBtn) {
    els.aiVoiceTalkBtn.textContent = "■";
    els.aiVoiceTalkBtn.textContent = "Stop";
    els.aiVoiceTalkBtn.classList.add("listening");
    els.aiVoiceTalkBtn.setAttribute("aria-label", "Stop AI microphone");
  }
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I am listening. Pause when finished; I will send your message.";
  aiRecognition.onresult = (event) => {
    if (!aiListening || aiManualStopRequested || aiLiveOrganizing) return;
    let transcript = "";
    for (let index = 0; index < event.results.length; index += 1) {
      transcript += event.results[index][0]?.transcript || "";
    }
    const nextTranscript = [aiSessionTranscriptBase, transcript.trim()].filter(Boolean).join(" ");
    if (els.aiSpokenText) els.aiSpokenText.value = nextTranscript;
    if (els.aiChatInput) els.aiChatInput.value = nextTranscript;
    saveAiAssistantDraftState();
    renderAiFeedback([], getAiAssistantSettings().language);
    if (nextTranscript.trim() && !aiLiveOrganizing) startAiSilenceCountdown();
  };
  aiRecognition.onerror = (event) => {
    if (aiListening && !aiManualStopRequested && ["no-speech", "network"].includes(event?.error || "")) {
      if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I am still listening. Pause when finished, or press Stop to organize now.";
      return;
    }
    stopAiListening("Microphone listening stopped.");
  };
  aiRecognition.onend = () => {
    if (aiSpeechPausedRecognition) return;
    if (aiListening && !aiManualStopRequested) {
      if (!aiSilenceCountdownTimer && els.aiVoiceStatus) {
        els.aiVoiceStatus.textContent = "I am still listening. Pause when finished, or press Stop to organize now.";
      }
      window.setTimeout(() => {
        try {
          aiRecognition?.start?.();
        } catch (error) {
          stopAiListening("Listening ended.");
        }
      }, 250);
    }
  };
  try {
    ...
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Microphone could not start. Please check browser permission.";
    showAppMessage("Microphone could not start. Please check browser permission.", "warning", "AI Assistant");
  }
}

function onAiMicrophoneClick() {
  if (aiListening) {
    if ((els.aiSpokenText?.value || "").trim()) {
      finishAiListeningAndOrganize();
    } else {
      stopAiListening("Stopped by user.");
    }
    return;
  }
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I am listening.";
  startAiRecognition();
}

function startAiDialog() {
  const mode = els.aiAssistantMode?.value || "summary";
  aiDialogActive = true;
  aiDialogAskedQuestions = false;
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I am listening.";
  if (!aiListening && els.aiAssistantDialog?.open) startAiRecognition();
}

function openAiAssistantDialog(mode = "summary", options = {}) {
  const settings = getAiAssistantSettings();
  const isSettings = options.settings === true;
  els.aiAssistantDialogTitle && (els.aiAssistantDialogTitle.textContent = isSettings ? (isAiGreek(settings) ? "Ρυθμίσεις AI Assistant" : "AI Assistant Settings") : getAiModeLabel(mode, settings));
  els.aiDraftSection?.classList.toggle("hidden", isSettings);
  els.aiSettingsSection?.classList.toggle("hidden", !isSettings);
  els.saveAiSettingsBtn?.classList.toggle("hidden", !isSettings);
  els.saveAiDraftBtn?.classList.toggle("hidden", isSettings);
  if (els.aiSaveTarget) els.aiSaveTarget.value = options.target || settings.defaultTarget || "project-notes";
  const requestedProjectId = Object.prototype.hasOwnProperty.call(options, "projectId") ? options.projectId : state.selectedProjectId;
  populateAiContext(requestedProjectId);
  setAiAssistantMode(mode);
  const restoredDraft = !isSettings && restoreAiAssistantDraftState(requestedProjectId);
  if (restoredDraft) {
    // Restored content intentionally wins over the default template.
  }
  syncAiAssistantControls(settings);
  syncAiContextAvailability();
  if (!restoredDraft) resetAiChatForProject();
  renderAiLearningList();
  els.aiMinimizedBar?.classList.add("hidden");
  if (!els.aiAssistantDialog?.open) els.aiAssistantDialog?.showModal();
}

function minimizeAiAssistantDialog() {
  saveAiAssistantDraftState();
  if (aiListening) stopAiListening("Assistant minimized.");
  stopAiSpeech();
  els.aiAssistantDialog?.close();
  els.aiMinimizedBar?.classList.remove("hidden");
}

function restoreMinimizedAiAssistant() {
  els.aiMinimizedBar?.classList.add("hidden");
  if (!els.aiAssistantDialog?.open) els.aiAssistantDialog?.showModal();
  renderAiLearningList();
}

async function confirmAiAssistantClose() {
  return showAppConfirm(
    "You are about to leave the AI Assistant. Save or export anything important first. Leave now?",
    "Leave AI Assistant",
    { eyebrow: "AI Assistant", confirmLabel: "Leave", cancelLabel: "Stay", tone: "warning" },
  );
}

async function closeAiAssistantDialog(options = {}) {
  if (options.confirm !== false) {
    const confirmed = await confirmAiAssistantClose();
    if (!confirmed) return;
  }
  const shouldDiscardDraft = options.preserveDraft === false;
  aiSkipDraftSaveOnClose = shouldDiscardDraft;
  ...
  stopAiSpeech();
  aiDialogActive = false;
  aiDialogAskedQuestions = false;
  els.aiAssistantDialog?.close();
  els.aiMinimizedBar?.classList.add("hidden");
}

function cancelAiAssistantDialog() {
  closeAiAssistantDialog({ preserveDraft: false });
}

function startNewAiSection() {
  if (aiListening) stopAiListening("Stopped listening.");
  stopAiSpeech();
  aiDialogActive = false;
  aiDialogAskedQuestions = false;
  aiOpenQuestions = [];
  ...
  aiLastQuestionAsked = "";
  if (els.aiSpokenText) els.aiSpokenText.value = "";
  if (els.aiDraftText) els.aiDraftText.value = getAiDraftTemplate(els.aiAssistantMode?.value || "summary", getAiAssistantSettings());
  aiLastGeneratedDraft = els.aiDraftText?.value || "";
  renderAiFeedback([]);
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = getAiFriendlyPrompt(els.aiAssistantMode?.value || "summary");
  clearAiAssistantDraftState();
  resetAiChatForProject();
}

function onAiSettingsSave() {
  const language = els.aiLanguage?.value || "English";
  saveAiAssistantSettings({
    language,
    defaultTarget: els.aiDefaultTarget?.value || "project-notes",
  });
  syncAiAssistantSummary();
  syncAiAssistantControls();
  closeAiAssistantDialog({ confirm: false });
}

function getAiDraftTarget(project, targetKey) {
  if (!project) return null;
  if (targetKey === "area-notes" || targetKey === "material-list") {
    const area = getAiContextArea(project);
    if (area) return area;
  }
...
function onAiDraftSave(event) {
  event.preventDefault();
  saveAiConversation();
}

function readAiDraftAloud() {
  const text = (els.aiDraftText?.value || "").trim();
  if (!text) {
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Create a draft first, then I can read it back to you.";
    return;
  }
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "I am reading the draft aloud. Listen for anything that sounds wrong.";
  speakAiText(text, () => {
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Finished reading. You can add more, edit, or save.";
  });
}

function slugifyFileName(value, fallback = "ai-note") {
...
function buildAiWordHtml({ text, title, context }) {
  const htmlBody = String(text || "")
    .split(/\n{2,}/)
...
<head>
  ...
  <style>
    body { font-family: Calibri, Arial, sans-serif; font-size: 12pt; line-height: 1.45; color: #1f2a2a; }
    h1 { font-size: 18pt; margin-bottom: 6pt; }
    .meta { color: #667; margin-bottom: 16pt; }
    p { margin: 0 0 10pt; }
  </style>
</head>
<body>
...
function downloadAiTextAsWord({ text, title, context, fileName }) {
  const html = buildAiWordHtml({ text, title, context });
  const blob = new Blob(["\ufeff", html], { type: "application/msword;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = fileName || `${slugifyFileName(context)}-${slugifyFileName(title)}.doc`;
...
function downloadAiChatMessageAsWord(message, index = 0) {
  const text = String(message?.content || "").trim();
  if (!text) {
    showAppMessage("There is no AI answer to export.", "warning", "AI Assistant");
    return;
  }
  const context = getAiContextLabel();
  const title = "AI answer";
  downloadAiTextAsWord({
    text,
    title,
    context,
    fileName: `${slugifyFileName(context)}-${slugifyFileName(title)}-${index + 1}.doc`,
  });
  showAppMessage("AI answer exported to Word.", "success", "AI Assistant");
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "AI answer exported to Word.";
}

function downloadAiDraftAsWord() {
  const text = (els.aiDraftText?.value || "").trim();
  if (!text) {
    if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Create or edit a note first, then I can download it as a Word document.";
    return;
  }
  const context = getAiContextLabel();
  const title = getAiModeLabel(els.aiAssistantMode?.value || "summary");
  downloadAiTextAsWord({
    text,
    title,
    context,
    fileName: `${slugifyFileName(context)}-${slugifyFileName(title)}.doc`,
  });
  if (els.aiVoiceStatus) els.aiVoiceStatus.textContent = "Word document downloaded.";
  saveAiAssistantDraftState();
}
...
function bindEvents() {
  ...
  els.projectAiBtn?.addEventListener("click", () => openAiAssistantDialog("offer", { target: "offer-draft", projectId: state.selectedProjectId || "" }));
  els.workspaceAiVoiceBtn?.addEventListener("click", () => openAiAssistantDialog("material", { target: "material-list", projectId: state.selectedProjectId || "" }));
  els.aiFloatingBtn?.addEventListener("click", () => openAiAssistantDialog("summary", { projectId: "" }));
  els.aiAssistantNewDraftBtn?.addEventListener("click", () => openAiAssistantDialog("summary", { projectId: "" }));
  els.aiNewUnassignedChatBtn?.addEventListener("click", () => openAiAssistantDialog("summary", { projectId: "" }));
  els.aiAssistantSettingsBtn?.addEventListener("click", () => openAiAssistantDialog("summary", { settings: true }));
  els.closeAiAssistantBtn?.addEventListener("click", closeAiAssistantDialog);
  els.minimizeAiAssistantBtn?.addEventListener("click", minimizeAiAssistantDialog);
  els.aiMinimizedBar?.addEventListener("click", restoreMinimizedAiAssistant);
  els.cancelAiAssistantBtn?.addEventListener("click", cancelAiAssistantDialog);
  els.aiStartNewSectionBtn?.addEventListener("click", startNewAiSection);
  els.aiLearnAddBtn?.addEventListener("click", onAiLearnAdd);
  els.aiLearningList?.addEventListener("click", onAiLearningListClick);
  els.saveAiSettingsBtn?.addEventListener("click", onAiSettingsSave);
  els.aiAssistantForm?.addEventListener("submit", onAiDraftSave);
  els.aiAssistantDialog?.addEventListener("cancel", (event) => {
    event.preventDefault();
    saveAiAssistantDraftState();
  });
  els.aiAssistantDialog?.addEventListener("close", () => {
    if (aiSkipDraftSaveOnClose) {
      aiSkipDraftSaveOnClose = false;
      return;
    }
    saveAiAssistantDraftState();
  });
  els.aiFeedbackPanel?.addEventListener("click", onAiFeedbackClick);
  els.aiStartDialogBtn?.addEventListener("click", startAiDialog);
  els.aiVoiceTalkBtn?.addEventListener("click", onAiMicrophoneClick);
  els.aiAnswerNowBtn?.addEventListener("click", finishAiListeningAndOrganize);
  els.aiKeepListeningBtn?.addEventListener("click", keepAiListening);
  els.aiSendMessageBtn?.addEventListener("click", () => sendAiChatMessage());
  els.aiChatLog?.addEventListener("click", onAiChatActionClick);
  els.aiConversationDraftList?.addEventListener("click", onAiConversationDraftListClick);
  els.aiReadPlayBtn?.addEventListener("click", playLatestAiAnswer);
  els.aiReadPauseBtn?.addEventListener("click", pauseAiSpeech);
  els.aiReadStopBtn?.addEventListener("click", stopAiSpeech);
  ...
  els.aiCreateDraftBtn?.addEventListener("click", updateAiDraftFromSpeech);
  els.aiReadDraftBtn?.addEventListener("click", makeAiConversationResume);
  els.aiReadNoteBottomBtn?.addEventListener("click", makeAiConversationResume);
  els.aiDownloadWordBtn?.addEventListener("click", downloadAiDraftAsWord);
  els.areaBulkAddRowBtn?.addEventListener("click", () => createAreaBulkRow());
  els.areaBulkCreateBtn?.addEventListener("click", createBulkAreasFromTable);
  els.areaBulkRows?.addEventListener("click", onAreaBulkRowsClick);
  els.aiSpokenText?.addEventListener("input", saveAiAssistantDraftState);
  els.aiDraftText?.addEventListener("input", saveAiAssistantDraftState);
  els.aiSaveTarget?.addEventListener("change", saveAiAssistantDraftState);
  els.aiContextProject?.addEventListener("change", () => {
    populateAiContext(els.aiContextProject?.value || "");
    setAiAssistantMode(els.aiAssistantMode?.value || "summary", { keepSpokenText: true });
    syncAiContextAvailability();
    resetAiChatForProject();
    saveAiAssistantDraftState();
  });
  els.aiContextArea?.addEventListener("change", () => {
    setAiAssistantMode(els.aiAssistantMode?.value || "summary", { keepSpokenText: true });
    syncAiContextAvailability();
    saveAiAssistantDraftState();
  });
  for (const button of els.aiModeChoices || []) {
    button.addEventListener("click", () => {
      setAiAssistantMode(button.dataset.aiModeChoice || "summary");
      saveAiAssistantDraftState();
    });
  }
  els.aiQuickLanguage?.addEventListener("change", saveAiQuickSettings);
  els.aiAnswerDelaySeconds?.addEventListener("change", saveAiQuickSettings);
  els.aiQuickCustomPause?.addEventListener("change", () => {
    saveAiQuickSettings();
  });
  els.aiQuickEndWord?.addEventListener("change", saveAiQuickSettings);
  els.aiAssistantMode?.addEventListener("change", () => {
    const mode = els.aiAssistantMode?.value || "summary";
    setAiAssistantMode(mode);
    saveAiAssistantDraftState();
  });
  document.querySelectorAll("[data-ai-mode-start]").forEach((button) => {
    button.addEventListener("click", () => openAiAssistantDialog(button.dataset.aiModeStart || "summary", { projectId: "" }));
  });
  syncAiAssistantSummary();
  setAiSpeechControlState("idle");
  ...
  els.projectClientSelect?.addEventListener("change", () => {
    syncProjectContactOverrideFields();
    refreshDraftProjectLockState();
  });
  els.projectAddressDifferent?.addEventListener("change", () => syncProjectContactOverrideFields());
  els.projectPhoneDifferent?.addEventListener("change", () => syncProjectContactOverrideFields());
...
function onBeforeUnload(event) {
  if (!getDirtyTrackedForms().length && !hasAiAssistantDraftContent()) return;
  if (hasAiAssistantDraftContent()) saveAiAssistantDraftState();
  event.preventDefault();
  event.returnValue = "";
}
...
function createBlankProject() {
  return {
    ...
    addressDiffersFromClient: false,
    phoneDiffersFromClient: false,
    address: "",
    phone: "",
...
function onProjectSave(event) {
  ...
  project.addressDiffersFromClient = Boolean(els.projectAddressDifferent?.checked);
  project.phoneDiffersFromClient = Boolean(els.projectPhoneDifferent?.checked);
  project.address = project.addressDiffersFromClient ? String(els.projectAddress?.value || "").trim() : "";
  project.phone = project.phoneDiffersFromClient ? normalizePhoneValue(els.projectPhone?.value || "") : "";
...
function openAreaDialog(areaId = null) {
  ...
  els.areaForm?.classList.toggle("area-bulk-create-mode", !isEditing);
  els.areaForm?.classList.toggle("area-edit-single-mode", isEditing);
  if (els.areaDialogTitle) els.areaDialogTitle.textContent = isEditing ? "Edit area" : "Create areas";
  if (els.areaName) {
    els.areaName.disabled = !isEditing;
    els.areaName.required = isEditing;
  }
  if (els.areaFloor) els.areaFloor.disabled = !isEditing;
  els.areaName.value = area?.name || "";
  populateAreaFloorOptions(project, area?.floor || "");
  setAreaIcon(area?.iconKey || "none");
  resetAreaBulkRows();
  document.querySelector("#area-bulk-section")?.setAttribute("open", "");
...
function closeAreaDialog(forceClose = false) {
  ...
  editingAreaId = null;
  els.areaForm?.classList.remove("area-bulk-create-mode", "area-edit-single-mode");
  if (els.areaName) {
    els.areaName.disabled = false;
    els.areaName.required = true;
  }
  if (els.areaFloor) els.areaFloor.disabled = false;
  els.areaForm?.reset();
  setAreaIcon("none");
  if (els.areaFloor) els.areaFloor.innerHTML = "";
  if (els.areaBulkRows) els.areaBulkRows.innerHTML = "";
  rememberFormSnapshot("area", els.areaForm);
  if (els.areaDialog?.open) els.areaDialog.close();
}

function inferAreaIconFromName(name = "") {
  const type = inferProjectAreaRoomType(name);
  if (type === "bathroom") return "bathroom";
  if (type === "kitchen") return "kitchen";
  if (type === "wc") return "bathroom";
  const compact = normalizeAreaLookupText(name).replace(/\s+/g, "");
  if (/balconi|balkoni|balcony|mpalkoni/.test(compact)) return "balcony";
  if (/bedroom|domatio/.test(compact)) return "bedroom";
  return "none";
}

function createAreaBulkRow(defaults = {}) {
  if (!els.areaBulkRows) return;
  const row = document.createElement("tr");
  const iconKey = normalizeAreaIconKey(defaults.iconKey || inferAreaIconFromName(defaults.name || ""));
  row.innerHTML = `
    <td><input class="area-bulk-name" type="text" value="${escapeHtml(defaults.name || "")}" placeholder="name"></td>
    <td><input class="area-bulk-floor" type="text" value="${escapeHtml(defaults.floor || "")}" placeholder="1"></td>
    <td class="area-bulk-icon-cell">
      <input class="area-bulk-icon" type="hidden" value="${escapeHtml(iconKey)}">
      <button class="area-bulk-icon-btn" type="button" aria-label="Choose area icon"></button>
      <div class="area-bulk-icon-menu hidden" role="menu">
        ${AREA_ICON_PRESETS.map((preset) => `<button class="area-bulk-icon-option" type="button" data-icon-key="${preset.key}" title="${escapeHtml(preset.label)}">${buildAreaIconMarkup(preset.key, preset.label)}<span>${escapeHtml(preset.label)}</span></button>`).join("")}
      </div>
    </td>
  `;
  updateAreaBulkIconButton(row);
  row.querySelector(".area-bulk-name")?.addEventListener("input", () => {
    const input = row.querySelector(".area-bulk-icon");
    if (input && input.value === "none") {
      input.value = inferAreaIconFromName(row.querySelector(".area-bulk-name")?.value || "");
      updateAreaBulkIconButton(row);
    }
  });
  els.areaBulkRows.append(row);
}

function updateAreaBulkIconButton(row) {
  const iconInput = row.querySelector(".area-bulk-icon");
  const button = row.querySelector(".area-bulk-icon-btn");
  const key = normalizeAreaIconKey(iconInput?.value || "none");
  const preset = AREA_ICON_PRESETS.find((entry) => entry.key === key) || AREA_ICON_PRESETS[0];
  if (!button) return;
  button.innerHTML = `${buildAreaIconMarkup(preset.key, preset.label)}<span>${escapeHtml(preset.label)}</span>`;
}

function closeAreaBulkIconMenus(exceptMenu = null) {
  els.areaBulkRows?.querySelectorAll(".area-bulk-icon-menu").forEach((menu) => {
    if (menu !== exceptMenu) menu.classList.add("hidden");
  });
}

function onAreaBulkRowsClick(event) {
  const iconButton = event.target.closest?.(".area-bulk-icon-btn");
  if (iconButton) {
    const menu = iconButton.closest(".area-bulk-icon-cell")?.querySelector(".area-bulk-icon-menu");
    if (!menu) return;
    const willOpen = menu.classList.contains("hidden");
    closeAreaBulkIconMenus(menu);
    menu.classList.toggle("hidden", !willOpen);
    return;
  }
  const option = event.target.closest?.(".area-bulk-icon-option");
  if (option) {
    const row = option.closest("tr");
    const input = row?.querySelector(".area-bulk-icon");
    if (input) input.value = normalizeAreaIconKey(option.dataset.iconKey || "none");
    if (row) updateAreaBulkIconButton(row);
    closeAreaBulkIconMenus();
  }
}

function resetAreaBulkRows() {
  if (!els.areaBulkRows) return;
  els.areaBulkRows.innerHTML = "";
  const examples = [
    { name: "Mpanio", floor: "1", iconKey: "bathroom" },
    { name: "mpanio2", floor: "1", iconKey: "bathroom" },
    { name: "wc", floor: "1", iconKey: "bathroom" },
    { name: "Mpanio mikro", floor: "2", iconKey: "bathroom" },
    { name: "mpani piso", floor: "2", iconKey: "bathroom" },
    { name: "kouzina", floor: "2", iconKey: "kitchen" },
  ];
  for (const row of examples) createAreaBulkRow(row);
}

function createBulkAreasFromTable() {
  const project = getCurrentProject();
  if (!project || !els.areaBulkRows) return;
  if (!requirePermission(canManageProject(project), "Only admins or assigned managers can create Areas.")) return;
  const rows = Array.from(els.areaBulkRows.querySelectorAll("tr"));
  const entries = rows.map((row) => ({
    name: row.querySelector(".area-bulk-name")?.value.trim() || "",
    floor: normalizeFloorName(row.querySelector(".area-bulk-floor")?.value || ""),
    iconKey: normalizeAreaIconKey(row.querySelector(".area-bulk-icon")?.value || "none"),
  })).filter((entry) => entry.name);
  if (!entries.length) {
    showAppMessage("Add at least one area name in the table.", "warning", "Areas");
    return;
  }
  const now = new Date().toISOString();
  for (const entry of entries) {
    if (entry.floor && !normalizeFloorList(project.floors).some((floor) => floor.toLowerCase() === entry.floor.toLowerCase())) {
      project.floors = normalizeFloorList([...(project.floors || []), entry.floor]);
    }
    project.areas.unshift({
      id: crypto.randomUUID(),
      name: entry.name,
      iconKey: entry.iconKey || inferAreaIconFromName(entry.name),
      floor: entry.floor,
      createdAt: now,
      items: [],
      teamIds: [],
    ...
    logAudit("Area Created", { objectType: "area", objectName: entry.name, projectId: project.id });
  }
  selectedProjectAreaId = project.areas[0]?.id || selectedProjectAreaId;
  currentWorkspaceTab = "folders-hub";
  currentProjectDetailsTab = "areas";
  closeAreaDialog(true);
  persist();
  render();
  showAppMessage(`${entries.length} areas created.`, "success", "Areas");
}
...
function render() {
  ...
  if (project) {
    ...
    syncProjectContactOverrideFields(null);
    rememberFormSnapshot("project", els.projectForm);
  }
  renderViews();
...
function renderProjectSetupState() {
  ...
  syncProjectContactOverrideFields(getCurrentProject());
...
function renderViews() {
  ...
  const views = {
    ...
    "ai-assistant": els.aiAssistantView,
    teams: els.teamsView,
  ...
  els.appShell?.classList.remove("view-projects", "view-planner", "view-daily-works", "view-ai-assistant", "view-teams", "view-equipment", "view-clients", "view-theme", "view-audit");
...
function hydrateProjectForm(project) {
  ...
  if (els.projectAddressDifferent) els.projectAddressDifferent.checked = Boolean(project.addressDiffersFromClient);
  if (els.projectPhoneDifferent) els.projectPhoneDifferent.checked = Boolean(project.phoneDiffersFromClient);
  if (els.projectAddress) els.projectAddress.value = project.address || "";
  if (els.projectPhone) els.projectPhone.value = project.phone || "";
  syncProjectContactOverrideFields(project);
}
...
function syncProjectContactOverrideFields(project = getCurrentProject()) {
  const client = getClientById(els.projectClientSelect?.value || project?.clientId || "");
  const addressDifferent = Boolean(els.projectAddressDifferent?.checked);
  const phoneDifferent = Boolean(els.projectPhoneDifferent?.checked);
  const locked = !project || !canManageProject(project) || isDraftProjectLocked(project);
  if (els.projectAddressDifferent) els.projectAddressDifferent.disabled = locked;
  if (els.projectPhoneDifferent) els.projectPhoneDifferent.disabled = locked;
  if (els.projectAddress) {
    els.projectAddress.disabled = locked || !addressDifferent;
    if (addressDifferent) {
      if (!els.projectAddress.value && project?.address) els.projectAddress.value = project.address;
    } else {
      els.projectAddress.value = client?.address || "";
    }
    els.projectAddress.placeholder = addressDifferent ? "Project site address" : "Inherited from selected client";
  }
  if (els.projectPhone) {
    els.projectPhone.disabled = locked || !phoneDifferent;
    if (phoneDifferent) {
      if (!els.projectPhone.value && project?.phone) els.projectPhone.value = project.phone;
    } else {
      els.projectPhone.value = client?.tel || "";
    }
    els.projectPhone.placeholder = phoneDifferent ? "Project site telephone" : "Inherited from selected client";
  }
}
...
function collectProjectSearchValues(project) {
  ...
  return [
    ...
    project.address,
    project.phone,
    getResolvedProjectAddress(project),
    getResolvedProjectPhone(project),
    ...clientValues,
    ...managerValues,
    ...memberValues,
    ...areaValues,
...
function renderDailyWorkFormShortcuts() {
  ...
  const address = String(els.dailyWorkAddress?.value || "").trim();
  const mapLink = normalizeUrlValue(els.dailyWorkMapLink?.value || buildGoogleMapsSearchLink(address));
  els.dailyWorkContactShortcuts.classList.toggle("hidden", !phone && !address && !mapLink);
  els.dailyWorkContactShortcuts.innerHTML = `
    ${address ? `<span class="meta-pill">${escapeHtml(address)}</span>` : ""}
    ${mapLink ? `<a class="ghost-btn" href="${escapeHtml(mapLink)}" target="_blank" rel="noopener noreferrer">Go</a>` : ""}
  ...
  if (els.dailyWorkMapOpenBtn) {
    ...
    els.dailyWorkMapOpenBtn.textContent = "Go";
  }
}
...
function buildAddressActionMarkup(address, options = {}) {
  ...
  return `
    <span class="${escapeHtml(options.className || "meta-pill project-contact-action-pill")}">
      ...
      <a href="${escapeHtml(mapLink)}" target="_blank" rel="noopener noreferrer">${escapeHtml(buttonLabel)}</a>
    </span>
  `;
}
...
function buildPhoneActionMarkup(phone, options = {}) {
  ...
  return `
    <span class="${escapeHtml(options.className || "meta-pill project-contact-action-pill")}">
      ...
      <a href="tel:${escapeHtml(cleanPhone)}">${escapeHtml(buttonLabel)}</a>
    </span>
  `;
}
...
function getResolvedProjectAddress(project) {
  const client = getProjectClient(project);
  return (project?.addressDiffersFromClient ? project.address : client?.address || "").trim();
}

function getResolvedProjectPhone(project) {
  const client = getProjectClient(project);
  return normalizePhoneValue(project?.phoneDiffersFromClient ? project.phone : client?.tel || "");
}
...
function openDailyWorkMapLink() {
  const mapLink = normalizeUrlValue(els.dailyWorkMapLink?.value || buildGoogleMapsSearchLink(els.dailyWorkAddress?.value || ""));
  if (!mapLink) {
    showAppMessage("Add an address or Google Maps link first.", "warning", "Daily Works");
    return;
  }
...
function onDailyWorkSave(event) {
  ...
  const draft = createDailyWork({
    ...
    address: els.dailyWorkAddress?.value || "",
    phone: els.dailyWorkPhone?.value || "",
    mapLink: els.dailyWorkMapLink?.value || buildGoogleMapsSearchLink(els.dailyWorkAddress?.value || ""),
...
function buildDailyWorkCard(work) {
  ...
  const phoneLink = work.phone ? `<span class="daily-work-phone-inline">Tel: ${escapeHtml(work.phone)}</span><a href="tel:${escapeHtml(work.phone)}" class="daily-work-contact-link">Call</a>` : "";
  const mapUrl = normalizeUrlValue(work.mapLink || buildGoogleMapsSearchLink(work.address || ""));
  const mapLink = mapUrl ? `<a href="${escapeHtml(mapUrl)}" target="_blank" rel="noopener noreferrer" class="daily-work-contact-link">Go</a>` : "";
  ...
  for (const contact of contacts) {
    ...
    row.innerHTML = `
      ...
      <div class="daily-work-contact-actions">
        ...
        ${contact.mapLink ? `<a href="${escapeHtml(contact.mapLink)}" target="_blank" rel="noopener noreferrer" class="ghost-btn">Go</a>` : ""}
...
function renderWorkspaceTabs(project) {
  ...
  if (els.workspaceAiVoiceBtn) {
    els.workspaceAiVoiceBtn.classList.toggle("hidden", !(currentView === "projects" && Boolean(project)));
  }
...
function renderProjectMeta(project) {
  const client = getProjectClient(project);
  const address = getResolvedProjectAddress(project);
  const tel = getResolvedProjectPhone(project);
