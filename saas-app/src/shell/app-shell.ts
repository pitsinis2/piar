...
const directoryData: Record<string, DirectorySection> = {
  ...
  "ai-assistant": {
    title: "AI Assistant",
    items: [
      { name: "Offer draft from speech", subtitle: "Draft - needs review", details: ["Speech command: start offer draft", "Pause mode follows user settings", "Always edit before sending"] },
      { name: "Voice note / project summary", subtitle: "Project and area notes", details: ["Speech command: start project summary", "Summarizes the spoken content", "User chooses save target"] },
      { name: "Material list from speech", subtitle: "Unclear items need check", details: ["Speech command: start material list", "End command: over", "Unclear quantities stay marked as needs check"] },
    ],
  },
  audit: {
    title: "Audit Log",
...
function memberDisplay(member: TeamMemberChoice) {
  ...
  const state = {
    ...
    assistantSettings: {
      language: "English",
      pauseMode: "Long pause mode",
      customPause: "6",
      startWord: "start",
      endWord: "over",
      defaultSaveTarget: "Project notes",
  ...
  function loadPreviewState() {
    try {
      ...
      if (saved.assistantSettings) {
        Object.assign(state.assistantSettings, saved.assistantSettings);
      }
    } catch (error) {
  ...
  function renderRail() {
    return `
      <aside class="saas-sidebar">
        ...
        <button class="rail-tab ${state.currentSection === "ai-assistant" ? "active" : ""}" data-section="ai-assistant">AI Assistant</button>
  ...
  function directorySubtitle(section: string, index: number) {
    const subtitles: Record<string, string[]> = {
      ...
      "ai-assistant": ["Draft - needs review", "Project and area notes", "Unclear items need check"],
  ...
  function renderDirectoryWorkspace() {
    ...
    if (state.currentSection === "ai-assistant") return renderAssistantWorkspace();
  ...
  function renderAssistantWorkspace() {
    ...
    return `
      <section class="project-workspace directory-workspace">
        <header class="project-hero-card ai-hero-card">
          <div>
            <h1>AI Assistant</h1>
            <p>Speech to safe editable drafts</p>
            <p>${escapeHtml(settings.language)} | ${escapeHtml(settings.pauseMode)} | "${escapeHtml(settings.startWord)}" to start, "${escapeHtml(settings.endWord)}" to finish</p>
          </div>
          <div class="hero-actions">
            <button data-modal="ai-assistant">New draft</button>
            <button data-modal="ai-settings">Settings</button>
          </div>
        </header>
        <section class="assistant-mode-grid">
          ${assistantModeCard("Offer draft from speech", "Draft - needs review", "Turn spoken site notes into an offer draft that cannot be sent until reviewed.", "ai-offer")}
          ${assistantModeCard("Voice note / project summary", "Editable note draft", "Summarize long spoken updates for project notes or area notes.", "ai-summary")}
          ${assistantModeCard("Material list from speech", "Needs check markers", "Capture materials from natural speech and keep unclear items flagged.", "ai-material")}
        </section>
        <section class="assistant-settings-strip">
          <article>
            <strong>Pause behavior</strong>
            <span>${escapeHtml(settings.pauseMode)} (${escapeHtml(settings.customPause)} sec custom)</span>
          </article>
          <article>
            <strong>Language rule</strong>
            <span>${escapeHtml(settings.language)} controls speech input and final output.</span>
          </article>
          <article>
            <strong>Default save target</strong>
            <span>${escapeHtml(settings.defaultSaveTarget)}</span>
          </article>
        </section>
      </section>
    `;
  ...
  function assistantModeCard(title: string, badge: string, copy: string, modal: string) {
    return `
      <article class="content-card assistant-mode-card">
        <span>${escapeHtml(badge)}</span>
        <h2>${escapeHtml(title)}</h2>
        <p>${escapeHtml(copy)}</p>
        <button class="primary-action" data-modal="${modal}">Start</button>
      </article>
    `;
  }
    ...
    return `
      <header class="project-hero-card" style="--project-surface:${project.color}">
        <div>
          ...
          <div class="hero-actions">
          <button data-modal="ai-project">AI</button>
  ...
  function renderAreaTabs(project: Project) {
    ...
    return `
      <nav class="area-tabs">
        ...
        <button class="filter-btn" data-modal="ai-voice">Voice</button>
  ...
  function renderModal(project: Project, area: ProjectArea) {
    ...
    const titleMap: Record<string, string> = {
      ...
      "ai-assistant": "AI Assistant",
      "ai-project": "Project AI",
      "ai-voice": "Voice Assistant",
      "ai-offer": "Offer draft from speech",
      "ai-summary": "Voice note / project summary",
      "ai-material": "Material list from speech",
  ...
  function modalBody(project: Project, area: ProjectArea) {
    ...
    if (["ai-assistant", "ai-project", "ai-voice", "ai-offer", "ai-summary", "ai-material"].includes(state.modal)) {
      return renderAssistantDraftForm(project, area, state.modal);
    }
    if (state.modal === "ai-settings") {
      return renderAssistantSettingsForm();
    }
    ...
    if (state.modal === "new-area") {
      return `
        ...
        <label>Floor<input data-area-floor placeholder="1st floor"></label>
    ...
    return `
      ...
      <section class="form-panel">
        <div class="form-row three">
          ...
          <label>Start date<input data-project-start type="date" value="2026-05-03"></label>
        </div>
        <label>Address<input data-project-address placeholder="Street, city"></label>
        <div class="form-row">
          <label>Client<input data-project-client placeholder="Client name"></label>
          <label>Project manager<input data-project-manager placeholder="Manager"></label>
        </div>
      </section>
      <section class="form-panel">
        <h3>Project Color</h3>
      ...
      <section class="form-panel">
        ...
        <div class="form-row">
          ...
          <label>Floor<input data-project-first-floor placeholder="1st floor"></label>
        </div>
      </section>
      <div class="modal-footer">
        <button data-close>Cancel</button>
  ...
  function renderAssistantDraftForm(project: Project, area: ProjectArea, modal: string) {
    const settings = state.assistantSettings;
    const mode = modal === "ai-material" || modal === "ai-voice"
      ? "Material list from speech"
      : modal === "ai-offer" || modal === "ai-project"
        ? "Offer draft from speech"
        : modal === "ai-summary"
          ? "Voice note / project summary"
          : "Choose assistant mode";
    const draftText = mode === "Material list from speech"
      ? "- 8 m copper pipe, needs check\n- 12 wall brackets\n- Silicone for bathroom joints, needs check"
      : mode === "Offer draft from speech"
        ? "Draft - needs review\nPrepare an offer for the selected project based on the spoken site notes. Include labor, materials, and open questions before sending."
        : "Project summary draft\nTechnician reported progress in the selected area. Review the summary, edit details, then save it to the chosen target.";
    return `
      <section class="assistant-capture">
        <div class="assistant-command-row">
          <span>Say "${escapeHtml(settings.startWord)} ${escapeHtml(mode.toLowerCase())}"</span>
          <span>Say "${escapeHtml(settings.endWord)}" when finished</span>
          <span>${escapeHtml(settings.pauseMode)}</span>
        </div>
        <label>Assistant mode
          <select data-ai-mode>
            <option ${mode === "Offer draft from speech" ? "selected" : ""}>Offer draft from speech</option>
            <option ${mode === "Voice note / project summary" ? "selected" : ""}>Voice note / project summary</option>
            <option ${mode === "Material list from speech" ? "selected" : ""}>Material list from speech</option>
          </select>
        </label>
        <label>Spoken text preview
          <textarea data-ai-spoken rows="4">Start ${mode.toLowerCase()}. Work in ${area.name} for ${project.name}. Please prepare the draft and mark unclear parts. ${settings.endWord}.</textarea>
        </label>
        <label>AI draft
          <textarea data-ai-draft rows="7">${escapeHtml(draftText)}</textarea>
        </label>
        <label>Save target
          <select data-ai-target>
            <option ${settings.defaultSaveTarget === "Project notes" ? "selected" : ""}>Project notes</option>
            <option ${settings.defaultSaveTarget === "Area notes" ? "selected" : ""}>Area notes</option>
            <option ${settings.defaultSaveTarget === "Material list to order" ? "selected" : ""}>Material list to order</option>
            <option ${settings.defaultSaveTarget === "Offer draft" ? "selected" : ""}>Offer draft</option>
          </select>
        </label>
        <p class="assistant-safety-note">AI output is always a draft. Review and edit before saving or sending.</p>
        <div class="modal-footer">
          <button data-close>Cancel</button>
          <button class="primary-action" data-save-ai-draft>Save draft</button>
        </div>
      </section>
    `;
  }

  function renderAssistantSettingsForm() {
    const settings = state.assistantSettings;
    return `
      <section class="form-panel assistant-settings-form">
        <div class="form-row three">
          <label>Assistant language
            <select data-ai-language>
              ${["English", "German", "Greek", "Italian"].map((language) => `<option ${settings.language === language ? "selected" : ""}>${language}</option>`).join("")}
            </select>
          </label>
          <label>Pause mode
            <select data-ai-pause-mode>
              ${["Short pause mode", "Normal pause mode", "Long pause mode", "Custom pause time"].map((mode) => `<option ${settings.pauseMode === mode ? "selected" : ""}>${mode}</option>`).join("")}
            </select>
          </label>
          <label>Custom pause seconds<input data-ai-custom-pause type="number" min="1" value="${escapeHtml(settings.customPause)}"></label>
        </div>
        <div class="form-row three">
          <label>Magic start word<input data-ai-start-word value="${escapeHtml(settings.startWord)}"></label>
          <label>Magic end word<input data-ai-end-word value="${escapeHtml(settings.endWord)}"></label>
          <label>Default save target
            <select data-ai-default-target>
              ${["Project notes", "Area notes", "Material list to order", "Offer draft"].map((target) => `<option ${settings.defaultSaveTarget === target ? "selected" : ""}>${target}</option>`).join("")}
            </select>
          </label>
        </div>
      </section>
      <div class="modal-footer">
        <button data-close>Cancel</button>
        <button class="primary-action" data-save-ai-settings>Save settings</button>
      </div>
    `;
  }
  ...
  function bindEvents() {
    ...
    root!.querySelector<HTMLElement>("[data-add-area]")?.addEventListener("click", () => {
      const project = selectedProject();
      const areaName = root!.querySelector<HTMLInputElement>("[data-area-name]")?.value.trim() || "New area";
      const areaFloor = root!.querySelector<HTMLInputElement>("[data-area-floor]")?.value.trim() || "No floor";
    ...
    root!.querySelector<HTMLElement>("[data-add-project]")?.addEventListener("click", () => {
      ...
      const start = root!.querySelector<HTMLInputElement>("[data-project-start]")?.value || "2026-05-03";
      const address = root!.querySelector<HTMLInputElement>("[data-project-address]")?.value || "New address";
      const client = root!.querySelector<HTMLInputElement>("[data-project-client]")?.value || "New client";
      const manager = root!.querySelector<HTMLInputElement>("[data-project-manager]")?.value || "Company admin";
      const firstArea = root!.querySelector<HTMLInputElement>("[data-project-first-area]")?.value || "";
      const firstFloor = root!.querySelector<HTMLInputElement>("[data-project-first-floor]")?.value || "";
    ...
    root!.querySelector<HTMLElement>("[data-save-ai-settings]")?.addEventListener("click", () => {
      state.assistantSettings = {
        language: root!.querySelector<HTMLSelectElement>("[data-ai-language]")?.value || "English",
        pauseMode: root!.querySelector<HTMLSelectElement>("[data-ai-pause-mode]")?.value || "Long pause mode",
        customPause: root!.querySelector<HTMLInputElement>("[data-ai-custom-pause]")?.value || "6",
        startWord: root!.querySelector<HTMLInputElement>("[data-ai-start-word]")?.value.trim() || "start",
        endWord: root!.querySelector<HTMLInputElement>("[data-ai-end-word]")?.value.trim() || "over",
        defaultSaveTarget: root!.querySelector<HTMLSelectElement>("[data-ai-default-target]")?.value || "Project notes",
      };
      state.modal = "";
      render();
    });
    root!.querySelector<HTMLElement>("[data-save-ai-draft]")?.addEventListener("click", () => {
      const mode = root!.querySelector<HTMLSelectElement>("[data-ai-mode]")?.value || "Voice note / project summary";
      const target = root!.querySelector<HTMLSelectElement>("[data-ai-target]")?.value || "Project notes";
      const draft = root!.querySelector<HTMLTextAreaElement>("[data-ai-draft]")?.value.trim() || "AI draft";
      const lowerMode = mode.toLowerCase();
      const kind = target === "Material list to order" ? "file" : target === "Offer draft" ? "plan" : "note";
      const badge = target === "Material list to order" ? "MATERIAL" : target === "Offer draft" ? "OFFER" : "NOTE";
      state.items.push({
        projectId: state.selectedProjectId,
        areaId: target === "Project notes" || target === "Offer draft" ? "info" : selectedArea().id,
        kind,
        badge,
        name: mode === "Offer draft from speech" ? "Draft - needs review" : mode,
        meta: `${target} | ${lowerMode.includes("material") ? "needs check markers kept" : "editable AI draft"} | ${draft.slice(0, 80)}`,
      });
      state.modal = "";
      render();
    });
